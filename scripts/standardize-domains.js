...
const DOMAIN_METADATA = {
  ...
  9: {
    ...
    tier: 2,
    description: 'Understanding how components interact at the molecular level',
  },
  10: {
  ...
  11: {
    ...
    tier: 2,
    description: 'Understanding uncertainty and making probabilistic decisions',
  },
  12: {
  ...
  28: {
    ...
    tier: 3,
    description: 'Understanding historical patterns and institutional change',
  },
  29: {
  ...
  34: {
    ...
    tier: 3,
    description: 'Understanding spatial relationships and geographic factors',
  },
  35: {
...
function standardizeModel(model, domainId) {
  return {
    modelId:
      model.modelId ||
      `domain-${domainId}-${model.modelTitle?.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
    ...
    tags: model.tags || [],
    curatedHighlights:
      model.curatedHighlights?.map(highlight => ({
  ...
  files.forEach(file => {
    if (
      ...
      if (file.endsWith('.json')) {
        ...
        console.log(
          'Skipping corrupted domains 3 to 5.md file (Microsoft Word format)'
        );
      } else {
...
function main() {
  ...
  console.log(
    `📊 Found ${models.length} models across ${new Set(models.map(m => m.domainId)).size} domains`
  );
  ...
  DOMAIN_GROUPS.forEach(group => {
    ...
    console.log(
      `✅ Created ${group.name}.md with ${groupModels.length} models`
    );
  });
  ...
  const summary = {
    ...
    missingDomains: Array.from({ length: 40 }, (_, i) => i + 1).filter(
      id => !modelsByDomain[id]
    ),
    domainGroups: DOMAIN_GROUPS.map(group => ({
      ...
      models: Array.from(
        ...
        .map(domainId => ({
          domainId,
          modelCount: modelsByDomain[domainId]?.length || 0,
        })),
    })),
  };
