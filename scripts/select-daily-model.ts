...
function loadQueueFile(): QueueFile {
  ...
  if (!q)
    throw new Error(`Missing/empty queue at ${QUEUE_PATH}. Run --build first.`);
  if (Array.isArray(q)) {
    throw new Error(
      `Legacy array-format queue at ${QUEUE_PATH} — re-run --build to write the epoch-stamped format.`
    );
  }
  if (
    !q.epoch ||
    !DATE_RE.test(q.epoch) ||
    !Array.isArray(q.models) ||
    q.models.length === 0 ||
    typeof q.parked !== 'number'
  ) {
    throw new Error(
      `Malformed queue file at ${QUEUE_PATH} (need { epoch, parked, models[] }). Run --build.`
    );
  }
  return q;
}
...
export function publishedUses(slug: string, beforeDate: string): string[] {
  let files: string[];
  try {
    files = fs.readdirSync(UPDATES_DIR);
  } catch {
    return [];
  }
  const out: string[] = [];
  for (const f of files.sort()) {
    ...
    try {
      if (fs.readFileSync(path.join(UPDATES_DIR, f), 'utf8').includes(slug))
        out.push(m[1]!);
    } catch {
      /* skip */
    }
...
export function refusalReason(slug: string, dateStr: string): string | null {
  ...
  if (since < SHORT_COOLDOWN_DAYS)
    return `published ${last} (${since}d ago) — inside the ${SHORT_COOLDOWN_DAYS}-day cooldown`;
...
export function select(dateStr: string): {
  date: string;
  queueIndex: number;
  skippedFrom?: number;
  skipNote?: string;
  ...
  return {
    ...
    queueIndex: base,
    skippedFrom: base,
    skipNote: `NO CLEAN SLUG IN QUEUE — shipping the base assignment. ${notes[0]}`,
    ...m,
  };
}

function appendLedger(
  pick: { date: string; queueIndex: number } & Model
): void {
...
function invariantFails(q: QueueFile, ledger: LedgerRow[]): string[] {
  ...
  if (len < 100)
    fails.push(
      `queue length ${len} < 100 — cannot guarantee a 100-day no-repeat`
    );
  if (new Set(q.models.map(m => m.slug)).size !== len)
    fails.push('queue contains duplicate slugs');
  if (q.parked < 0 || q.parked > len)
    fails.push(`parked=${q.parked} out of range`);
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    for (let j = i; j < Math.min(i + 100, days.length); j++) {
      if (seen.has(days[j]!.slug)) {
        fails.push(
          `slug ${days[j]!.slug} repeats within 100 days of index ${i}`
        );
        break;
      }
  ...
  for (let i = 1; i < headLen; i++)
    if (days[i]!.domain === days[i - 1]!.domain) consecSameHead++;
  if (consecSameHead > 0)
    fails.push(
      `${consecSameHead} consecutive same-domain day-pairs in the head (expected 0)`
    );

  // (3) Wrap: day len returns to queue[0].
  if (days[len]!.slug !== q.models[0]!.slug)
    fails.push(`day ${len} did not wrap to queue[0]`);
  ...
  if (firstSig !== -1) {
    ...
    if (nextSig !== -1 && nextSig - firstSig < 100)
      fails.push(`${sig} recurs after only ${nextSig - firstSig} days`);
  }
  ...
  for (const r of ledger) {
    ...
    const gap = Math.round(
      (utcDay(q.epoch) + idx * DAY_MS - utcDay(r.date)) / DAY_MS
    );
    if (gap < 100) {
      fails.push(
        `${r.slug} played ${r.date} would replay after only ${gap} days (queue index ${idx}) — parking failed`
      );
    }
  }
...
function parseCatalog(md: string): Model[] {
  ...
  for (const ln of md.split('\n')) {
    ...
    if (mm && domain)
      models.push({ name: mm[1].trim(), slug: mm[2].trim(), domain });
  }
  return models;
}
...
function buildQueue(): QueueFile {
  ...
  for (const r of ledger) {
    if (!r?.slug || !r?.date || !isValidDate(r.date) || r.date >= epoch)
      continue;
    if (!lastPlay.has(r.slug) || r.date > lastPlay.get(r.slug)!)
      lastPlay.set(r.slug, r.date);
  }
  ...
  if (fails.length) {
    throw new Error(
      '--build produced a queue that violates the rotation invariants — refusing to write:\n  - ' +
        fails.join('\n  - ')
    );
  }
  return qf;
...
function selftest(): number {
  ...
  const t = (ok: boolean, label: string) => {
    ...
    if (!ok) lifeFails.push(label);
  };
  const REPEATED = 'levels-of-emergence-scale-transitions';
  const uses = publishedUses(REPEATED, '2026-08-01');
  t(
    uses.length >= LIFETIME_THRESHOLD,
    `[IMP-112] archive shows ${uses.length} prior published uses of ${REPEATED} (${uses.join(', ')}) — the 7-row ledger showed 0`
  );
  t(
    refusalReason(REPEATED, '2026-08-01') !== null,
    `[IMP-112] refusalReason FIRES on the 08-01 assignment (5th appearance, 34d after 06-28 — clears 30d, fails 180d)`
  );
  const fresh = q.models.find(
    m => publishedUses(m.slug, '2026-08-01').length === 0
  );
  t(
    !!fresh && refusalReason(fresh.slug, '2026-08-01') === null,
    `[IMP-112] refusalReason SILENT on a never-published slug (${fresh ? fresh.slug : 'none found'}) — the gate does not refuse everything`
  );
  const pick0801 = select('2026-08-01');
  t(
    pick0801.slug !== REPEATED,
    `[IMP-112] select('2026-08-01') no longer assigns the 5x-repeated slug (now ${pick0801.slug}, skippedFrom ${pick0801.skippedFrom})`
  );
  t(
    refusalReason(pick0801.slug, '2026-08-01') === null,
    `[IMP-112] the skip replacement itself clears both cooldowns`
  );
  ...
  t(
    pick0801.slug !== pick0802.slug,
    `[IMP-112] the skip does not collide with tomorrow's assignment (08-01 ${pick0801.slug} vs 08-02 ${pick0802.slug})`
  );
  if (lifeFails.length) {
    console.error(
      'SELFTEST FAIL (IMP-112 lifetime-use):\n  - ' + lifeFails.join('\n  - ')
    );
    return 1;
  }
  ...
  if (arg === '--selftest') {
    ...
    if (arg === '--date') {
      if (!process.argv[3] || !isValidDate(process.argv[3])) {
        console.error(
          `Invalid --date ${JSON.stringify(process.argv[3] ?? '')} — expected a real calendar date, YYYY-MM-DD.`
        );
        process.exit(1);
      }
      ...
      console.error(
        `Unknown argument ${JSON.stringify(arg)}. Usage: [--date YYYY-MM-DD] | --selftest | --build`
      );
      process.exit(1);
    } else {
      dateStr = todayET();
    ...
    console.log(
      JSON.stringify(isToday ? pick : { ...pick, peek: true }, null, 2)
    );
  }
}
