...
const modelsMatch = readwiseContent.match(
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (content.startsWith('[') || content.startsWith('{')) {
        ...
        if (models.length > 0) {
          console.log(
            `  ✅ Loaded ${models.length} models from JSON file: ${file}`
          );
        }
      } else if (ext === '.md') {
        ...
        if (models.length > 0) {
          console.log(
            `  ✅ Loaded ${models.length} models from markdown: ${file}`
          );
        }
      }
...
const slugMappingPath = path.join(
  __dirname,
  '..',
  'lib',
  'parse-all-domains.ts'
);
const slugMappingContent = fs.readFileSync(slugMappingPath, 'utf8');

// Extract the MODEL_SLUG_MAPPINGS object
const mappingMatch = slugMappingContent.match(
  /const MODEL_SLUG_MAPPINGS: \{ \[key: string\]: string \} = \{([\s\S]*?)\};/
);
let slugToIdMap = {};
...
if (mappingMatch) {
  ...
  console.log(
    `✅ Loaded ${Object.keys(slugToIdMap).length} slug mappings from parse-all-domains.ts\n`
  );
}
    ...
    .forEach(domain => {
      ...
      domainModels.forEach((model, index) => {
        ...
        if (highlights && highlights.curatedHighlights.length > 0) {
          ...
          const sortedHighlights = [...highlights.curatedHighlights].sort(
...
const outputPath = path.join(
  __dirname,
  '..',
  'MENTAL_MODELS_READWISE_HIGHLIGHTS.md'
);
