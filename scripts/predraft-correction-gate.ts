...
export function extractCorrections(
  ...
  const push = (wrong: string, right: string) => {
    ...
    seen.add(wrong);
    out.push({ wrong, right, component });
  };
...
function runOnDate(date: string, stage: string, root = process.cwd()): number {
  ...
  if (!draftPath) {
    console.error(`predraft-correction-gate: no ${stage} for ${date}`);
    return 2;
  }
  ...
  for (const c of COMPONENTS) {
    ...
    if (fs.existsSync(p))
      all = all.concat(extractCorrections(fs.readFileSync(p, 'utf8'), c));
  }
  const bad = violations(draft, all);
  console.log(
    `predraft-correction-gate ${date} — ${path.basename(draftPath)} · ${all.length} recorded correction(s) across pre-drafts`
  );
  for (const v of bad) {
    console.error(
      `  ✗ 🔴 correction-reverted [${v.component}]: the draft still contains "${v.wrong}", which the pre-draft's own fact-check superseded with "${v.right}". Use the corrected text.`
    );
  }
  if (bad.length) {
    console.error(
      `\n✗ ${bad.length} SUPERSEDED value(s) survived into ${stage}. Replace with the pre-draft's corrected text.`
    );
    return 1;
  }
...
function selftest(): number {
  ...
  t(
    'extracts the real 08-01 FIXED-to pair',
    cs.some(c => c.wrong.includes('46%') && c.right.includes('40%'))
  );
  t(
    'extracts the real 08-01 arrow pair',
    cs.some(c => c.wrong === 'in February' && c.right === 'in early 2026')
  );
  t(
    'FIRES on the pre-correction figure (the real 08-01 v1 bug)',
    violations('trades roughly 46% below its 2025 high after a drawdown', cs)
      .length === 1
  );
  t(
    'SILENT on the corrected figure (the real 08-01 published text)',
    violations(
      'trades about 40% below its 2025 high after a drawdown near 56% in early 2026',
      cs
    ).length === 0
  );
  t(
    'SILENT when no corrections are recorded',
    violations('any text at all', extractCorrections('no fact-check here'))
      .length === 0
  );
  t(
    'ignores trivially short strings',
    extractCorrections('"a" FIXED to "b"').length === 0
  );
  t(
    'does not double-count a repeated correction',
    extractCorrections(real + '\n' + real).length === cs.length
  );
  console.log(
...
function main(): number {
  ...
  if (!date) {
    console.error(
      'usage: predraft-correction-gate.ts <YYYY-MM-DD> [--stage v1|v1.5|v2|published] | --selftest'
    );
    return 2;
  }
