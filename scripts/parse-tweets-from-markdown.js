...
function parseTweetsFromMarkdown(markdownPath) {
  ...
  for (let i = 1; i < promptSections.length; i++) {
    ...
    const urlMatch = section.match(
      /https:\/\/www\.cosmictrex\.xyz\/models\/([^\s]+)/
    );
    const slug = urlMatch
      ? urlMatch[1]
    ...
    const tweetMatches = section.matchAll(
      /\*\*Tweet \d+:\*\*\s*\n\n([\s\S]*?)(?=\*\*Tweet \d+:|$)/g
    );
    ...
    for (const match of tweetMatches) {
      ...
      const fromBookMatch = fullTweet.match(
        /From ([^:]+):\s*\n\n"([^"]+)"\s*\n\n([^\n]+)/
      );
      ...
      if (fromBookMatch && invitationMatch) {
        ...
        if (parts.length >= 3) {
          ...
          const modelNameIdx = parts.findIndex(
            p => p.includes(modelName) && !p.includes('From ')
          );

          if (modelNameIdx >= 0) {
            part1 = parts
              .slice(0, modelNameIdx + 1)
              .join('\n\n')
              .trim();
            ...
            if (quoteStart >= 0) {
              const invStart = parts.findIndex(p =>
                invitationPatterns.some(pat => pat.test(p))
              );
              if (invStart >= 0) {
          ...
          if (part1 && part2 && part3) {
            ...
            console.warn(
              `⚠️  Could not parse tweet for ${modelName}, using fallback`
            );
            const trimmed =
              fullTweet.length > 280
                ? fullTweet.substring(0, 277) + '...'
                : fullTweet;
            modelTweets.push({
              number: tweetNum++,
              text: trimmed,
...
function saveTweetsForPosting(tweets, outputDir) {
  ...
  for (let modelIdx = 0; modelIdx < tweets.length; modelIdx++) {
    ...
    for (let threadIdx = 0; threadIdx < threads.length; threadIdx++) {
      ...
      const filePath = path.join(
        dayDir,
        `${tweet.slug}-thread-${threadIdx + 1}.json`
      );
      ...
      console.log(
        `✅ Scheduled ${tweet.model} - Thread ${threadIdx + 1} for ${dayNames[threadDay]} ${dateStr} at ${timeStr}`
      );
    }
  }
  ...
  console.log(
    `📊 ${tweets.length} models scheduled for week of ${nextMonday.toISOString().split('T')[0]}`
  );
...
console.log(
  `✅ Parsed ${tweets.length} models with ${tweets.reduce((sum, t) => sum + t.totalTweets, 0)} total tweets\n`
);
