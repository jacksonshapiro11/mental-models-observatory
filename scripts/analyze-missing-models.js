...
function countModelsInFile(filePath, fileName) {
  try {
    ...
    if (modelIdMatches) {
      ...
      console.log(
        `${fileName}: ${modelIdMatches.length} models (found "modelId" patterns)`
      );
    ...
    if (sectionMatches) {
      ...
      console.log(
        `${fileName}: ${sectionMatches.length} models (found ### section patterns)`
      );
    ...
    if (jsonBlockMatches) {
      ...
      console.log(
        `${fileName}: ${jsonBlockMatches.length} models (found JSON blocks)`
      );
