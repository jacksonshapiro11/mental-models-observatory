...
console.log(
  '📖 Loading new content from Mental Models Description/ by CODE...\n'
);
...
console.log(
  `✅ Loaded ${Object.keys(newContentByCode).length} models from description files\n`
);
...
for (const code in newContentByCode) {
  if (modelsByCode[code]) {
    ...
    console.log(
      `❌ CODE ${code}: Not found in readwise-data.ts (${newContentByCode[code].name})`
    );
  }
}
...
for (const mismatch of nameMismatches) {
  ...
  if (descContent && dataModel) {
    ...
    const descSample = descContent.description
      .substring(0, 200)
      .toLowerCase()
      .replace(/\s+/g, ' ');
    const dataSample = dataModel.description
      .substring(0, 200)
      .toLowerCase()
      .replace(/\s+/g, ' ');
    ...
    if (similarity > 0.5) {
      ...
      console.log(
        `✅ CODE ${code}: SUBSTANCE MATCHES (${(similarity * 100).toFixed(1)}% similarity)`
      );
      console.log(`   Description file: "${mismatch.descriptionFile}"`);
      console.log(`   readwise-data.ts: "${mismatch.readwiseData}"`);
      console.log(
        `   → Names differ but content is the SAME. Will update content by code.\n`
      );
    } else {
      console.log(
        `❌ CODE ${code}: SUBSTANCE DIFFERS (${(similarity * 100).toFixed(1)}% similarity)`
      );
      console.log(`   Description file: "${mismatch.descriptionFile}"`);
      console.log(`   readwise-data.ts: "${mismatch.readwiseData}"`);
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (currentModelCode && newContentByCode[currentModelCode]) {
    ...
    if (line.includes('"description":')) {
      ...
      const escaped = updated.description
        .replace(/\\/g, '\\\\')
        .replace(/"/g, '\\"');
    ...
    if (inArray) {
      if (line.trim() === '],') {
        ...
        if (arrayType === 'applications') {
          updateCount++;
          console.log(
            `✏️  CODE ${currentModelCode}: ${modelsByCode[currentModelCode]?.name || 'Unknown'}`
          );
...
console.log('\n📋 Summary:');
console.log(
  `   - Models in description files: ${Object.keys(newContentByCode).length}`
);
