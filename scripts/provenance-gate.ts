...
const ABSENCE =
  /\b(absent|missing|not found|none found|does not exist|no pre-?draft|was ?n[o']?t produced|generated inline|inline generation|no such draft)\b/i;
 ...
 */
export function absenceFabrications(
  v1: string,
  state: Record<Component, DraftState>
): Finding[] {
  const out: Finding[] = [];
  ...
  for (const comp of COMPONENTS) {
    ...
    const asserted = lines.some(
      l => compRe.test(l) && ABSENCE.test(l) && !NEGATIVE_DECLARATION.test(l)
    );
    const s = state[comp];
...
export function extractModelSlug(v1: string): string | null {
  const explore = v1.match(
    /Explore this model\]\([^)]*\/models\/([a-z0-9-]+)/i
  );
...
export function modelRecency(
  ...
  const age = Math.floor(
    (Date.parse(today + 'T00:00:00Z') - Date.parse(lastUse + 'T00:00:00Z')) /
      86400000
  );
...
function runOnDate(date: string, root = process.cwd()): number {
  ...
  if (!v1Path) {
    console.error(
      `provenance-gate: no v1 for ${date} (looked for -v1.md and -v1-pre-quality-gate.md)`
    );
    return 2;
  }
  ...
  console.log(
    `provenance-gate ${date} — v1=${path.basename(v1Path)} · model-slug=${slug ?? 'none'}`
  );
  for (const f of findings)
    console.error(`  ✗ ${f.severity} ${f.check}: ${f.message}`);
  if (findings.length) {
    console.error(
  ...
  console.log(
    '  ✓ all Writer self-reports (pre-draft presence, model recency) match disk.'
  );
  return 0;
}
...
function selftest(): number {
  ...
  t(
    'CHECK A FIRES on real 07-24 "take-draft: ABSENT" while file present-before-v1',
    aFire.length === 1 && aFire[0]!.message.includes('take-draft')
  );
  t(
    'CHECK A SILENT on the three IMPORTED components (only take fires)',
    aFire.every(f => f.message.includes('take-draft'))
  );
  ...
  t(
    'GUARD: silent on a wrapped "ABSENT (0/4): (none)" manifest line (08-07 false positive)',
    absenceFabrications(v1_0807_wrap, allOn).length === 0
  );
  t(
    'GUARD: silent on "ABSENT: (none)" phrasing',
    absenceFabrications('take-draft, signal-draft. ABSENT: (none).', allOn)
      .length === 0
  );
  t(
    'GUARD does NOT swallow a real absence assertion (07-24 still fires)',
    absenceFabrications(v1_0724, allOn).length === 1
  );
  t(
    'GUARD does NOT swallow a real absence with a nearby digit',
    absenceFabrications(
      'take-draft: ABSENT. Generated inline; 3 candidates tried.',
      allOn
    ).length === 1
  );

  // CHECK A — silent when the claimed-absent draft is genuinely absent.
  const takeOff: Record<Component, DraftState> = {
    ...allOn,
    'take-draft': off,
  };
  t(
    'CHECK A SILENT when take-draft truly absent',
    absenceFabrications(v1_0724, takeOff).length === 0
  );

  // CHECK A — silent on a legitimate SCHEDULING GAP: file present now but landed AFTER v1.
  const takeGap: Record<Component, DraftState> = {
    ...allOn,
    'take-draft': gap,
  };
  t(
    'CHECK A SILENT on scheduling gap (pre-draft mtime > v1 — Writer could not have seen it)',
    absenceFabrications(v1_0724, takeGap).length === 0
  );

  // CHECK A — silent on an all-IMPORTED ledger (a clean night).
  const cleanLedger =
    'take-draft: IMPORTED. signal-draft: IMPORTED. discovery-draft: IMPORTED. cc-predraft: IMPORTED.';
  t(
    'CHECK A SILENT on all-IMPORTED ledger',
    absenceFabrications(cleanLedger, allOn).length === 0
  );
  ...
  t(
    'extractModelSlug pulls the Explore-link slug',
    extractModelSlug(v1_model) === slug
  );
  const archive = [
    { date: '2026-06-25', body: `body /models/${slug} here` }, // 29 days before 07-24
    {
      date: '2026-05-28',
      body: 'body /models/data-interpretation-aesthetic-vs-functional-truth',
    },
  ];
  const rDirty = modelRecency(slug, archive, '2026-07-24');
  t(
    'CHECK B FIRES: slug used 06-25 (29d) is INSIDE 30d window (spans month boundary)',
    rDirty.violation && rDirty.lastUse === '2026-06-25' && rDirty.ageDays === 29
  );
  t(
    'CHECK B finding names the slug and the true last-use',
    modelRecencyFinding(slug, rDirty)?.message.includes('2026-06-25') ?? false
  );
  ...
  t(
    'CHECK B SILENT: replacement slug last used 05-28 (57d) is OUTSIDE the window',
    !rClean.violation && rClean.ageDays === 57
  );

  // CHECK B — silent when the model has never been used.
  const rNever = modelRecency('brand-new-model', archive, '2026-07-24');
  t(
    'CHECK B SILENT when model never used',
    !rNever.violation && rNever.lastUse === null
  );

  // CHECK B — must not count TODAY's own publication as a prior use.
  const archiveWithToday = [
    ...archive,
    { date: '2026-07-24', body: `/models/${cleanSlug}` },
  ];
  t(
    'CHECK B excludes today (strictly-before) so a just-published slug is not self-flagged',
    !modelRecency(cleanSlug, archiveWithToday, '2026-07-24').violation
  );
...
export function buildManifest(
  date: string,
  root = process.cwd()
): { body: string; present: Component[]; absent: Component[] } {
...
function stampOnDate(date: string, root = process.cwd()): number {
  ...
  console.log(
    `provenance-gate --stamp ${date} → ${path.basename(out)} (${body.length} bytes)`
  );
  console.log(
    `  PRESENT (${present.length}/4): ${present.join(', ') || '(none)'}`
  );
  console.log(
    `  ABSENT  (${absent.length}/4): ${absent.join(', ') || '(none)'}`
  );
  if (!present.length)
    console.error(
      '  ⚠ no pre-drafts on disk — the Writer will generate all four inline. Check the 5:30-5:45 PM tasks ran.'
    );
  return 0;
}
...
function main(): number {
  ...
  if (!date) {
    console.error(
      'usage: provenance-gate.ts <YYYY-MM-DD> [--stamp] | --selftest'
    );
    return 2;
  }
