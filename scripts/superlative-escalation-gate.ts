...
export interface Finding { check: 'superlative-escalation'; severity: '🔴'; phrase: string; sentence: string; }

// Unbounded HISTORICAL superlatives — the class our ~14-day archive cannot disprove.
const EXTREME = 'record|lowest|highest|fewest|most|largest|smallest|strongest|weakest|worst|best|biggest|greatest|deepest|steepest|longest';
const TEMPORAL_PATTERNS: RegExp[] = [
  new RegExp(`\\b(?:${EXTREME})\\b[^.]{0,40}?\\bsince\\s+(?:1[89]|20)\\d\\d\\b`, 'i'),
...
const CITATION = /\b(per|according to|BLS|BEA|the Fed|Federal Reserve|Treasury|Bloomberg|Reuters|WSJ|Financial Times|OECD|IMF|Census|data from|reported by|figures from|source:)\b|https?:\/\//i;
...
export function temporalSuperlatives(body: string): { phrase: string; sentence: string }[] {
  const out: { phrase: string; sentence: string }[] = [];
  for (const s of sentences(body)) {
    for (const re of TEMPORAL_PATTERNS) {
      const m = s.match(re);
      if (m) { out.push({ phrase: m[0].toLowerCase().replace(/\s+/g, ' ').trim(), sentence: s }); break; }
    }
  }
  return out;
}

export function hasCitation(sentence: string): boolean { return CITATION.test(sentence); }

function norm(p: string): string { return p.toLowerCase().replace(/\s+/g, ' ').trim(); }
 ...
 */
export function escalations(v15: string, v2: string, resolvedKeys: string[] = []): Finding[] {
  ...
  for (const { phrase, sentence } of temporalSuperlatives(v2)) {
    ...
    seen.add(key);
    out.push({ check: 'superlative-escalation', severity: '🔴', phrase, sentence: sentence.slice(0, 160) });
  }
  return out;
}
...
function loadResolvedKeys(root: string, date: string): string[] {
  ...
  try {
    ...
    return JSON.stringify(j).toLowerCase().match(/(?:lowest|highest|record|all-time|since)[a-z0-9 ]{0,30}/g) ?? [];
  } catch { return []; }
}
...
function runOnDate(date: string, root = process.cwd()): number {
  ...
  if (!fs.existsSync(v2Path)) { console.error(`superlative-escalation-gate: no v2 for ${date}`); return 2; }
  ...
  console.log(`superlative-escalation-gate ${date} — v1.5=${fs.existsSync(v15Path) ? 'present' : 'MISSING(empty baseline)'}`);
  for (const f of findings) console.error(`  ✗ ${f.severity} superlative-escalation: "${f.phrase}" introduced in v2, absent from v1.5, no citation. — "${f.sentence}"`);
  if (findings.length) {
  ...
  console.log('  ✓ no uncited historical superlative introduced between v1.5 and v2.');
  return 0;
}

function selftest(): number {
  let ok = 0, fail = 0;
  const t = (n: string, c: boolean) => { if (c) { ok++; console.log(`  ✓ ${n}`); } else { fail++; console.error(`  ✗ ${n}`); } };

  // The real 07-24 shape.
  const v15 = 'Weekly jobless claims at 187,000 were the lowest of 2026, and the labor market stayed tight.';
  const v2_bad = 'Weekly jobless claims at 187,000 were the lowest since 1969, and the labor market stayed tight.';
  const fire = escalations(v15, v2_bad);
  t('FIRES on real 07-24 "lowest since 1969" introduced over v1.5 "lowest of 2026"', fire.length === 1 && fire[0]!.phrase.includes('since 1969'));

  // Bounded superlative is not a temporal-historical superlative — never fires.
  t('SILENT when v2 keeps the bounded "lowest of 2026"', escalations(v15, v15).length === 0);
  t('bounded "lowest of 2026" is not extracted as temporal', temporalSuperlatives(v15).length === 0);
  t('"lowest since 1969" IS extracted as temporal', temporalSuperlatives(v2_bad).length === 1);

  // The Editor's legal move (a): cite a source → no fire.
  const v2_cited = 'Weekly jobless claims at 187,000 were the lowest since 1969, per BLS.';
  t('SILENT when the introduced superlative carries a citation (per BLS)', escalations(v15, v2_cited).length === 0);
  ...
  t('SILENT when the historical superlative was already in v1.5', escalations(v15_hist, v2_hist).length === 0);

  // "record low" class fires when introduced uncited.
  t('FIRES on introduced "record low" with no citation', escalations('Yields were elevated.', 'Yields hit a record low.').length === 1);

  // truth.json resolution suppresses.
  t('SILENT when resolved in truth.json', escalations(v15, v2_bad, ['lowest since 1969']).length === 0);

  // No double-count of the same phrase across three placements.
  const v2_thrice = 'Claims were the lowest since 1969. The lowest since 1969 print stunned desks. M&M: lowest since 1969.';
  t('de-dupes the same phrase across 3 placements → 1 finding', escalations(v15, v2_thrice).length === 1);
...
function main(): number {
  ...
  if (!date) { console.error('usage: superlative-escalation-gate.ts <YYYY-MM-DD> | --selftest'); return 2; }
