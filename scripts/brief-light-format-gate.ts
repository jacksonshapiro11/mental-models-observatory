...
const SELECTION_REQUIRED: { label: string; accepts: string[] }[] = [
  ...
  { label: 'Markets Minute', accepts: ['MARKETS MINUTE'] },
  {
    ...
    accepts: ['INTERESTING THINGS', 'TWO THINGS'],
  },
...
const IDEAS_REQUIRED: { label: string; accepts: string[] }[] = [
  ...
  { label: 'Markets Minute', accepts: ['MARKETS MINUTE'] },
  {
    ...
    accepts: ['TWO THINGS', 'INTERESTING THINGS'],
  },
  { label: 'The Meditation', accepts: ['THE MEDITATION'] },
  { label: 'The Model', accepts: ['THE MODEL'] },
  { label: 'The Close', accepts: ['THE CLOSE'] },
...
function main(): number {
  ...
  if (!file) {
    console.error(
      'usage: brief-light-format-gate.ts <brief-light.md> [--enforce-length]'
    );
    return 2;
  }
  ...
  if (!isSelection && !isIdeas) {
    fails.push(
      'No lead section: expected "## ▸ THE UPDATE" (selection) or "## ▸ THE IDEAS" (ideas-first).'
    );
  }
  const isTwoTier = isSelection && (isV2Era || isWeeklyV2Era);
  const mode = isTwoTier
    ? isWeeklyV2Era
      ? 'TWO-TIER WEEKLY'
      : 'TWO-TIER'
    : isSelection
      ? 'SELECTION'
      : 'IDEAS-FIRST';
  const required = isTwoTier
    ? isWeeklyV2Era
      ? WEEKLY_V2_REQUIRED
      : SELECTION_V2_REQUIRED
    : isSelection
      ? SELECTION_REQUIRED
      : IDEAS_REQUIRED;
  ...
  if (
    weeklyKey
      ? !/^#\s+WEEKLY LIGHT\s*$/m.test(md)
      : !/^#\s+BRIEF LIGHT\s*$/m.test(md)
  )
    warns.push(
      `Missing "# ${weeklyKey ? 'WEEKLY' : 'BRIEF'} LIGHT" title line.`
    );
  if (
    !lines.some(l =>
      (weeklyKey ? /^##\s+Week of\s+\S/ : /^##\s+[A-Z][a-z]+day,/).test(
        l.trim()
      )
    )
  )
    warns.push(
      `Missing "${weeklyKey ? '## Week of [Month D] to [D], [YYYY]' : '## [Weekday, Month D, YYYY]'}" date line.`
    );
  if (!lines.some(l => /^###\s+\S/.test(l.trim())))
    warns.push('Missing "### [Daily Title]" line.');
  ...
  const matched = (accepts: string[]) =>
    headers.find(h => accepts.some(a => h.upper.startsWith(a)));
  for (const req of required) {
    if (!matched(req.accepts))
      fails.push(
        `Missing required section: "## ▸ ${req.accepts[0]}" (${req.label}). Site + audio will drop it.`
      );
  }
  ...
  for (let i = 1; i < orderIdx.length; i++) {
    if (orderIdx[i]!.at < orderIdx[i - 1]!.at) {
      warns.push(
        `Section order: "${orderIdx[i]!.label}" appears before "${orderIdx[i - 1]!.label}" (expected ${required.map(r => r.label).join(' → ')}).`
      );
      break;
    }
  }
  ...
  const leadPrefixes = isSelection
    ? ['THE UPDATE']
    : ['THE IDEA', 'THE BIG IDEA'];
  const leadHeaders = headers.filter(h =>
    leadPrefixes.some(p => h.upper.startsWith(p))
  );
  let cards = 0;
  for (const h of leadHeaders) {
    const titled =
      h.raw.includes(':') &&
      h.raw.slice(h.raw.indexOf(':') + 1).trim().length > 0;
    const end = headers.find(x => x.idx > h.idx)?.idx ?? lines.length;
    const body = lines.slice(h.idx + 1, end);
    const boldHeadlines = body.filter(l =>
      /^\*\*[^*].*[^*]\*\*\s*$/.test(l.trim())
    ).length;
  ...
  if (isTwoTier) {
    ...
    if (cards < 4)
      fails.push(
        `THE UPDATE has only ${cards} story headlines; two-tier format needs 4-5 deep stories. Add bold "**headline**" lines.`
      );
    else if (cards > 5)
      warns.push(
        `THE UPDATE has ${cards} stories (two-tier spec calls for 4-5 deep; move the extras to THE LINE).`
      );
  } else if (isSelection) {
    if (cards < 4)
      fails.push(
        `THE UPDATE has only ${cards} story headlines; selection format needs 5-7 (min 4). Add bold "**headline**" lines.`
      );
    else if (cards < 5 || cards > 7)
      warns.push(`THE UPDATE has ${cards} stories (spec calls for 5-7).`);
  } else {
    if (cards === 0)
      fails.push(
        'No ideas parse: need bold "**headline**" lines under "## ▸ THE IDEAS", or "## ▸ THE IDEA: <title>" sections.'
      );
    else if (cards < 2)
      warns.push(`Only ${cards} idea card parses (spec calls for 2-3).`);
  }
  ...
  if (lineH) {
    ...
    if (items === 0)
      fails.push(
        'THE LINE parses to zero items: each item is "**Bold conclusion-first headline.** one sentence" — bold lead at line start.'
      );
    else if (isWeeklyV2Era ? items < 9 || items > 16 : items < 6 || items > 12)
      warns.push(
        isWeeklyV2Era
  ...
  if (modelH) {
    ...
    const hasName =
      (modelH.raw.includes(':') &&
        modelH.raw.slice(modelH.raw.indexOf(':') + 1).trim().length > 0) ||
      body.some(l => /^###\s+\S/.test(l.trim()));
    const hasLink = body.some(l => /\[[^\]]*\]\([^)]+\)/.test(l));
    if (!hasName)
      warns.push('The Model has no name (### line or "THE MODEL: <name>").');
    if (isIdeas && !body.some(l => /^\*\*\s*use it/i.test(l.trim())))
      warns.push(
        'The Model has no **Use it:** takeaway (the ideas-first deep keeper needs one).'
      );
  ...
  if (medH) {
    ...
    const body = lines
      .slice(medH.idx + 1, end)
      .map(l => l.trim())
      .filter(Boolean);
    if (!body.some(l => /^\*["“”].+["“”]\*/.test(l)))
      warns.push('The Meditation has no *"quote"* line.');
  }
  ...
  const inEpoch =
    (briefDate !== '' && briefDate >= LIGHT_LEN_EPOCH) || isWeeklyV2Era;
  const lenMark =
    words > LEN_HARD ? '🔴' : words > LEN_HI || words < LEN_LO ? '🟡' : '✅';
  console.log(
    `${lenMark} LIGHT LENGTH: ${words.toLocaleString()} words ≈ ${mins.toFixed(1)} min at ${LIGHT_WPM} wpm (target ${LEN_LO.toLocaleString()}-${LEN_HI.toLocaleString()} ≈ ${Math.round(LEN_LO / LIGHT_WPM)}-${Math.round(LEN_HI / LIGHT_WPM)} min, hard ceiling ${LEN_HARD.toLocaleString()})`
  );
  if (words > LEN_HARD && inEpoch) {
    if (lenOverride) {
      ...
      console.log(
        `  🔴 OVER HARD CEILING by ${(words - LEN_HARD).toLocaleString()} words (${mins.toFixed(1)} min vs ${Math.round(LEN_LO / LIGHT_WPM)}-${Math.round(LEN_HI / LIGHT_WPM)}). NOT BLOCKING — the brief always ships.`
      );
      console.log(
        `     The generating step owns this (\`brief-light-format-gate <file> --enforce-length\` inside its own`
      );
      console.log(
        `     compression loop). If you are seeing this at the publish path, the generator did not compress`
      );
      console.log(
        `     and did not declare — that is the thing to fix, not the brief.`
      );
    } else {
      fails.push(
    ...
    warns.push(
      `Length ${words.toLocaleString()} is over target ${LEN_HI.toLocaleString()} (advisory band up to ${LEN_HARD.toLocaleString()}). Shorten lines first; never cut coverage.`
    );
  } else if (words < LEN_LO && inEpoch) {
    warns.push(
      `Length ${words.toLocaleString()} is under target ${LEN_LO.toLocaleString()}. Never blocking — but check whether coverage was dropped (every full-brief story belongs in a tier).`
    );
  }
