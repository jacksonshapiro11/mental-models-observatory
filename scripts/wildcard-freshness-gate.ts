...
export type DateClaim = { text: string; month: number; dayLo: number; dayHi: number; exact: boolean };
...
export function parseDateClaim(s: string): DateClaim | null {
  const m = MONTHS.join('|');
  const exact = new RegExp(`\\b(?:on|dated)?\\s*(?:(\\d{1,2})\\s+(${m})|(${m})\\s+(\\d{1,2}))\\b`, 'i').exec(s);
  if (exact) {
    const day = parseInt((exact[1] || exact[4])!, 10);
    const mon = MONTHS.indexOf((exact[2] || exact[3])!.toLowerCase()) + 1;
    if (day >= 1 && day <= 31) return { text: exact[0].trim(), month: mon, dayLo: day, dayHi: day, exact: true };
  }
  const vague = new RegExp(`\\b(early|mid|late)[-\\s]+(${m})\\b`, 'i').exec(s);
  if (vague) {
    ...
    return { text: vague[0].trim(), month: mon, dayLo: lo, dayHi: hi, exact: false };
  }
  const bare = new RegExp(`\\bin\\s+(${m})\\b`, 'i').exec(s);
  if (bare) return { text: bare[0].trim(), month: MONTHS.indexOf(bare[1]!.toLowerCase()) + 1, dayLo: 1, dayHi: 31, exact: false };
  return null;
}
...
export type Finding = { kind: 'unledgered-journal-date' | 'syndicator-authority' | 'date-mismatch'; journal: string; detail: string };

/** Distinctive terms an item and its ledger row should share: Capitalised or *italicised* names. */
function distinctiveTerms(item: string): string[] {
  const out = new Set<string>();
  for (const m of item.matchAll(/\*([A-Z][A-Za-z-]{3,})\*/g)) out.add(m[1]!.toLowerCase());
  for (const m of item.matchAll(/\b([A-Z][a-z]{4,})\b/g)) out.add(m[1]!.toLowerCase());
  return [...out];
}

export function auditWildCard(md: string, rows: Row[]): Finding[] {
  const findings: Finding[] = [];
  for (const item of wildCardItems(md)) {
    const jm = JOURNAL_RE.exec(item);
    if (!jm) continue;                       // no journal named → not this class
    const journal = jm[1]!.trim();
    // Only the clause that names the journal asserts the publication date; a date elsewhere in the
    // item (a discovery anniversary, a launch) is a different fact and must not be attributed here.
    const clause = item.slice(jm.index, jm.index + 160);
    ...
    let cand = rows.filter(r => `${r.key} ${r.note ?? ''}`.toLowerCase().includes(jl));
    if (cand.length > 1) {
      const terms = distinctiveTerms(item);
      const narrowed = cand.filter(r => terms.some(t => `${r.key} ${r.note ?? ''}`.toLowerCase().includes(t)));
    ...
    const syn = SYNDICATORS.find(d => src.includes(d));
    if (syn) {
      findings.push({
    ...
    const [, mo, dy] = row.date.match(/^\d{4}-(\d{2})-(\d{2})$/)!;
    const rMonth = parseInt(mo!, 10), rDay = parseInt(dy!, 10);
    if (rMonth !== claim.month || rDay < claim.dayLo || rDay > claim.dayHi) {
      findings.push({
        kind: 'date-mismatch', journal,
        detail: `the brief says "${journal} … ${claim.text}"; the ledger's publisher record says ${row.date}. ${claim.exact ? 'Those are different days.' : `"${claim.text}" admits days ${claim.dayLo}-${claim.dayHi} of month ${claim.month}; ${row.date} is outside it.`} One of them is wrong, and the published one is the expensive one.`,
      });
    }
  }
  return findings;
...
function runOne(briefPath: string, date: string, truthOverride?: string, quiet = false): number {
  ...
  const findings = auditWildCard(fs.readFileSync(briefPath, 'utf8'), loadRows(truthPath));
...
function sweep(n: number): number {
  ...
  for (const f of files) {
    ...
    const fs2 = auditWildCard(fs.readFileSync(path.join(dir, f), 'utf8'), loadRows(path.join(dir, `${date}-truth.json`)));
...
function selftest(): number {
  ...
  // 4. The vague window is still a claim — the real 08-05 ants item, "late July" against July 8.
  ...
  // 5-6. BOTH DIRECTIONS. A correct date, publisher-sourced and ledgered, must be SILENT — otherwise
  ...
  // 7. Out of scope stays out of scope: no journal named, or a journal named with no date asserted.
  ...
  // 8. The anniversary trap: a SECOND date later in the item must not be read as the publication date.
  ...
  // 9. THE EPOCH IS REAL — the archive is measured, never condemned (the IMP-125 lesson).
  const dir = path.join(process.cwd(), 'daily-briefs');
  ...
  console.log(`\nwildcard-freshness-gate selftest — ${fails ? 'FAILED' : 'PASS'} (unledgered + syndicator + date-mismatch + vague-window verified both directions)`);
  return fails ? 1 : 0;
}
