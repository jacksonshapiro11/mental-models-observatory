...
const JOURNAL_RE = new RegExp(
  `\\b(${JOURNALS.map(j => j.replace(/ /g, '\\s+')).join('|')}|PLOS\\s+\\w+|Annals\\s+of\\s+[A-Z][\\w\\s]{2,40}?)\\b`
);
...
export function parseDateClaim(s: string): DateClaim | null {
  ...
  const exact = new RegExp(
    `\\b(?:on|dated)?\\s*(?:(\\d{1,2})\\s+(${m})|(${m})\\s+(\\d{1,2}))(?:,?\\s+(20\\d{2}))?\\b`,
    'i'
  ).exec(s);
  if (exact) {
  ...
  const vague = new RegExp(
    `\\b(early|mid|late)[-\\s]+(${m})(?:,?\\s+(20\\d{2}))?\\b`,
    'i'
  ).exec(s);
  if (vague) {
    ...
    const [lo, hi] =
      which === 'early' ? [1, 10] : which === 'mid' ? [11, 20] : [21, 31];
  ...
  const bare = new RegExp(`\\bin\\s+(${m})(?:,?\\s+(20\\d{2}))?\\b`, 'i').exec(
    s
  );
  if (bare) {
    ...
    return {
      text: bare[0].trim(),
      month: MONTHS.indexOf(bare[1]!.toLowerCase()) + 1,
      dayLo: 1,
      dayHi: 31,
      exact: false,
      ...(year ? { year } : {}),
...
export function wildCardItems(md: string): string[] {
  ...
  return body
    ...
    .split(/\n\s*\n/)
    .map(s => s.trim())
    .filter(s => s.length > 0 && !/^[-*_]{3,}$/.test(s));
}
...
export type Finding = {
  kind:
    | 'unledgered-journal-date'
    | 'source-authority'
    | 'syndicator-authority'
    | 'date-mismatch';
  journal: string;
  detail: string;
};
...
function isPublisherAuthority(host: string): boolean {
  return (
    PUBLISHER_HOSTS.some(d => host === d || host.endsWith(`.${d}`)) ||
    /\.edu(?:\.[a-z]{2})?$/.test(host) ||
    /\.ac\.[a-z]{2}$/.test(host)
  );
}
...
export function auditWildCard(
  ...
  for (const item of wildCardItems(md)) {
    const journalMatches = [
      ...item.matchAll(new RegExp(JOURNAL_RE.source, 'g')),
    ];
    for (const jm of journalMatches) {
      ...
      const clause = item.slice(
        sentenceStart,
        sentenceEnd === -1 ? item.length : sentenceEnd + 1
      );
      ...
      const expectedYear =
        claim.year ??
        (briefDate ? parseInt(briefDate.slice(0, 4), 10) : undefined);
      if (
        (expectedYear !== undefined && ry !== expectedYear) ||
        mo !== claim.month ||
        dy! < claim.dayLo ||
        dy! > claim.dayHi
      ) {
...
export function loadRows(truthPath: string): Row[] {
  ...
  try {
    ...
    return Object.entries(j.claims ?? {}).map(([key, v]) => ({
      key,
      ...(v as object),
    })) as Row[];
  } catch {
...
function resolveBrief(arg: string): { brief: string; date: string } {
  ...
  return {
    brief: p,
    date: (path.basename(p).match(/(\d{4}-\d{2}-\d{2})/) ?? ['', ''])[1]!,
  };
}
...
function runOne(
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    if (!quiet)
      console.error(
        `Cannot determine brief date from filename: ${briefPath}. Use a YYYY-MM-DD filename so epoch and year enforcement cannot fail open.`
      );
    return 2;
  }
  const truthPath =
  ...
  if (!quiet) {
    console.log(
      `wildcard-freshness-gate — ${path.basename(briefPath)} (${inEpoch ? 'IN EPOCH — findings BLOCK' : `pre-${EPOCH} — reported, never condemned`})`
    );
    for (const f of findings)
      console.log(
        `  ${inEpoch ? '🔴' : '🟡'} [${f.kind}] ${f.journal}: ${f.detail}`
      );
    if (!findings.length)
      console.log(
        '  ✅ every journal-dated Wild Card item is backed by a resolved, publisher-sourced record.'
      );
  }
...
function sweep(n: number): number {
  ...
  for (const f of files) {
    ...
    const fs2 = auditWildCard(
      fs.readFileSync(path.join(dir, f), 'utf8'),
      loadRows(path.join(dir, `${date}-truth.json`)),
      date
    );
    if (fs2.length) {
      dated++;
      unledgered += fs2.length;
      console.log(
        `  ${date}: ${fs2.map(x => `${x.kind}/${x.journal}`).join(', ')}`
      );
    }
  }
  console.log(
  ...
  console.log(
    'That is the size of the class nobody was checking. It is not three unlucky nights.'
  );
  return 0;
}
...
function selftest(): number {
  ...
  const ANTS =
    "**Evolution appears to have built parenting out of spare parts from the hunger system.** Researchers at Rockefeller University reported in Nature in late July that two ancient signalling molecules decide whether a clonal raider ant tends the colony's larvae or walks away to forage: neuropeptide F pushes toward care, allatostatin A pushes toward leaving, and the balance flips as the ant ages.";
  ...
  const PREPOSED =
    '**A publication date placed before the journal name still belongs to the publication claim.** On August 4, researchers reported in *Nature* that a result had survived replication across three independent laboratories, with enough explanatory detail to clear the item-length floor.';
  ...
  const publisherRow = (key: string, note: string, date: string): Row => ({
    ...
    date,
    resolved: true,
    source: 'https://www.nature.com/articles/s41586-026-10713-2',
  });
  ...
  t(
    a.length === 1 && a[0]!.kind === 'unledgered-journal-date',
  ...
  const b = auditWildCard(wrap(UROKODIA), [
    publisherRow(
      'journal:urokodia',
      'Nature, Urokodia chelicerae, Leicester newsroom',
      '2026-07-01'
    ),
  ]);
  t(
    b.length === 1 &&
      b[0]!.kind === 'date-mismatch' &&
      /2026-07-01/.test(b[0]!.detail),
  ...
  const c = auditWildCard(wrap(UROKODIA), [
    {
      key: 'journal:urokodia',
      note: 'Nature, Urokodia',
      date: '2026-08-04',
      resolved: true,
      source: 'https://www.sciencedaily.com/releases/2026/08/260804.htm',
    },
  ]);
  t(
    c.length === 1 &&
      c[0]!.kind === 'syndicator-authority' &&
      /sciencedaily/.test(c[0]!.detail),
  ...
  const c2 = auditWildCard(wrap(UROKODIA), [
    {
      ...
      date: '2026-08-04',
      resolved: true,
      source: 'https://example.com/random-blog',
    },
  ]);
  t(
    c2.length === 1 && c2[0]!.kind === 'source-authority',
    '[IMP-133] FIRES when a resolved row cites neither a publisher nor a university newsroom'
  );
  const unrelated = auditWildCard(
    wrap(UROKODIA),
    [publisherRow('market:unrelated', 'Nature, Urokodia', '2026-08-04')],
    '2026-08-06'
  );
  t(
    unrelated.length === 1 && unrelated[0]!.kind === 'unledgered-journal-date',
    '[IMP-133] FIRES when an otherwise matching row is not in the journal:* claim class'
  );
  const wrongPaper = auditWildCard(
    wrap(UROKODIA),
    [
      publisherRow(
        'journal:different-paper',
        'Nature, unrelated coral fluorescence study',
        '2026-08-04'
      ),
    ],
    '2026-08-06'
  ...
  t(
    wrongPaper.length === 1 &&
      wrongPaper[0]!.kind === 'unledgered-journal-date',
  ...
  const d = auditWildCard(wrap(ANTS), [
    publisherRow(
      'journal:raider-ants',
      'Nature, clonal raider ant neuropeptide F',
      '2026-07-08'
    ),
  ]);
  t(
    d.length === 1 &&
      d[0]!.kind === 'date-mismatch' &&
      /21-31/.test(d[0]!.detail),
  ...
  t(
    auditWildCard(wrap(SHROUD), [
      publisherRow(
        'journal:shroud',
        'Scientific Reports, Shroud of Turin metagenomics, Lancashire',
        '2026-08-04'
      ),
    ]).length === 0,
  ...
  t(
    auditWildCard(wrap(ANTS), [
      publisherRow(
        'journal:raider-ants',
        'Nature, clonal raider ant',
        '2026-07-24'
      ),
    ]).length === 0,
  ...
  t(
    pre.length === 1 && pre[0]!.kind === 'unledgered-journal-date',
    '[IMP-133] FIRES when the publication date precedes the journal name in the same sentence'
  );
  const wrongYear = auditWildCard(
    wrap(PREPOSED),
    [
      publisherRow(
        'journal:replication',
        'Nature replication laboratories',
        '2025-08-04'
      ),
    ],
    '2026-08-06'
  ...
  t(
    wrongYear.length === 1 && wrongYear[0]!.kind === 'date-mismatch',
    '[IMP-133] FIRES when month/day match but the publisher record is from the wrong year'
  );
  const explicitYear = parseDateClaim(
    'Researchers reported in Nature on August 4, 2025.'
  );
  t(
    explicitYear?.year === 2025,
    '[IMP-133] preserves an explicitly asserted publication year instead of discarding it'
  );
  t(
    auditWildCard(
      wrap('Researchers in Nature on August 4 found a new fossil species.'),
      []
    ).length === 1,
    '[IMP-133] FIRES on a short journal-dated paragraph instead of silently excluding it'
  );
  const twoJournals = auditWildCard(
    wrap(
  ...
  t(
    auditWildCard(
      wrap(
        '**A city rebuilt its tram network and ridership tripled.** Officials in Grenoble said the line opened on August 2 after four years of work, and the surprise was who rode it: not commuters but shift workers, whose hours the old bus timetable never fit.'
      ),
      []
    ).length === 0,
  ...
  t(
    auditWildCard(
      wrap(
        "**Researchers reported in *Nature* that a two-centimetre Cambrian animal carried the first draft of a spider's fangs, recovered by X-ray tomography from the Chengjiang beds of southern China.**"
      ),
      []
    ).length === 0,
  ...
  const e = auditWildCard(wrap(UROKODIA), [
    publisherRow('journal:urokodia', 'Nature, Urokodia', '2026-08-04'),
  ]);
  t(
    e.length === 0,
    '[IMP-133] SILENT when the ledger matches the JOURNAL clause — the anniversary sentence later in the item is not misread as the publication date'
  );
  ...
  if (fs.existsSync(dir)) {
    ...
    t(
      archive.every(
        f => runOne(path.join(dir, f), f.slice(0, 10), undefined, true) === 0
      ),
...
function main(): number {
  ...
  if (args.includes('--sweep'))
    return sweep(parseInt(args[args.indexOf('--sweep') + 1] || '30', 10) || 30);
  const target = args.find(a => !a.startsWith('--'));
  if (!target) {
    console.error(
      'Usage: wildcard-freshness-gate.ts <YYYY-MM-DD | path-to-brief.md> [--truth <path>] | --sweep [n] | --selftest'
    );
    return 2;
  }
...
const invokedDirectly =
  !!process.argv[1] &&
  path.resolve(process.argv[1]).endsWith('wildcard-freshness-gate.ts');
