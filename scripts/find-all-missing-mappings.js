  ...
  for (const model of problematicModels) {
    try {
      ...
      const highlightCount = data.curatedHighlights
        ...
        : 0;
      console.log(
        `  ${model}: ${highlightCount} highlights ${highlightCount > 0 ? '✅' : '❌'}`
      );

      if (highlightCount === 0) {
        // Try to find similar Readwise IDs
        const keywords = model.split('-');
        const candidates = readwiseModelIds.filter(
          id =>
        ...
        if (candidates.length > 0) {
          console.log(
            `    Potential matches: ${candidates.slice(0, 3).join(', ')}`
          );
          ...
          try {
            ...
            const testCount = testData.curatedHighlights
              ? testData.curatedHighlights.length
              : 0;
            if (testCount > 0) {
              console.log(
                `    ✅ FOUND: ${candidates[0]} has ${testCount} highlights`
              );
            }
          } catch (e) {
            // Ignore test errors
