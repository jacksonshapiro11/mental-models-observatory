...
function parseMarkdown(markdownPath) {
  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    tweetBlocks.forEach((block, blockIdx) => {
      ...
      if (invalid.length > 0) {
        console.error(
          `❌ ${modelName} - Thread ${blockIdx + 1}: ${invalid.length} tweet(s) over 280 chars`
        );
...
function main() {
  ...
  threads.forEach(thread => {
    thread.tweets.forEach((tweet, idx) => {
      ...
      console.log(
        `   ${thread.modelName} - Tweet ${idx + 1}: ${len} chars ${status}`
      );
    });
  });
  console.log('');
  ...
  if (fs.existsSync(queueFile)) {
    ...
    if (existing.length > 0) {
      ...
      const lastDate = new Date(
        existing[existing.length - 1].scheduledDateTime
      );
      startDate = new Date(lastDate);
      startDate.setDate(startDate.getDate() + 1);
      startDate.setHours(0, 0, 0, 0);
      console.log(
        `📅 Appending after existing queue (starts ${startDate.toISOString().split('T')[0]})\n`
      );
    }
  } else {
    console.log(
      `📅 Starting today (${startDate.toISOString().split('T')[0]})\n`
    );
  }
  ...
  scheduled.slice(0, 5).forEach(item => {
    console.log(
      `   ${item.scheduledDate} ${item.scheduledTime} EST - ${item.modelName}`
    );
  });
