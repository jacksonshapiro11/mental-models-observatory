  ...
  for (let day = 0; day < 7; day++) {
    ...
    for (let timeSlot = 0; timeSlot < 3; timeSlot++) {
      ...
      const threadPath = path.join(
        outputDir,
        originalItem.date,
        `${originalItem.slug}-thread-${originalItem.thread}.json`
      );
      ...
      console.log(
        `✅ Scheduled ${originalItem.model} for ${dayName} ${dateStr} at ${estTime} EST (${utcTime} UTC)`
      );
