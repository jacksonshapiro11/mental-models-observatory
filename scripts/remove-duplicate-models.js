...
console.log(
  '📝 Removing duplicate models (13C, 14C, 15D, 18E, 18F, 19D, 20E, 20F, 20G)...\n'
);
...
for (const code of duplicateCodes) {
  ...
  if (matches.length === 0) {
    console.log(
      `   ⚠️  Model ${code} not found (might have been removed already)`
    );
    continue;
  }
  ...
  while (i < dataContent.length) {
    ...
    if (!inString) {
      ...
      if (braceCount === 0) {
        ...
        dataContent =
          dataContent.substring(0, commaBeforeModel) +
