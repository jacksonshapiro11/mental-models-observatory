...
export function loadCadence(
  root = process.cwd()
): Record<string, CadenceEntry> {
  const p = path.join(root, CADENCE_FILE);
  if (!fs.existsSync(p)) return {};
  return (JSON.parse(fs.readFileSync(p, 'utf-8')).tasks ?? {}) as Record<
    string,
    CadenceEntry
  >;
}
...
export function cadenceFor(
  task: string,
  roster: Record<string, CadenceEntry>
): CadenceEntry {
  return (
    roster[task] ??
    roster['_default'] ?? { cadence: 'daily', source: 'assumed' }
  );
}
...
export function isDue(
  ...
  if (e.cadence === 'weekly')
    return e.dow === undefined ? false : dowOf(iso) === e.dow;
  return true;
}
 ...
 */
export function starveBudgetDays(
  task: string,
  iso: string,
  roster: Record<string, CadenceEntry>,
  dailyBudget = 2
...
function main(): void {
  ...
  if (argv.includes('--selftest')) {
    ...
    t(
      '[roster] EVERY entry carries a falsifier — an entry with no falsifier is a belief, not a record',
      Object.entries(R)
        .filter(([k]) => !k.startsWith('_'))
        .every(([, v]) => !!(v as CadenceEntry).falsifier)
    );
    ...
    t(
      '[roster] every entry declares its epistemic status — measured, declared, or contested — so a reader can tell what was observed from what was asserted',
      Object.entries(R)
        .filter(([k]) => !k.startsWith('_'))
        .every(
          ([, v]) =>
            ['measured', 'declared', 'contested'].includes(
              String((v as CadenceEntry).source).split(' ')[0] ?? ''
    ...
    t(
      '[W3] a WEEKLY/Saturday component is not due Thu or Fri, and IS due Saturday',
      !isDue('weekly-sat', '2026-08-27', FX) &&
        !isDue('weekly-sat', '2026-08-28', FX) &&
        isDue('weekly-sat', '2026-08-29', FX)
    );
    t(
    ...
    t(
      '[W3] a DAILY component is due every day, including the days the weekly one is not',
      isDue('daily-thing', '2026-08-27', FX) &&
        isDue('daily-thing', '2026-08-29', FX)
    );
    t(
      '[W3] a Sunday-weekly is due 08-30 and not 08-29',
      isDue('weekly-sun', '2026-08-30', FX) &&
        !isDue('weekly-sun', '2026-08-29', FX)
    );
    t(
      '[W3] a PAUSED component is never due, and never alarms',
      !isDue('paused-thing', '2026-08-29', FX)
    );
    t(
      '[W3] a daily component keeps the 2-day Critical budget (IMP-223 unchanged)',
      starveBudgetDays('daily-thing', '2026-08-27', FX) === 2
    );
    t(
      '[W3] a weekly component gets next-run+1 — Thursday to Saturday is 2 days, so budget 3',
      starveBudgetDays('weekly-sat', '2026-08-27', FX) === 3
    );
    t(
      '[W3] and on its own run-day the budget is a full week + 1, not zero',
      starveBudgetDays('weekly-sat', '2026-08-29', FX) === 8
    );
    t(
      '[W3] a paused component can never starve a row',
      starveBudgetDays('paused-thing', '2026-08-29', FX) ===
        Number.POSITIVE_INFINITY
    );
    t(
      '[W3] an UNKNOWN task falls back to the default and is treated as daily',
      starveBudgetDays('nobody', '2026-08-27', FX) === 2 &&
        isDue('nobody', '2026-08-27', FX)
    );
    ...
    const CONTESTED_FX: Record<string, CadenceEntry> = {
      'fx-contested': {
        ...
        dow: 6,
        source: 'contested',
        falsifier: 'resolves 2026-08-29',
      } as CadenceEntry,
      'fx-owner': {
        cadence: 'weekly',
        dow: 6,
        source: 'owner-observed 2026-08-28',
        falsifier: 'no line by Saturday + grace',
      } as CadenceEntry,
    };
    t(
      '[R6] a CONTESTED entry keeps its alarm ON — it reads daily while the question is open, because a cadence claim that silences an alarm must never be merely declared',
      cadenceFor('fx-contested', CONTESTED_FX).cadence === 'daily' &&
        isDue('fx-contested', '2026-08-27', CONTESTED_FX)
    );
    t(
      '[R6] an OWNER-OBSERVED entry outranks a board inference and may go weekly — the owner can read the scheduler; no session can',
      cadenceFor('fx-owner', CONTESTED_FX).cadence === 'weekly' &&
        !isDue('fx-owner', '2026-08-27', CONTESTED_FX)
    );
    t(
      '[R6] and a weekly component still ALARMS on its own missed day — resolution narrows the window, it never removes the check',
  ...
  for (const [k, v] of Object.entries(R)) {
    ...
    console.log(
      `  ${k.padEnd(30)} ${v.cadence.padEnd(7)} ${v.source === 'declared' ? '⚠️ DECLARED' : 'measured  '}  due=${isDue(k, date, R)}  starve-budget=${starveBudgetDays(k, date, R)}d`
    );
  }
  process.exit(0);
}
