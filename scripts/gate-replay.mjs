...
const GATES = [
  ...
  g => (!ONLY || g.name === ONLY) && fs.existsSync(`scripts/${g.name}.ts`)
);

const dates = fs
  .readdirSync('content/daily-updates')
...
for (const g of GATES) {
  ...
  for (const d of dates) {
    ...
    const { out, code } = run(
      `node --experimental-strip-types scripts/${g.name}.ts ${target} ${extra}`
    );
    if (code === 2) {
      results[g.name][d] = '–';
      continue;
...
for (const g of GATES)
  console.log(
    g.name.padEnd(w) +
      dates.map(d => (results[g.name][d] || '?').padStart(7)).join('')
  );
console.log(`
