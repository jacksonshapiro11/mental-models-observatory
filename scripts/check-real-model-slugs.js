...
async function checkRealModelSlugs() {
    ...
    for (const model of domainModels) {
      try {
        ...
        if (checkedCount % 10 === 0) {
          console.log(
            `    ... checked ${checkedCount}/${models.length} models`
          );
  ...
  console.log('=== SUMMARY ===');
  console.log(
    `✅ Working models: ${workingModels.length}/${models.length} (${Math.round((workingModels.length / models.length) * 100)}%)`
  );
  console.log(
    `❌ Broken models: ${brokenModels.length}/${models.length} (${Math.round((brokenModels.length / models.length) * 100)}%)`
  );
  ...
  for (const [domain, domainModels] of Object.entries(brokenByDomain)) {
    ...
    for (const model of domainModels.slice(0, 5)) {
