...
function main() {
  ...
  if (!ledgerPath) {
    console.error('predictions-ledger.json not found');
    process.exit(2);
  }
  ...
  const dueSoon = open.filter(
    p => p.due && p.due >= today && p.due <= addDays(today, 14)
  );
  ...
  console.log(
    `ledger: ${path.basename(ledgerPath)}  ·  total calls: ${preds.length}`
  );
  console.log(
    `resolved: ${resolved.length}  (✓ ${right} right · ✗ ${wrong} wrong · ◐ ${partial} partial)`
  );
  console.log(`running accuracy: ${acc}  (partial = half credit)`);
  console.log(
    `open: ${open.length}  ·  overdue (ungraded past due): ${overdue.length}  ·  due ≤14d: ${dueSoon.length}`
  );
  ...
  for (const c of ['high', 'medium', 'low'] as const) {
    ...
    console.log(
      `  ${c.padEnd(6)} ${r.length ? `${rr}/${r.length} right (${pct(rr, r.length)})` : '— none resolved'}`
    );
  }
  ...
  if (overdue.length) {
    ...
    for (const p of overdue)
      console.log(
        `   • [${p.due}] ${p.claim.slice(0, 90)}  (made ${p.date_made}, ${p.confidence})`
      );
  }
  if (dueSoon.length) {
    console.log(`\n→ due within 14 days:`);
    for (const p of dueSoon)
      console.log(`   • [${p.due}] ${p.claim.slice(0, 90)}`);
  }
