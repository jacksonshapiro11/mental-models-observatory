...
export interface EveningTruth { flags: string[]; unresolved: string[]; pass: boolean }
...
export function renderReport(date: string, t: EveningTruth): string {
  ...
  if (!t.flags.length && !t.unresolved.length) {
    lines.push('No findings. If this holds 30 consecutive nights, E-TRUTH-BYPASS-EVENING-01 can close on evidence.');
  } else {
    if (t.flags.length) lines.push('## Flags for the morning gate', '', ...t.flags.map(f => `- ${f}`), '');
    if (t.unresolved.length) lines.push('## Unresolved claims', '', ...t.unresolved.map(u => `- ${u}`), '');
  }
  return lines.join('\n') + '\n';
}
...
function selftest(): number {
  ...
  t('parses the real 08-01 flag', p.flags.length === 1 && p.flags[0]!.includes('take-extraordinary-claim'));
  t('reads PASS correctly', p.pass === true);
  t('parses UNRESOLVED-FACT lines', parseFactGate('UNRESOLVED-FACT: GM +22%').unresolved.length === 1);
  t('clean input yields no findings', (() => { const c = parseFactGate('✅ FACT-GATE PASS'); return !c.flags.length && !c.unresolved.length && c.pass; })());
  t('report names the 30-night close condition when clean', renderReport('2026-08-01', parseFactGate('✅ FACT-GATE PASS')).includes('30 consecutive nights'));
  t('report lists findings when present', renderReport('2026-08-01', p).includes('take-extraordinary-claim'));
...
function main(): number {
  ...
  if (!date) { console.error('usage: evening-truth-gate.ts <YYYY-MM-DD> [--strict] | --selftest'); return 2; }
  const strict = args.includes('--strict');
  const v2 = path.join(process.cwd(), `daily-briefs/${date}-v2.md`);
  if (!fs.existsSync(v2)) { console.error(`evening-truth-gate: no v2 for ${date}`); return 2; }

  let out = '';
  try { out = execSync(`node --experimental-strip-types scripts/fact-gate.ts ${v2}`, { encoding: 'utf8', stdio: ['ignore','pipe','pipe'] }); }
  catch (e: any) { out = (e.stdout || '') + (e.stderr || ''); }

  const t = parseFactGate(out);
  const report = path.join(process.cwd(), `daily-briefs/${date}-evening-truth.md`);
  fs.writeFileSync(report, renderReport(date, t), 'utf8');

  console.log(`evening-truth-gate ${date} — ${t.flags.length} flag(s), ${t.unresolved.length} unresolved → ${path.basename(report)}`);
  for (const f of [...t.flags, ...t.unresolved]) console.error(`  🟡 ${f.slice(0, 160)}`);
  if (!t.flags.length && !t.unresolved.length) console.log('  ✓ evening chain is truth-clean.');
  else console.log('  🟡 WARN-ONLY — does not block. The morning gate inherits this worklist.');
