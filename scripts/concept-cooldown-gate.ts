...
function run(file: string, window: number, warnOnly: boolean): number {
  ...
  if (here.length === 0) {
    console.log(
      '✅ concept-cooldown-gate: no watched concept used as a Model/Take frame.'
    );
    return 0;
  }
  ...
  for (const term of here) {
    ...
    for (const cf of corpus) {
      try {
        if (
          watchHits(framingRegion(fs.readFileSync(cf, 'utf-8'))).includes(term)
        ) {
  ...
  console.log(
    `${warnOnly ? '⚠️ ' : '❌'} CONCEPT COOLDOWN ${warnOnly ? 'FLAG' : 'VIOLATION'}(S):`
  );
...
function selftest(): number {
  ...
  t(
    'framingRegion pulls Model + Take',
    /Tensegrity/.test(framingRegion(dailyModel)) &&
      /Dixit/.test(framingRegion(dailyModel))
  );
  t(
    'watchHits finds hysteresis in Take frame',
    watchHits(framingRegion(dailyModel)).includes('hysteresis')
  );
  const clean =
    '## 🧠 The Model\n### Tensegrity\nBalanced tension, no watched concept.\n## Discovery\nx';
  t(
    'clean brief has no watch hits',
    watchHits(framingRegion(clean)).length === 0
  );
  const weekly =
    '# ▸ THE MODEL\n### Return-Point Memory\nDisordered solids recall their largest excursion.\n# ▸ DISCOVERY\nx';
  t(
    'weekly headers detected; return-point memory caught',
    watchHits(framingRegion(weekly)).includes('return-point memory')
  );
  t(
    'discovery-only mention NOT counted as frame',
    watchHits(framingRegion('## Discovery\nA note on hysteresis in glasses.\n'))
      .length === 0
  );
...
if (!file) {
  console.error(
    'usage: concept-cooldown-gate.ts <file.md> [--window N] [--warn] | --selftest'
  );
  process.exit(2);
}
