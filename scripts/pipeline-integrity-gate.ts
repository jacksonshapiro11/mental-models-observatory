...
export function provenanceWarns(root: string, date: string): string[] {
  ...
  if (!exists(root, `${briefs}/${date}-predraft-manifest.md`)) {
    out.push(
      `PREDRAFT-STAMP-ABSENT: no ${date}-predraft-manifest.md — IMP-102's provenance-gate --stamp did not run before the Writer, so the ground-truth pre-draft manifest was never written and the input-layer fabrication guard is unexercised. The stamp is prose-wired in Pipeline_Controller, not the FDA brief-draft task body (ESC-006 → Jackson).`
    );
  }

  const v1 = readV1(root, date);
  if (v1) {
    const fabricated = WRITER_INPUT_DRAFTS.filter(
      c =>
...
export function selfReportContradiction(
  statusText: string,
  gateExit: number
): string | null {
...
export function gateSelfReportWarns(root: string, date: string): string[] {
  ...
  if (
    !statusText
      .split('\n')
      .some(l => /\bbrief-editor\b/.test(l) && SELFREPORT_PASS_RE.test(l))
  )
    return [];
  const r = spawnSync(
    process.execPath,
    ['--experimental-strip-types', gatePath, v2Path],
...
export function diagnose(root: string, date: string): Diagnosis {
  const briefs = 'daily-briefs';
  const componentsPresent = COMPONENT_DRAFTS.filter(c =>
    exists(root, `${briefs}/${date}-${c}.md`)
  );
  const hasV1 =
    exists(root, `${briefs}/${date}-v1.md`) ||
    exists(root, `${briefs}/${date}-v1-pre-quality-gate.md`);
  ...
  if (hasV2) {
    ...
    if (!hasCritic)
      warnings.push(
        `CRITIC-ABSENT: ${date}-v2.md exists but ${date}-critic.md does not — the feedback loop for this brief is broken (the improvement cycle inherits no adversarial read).`
      );
    ...
    return {
      ...
      fail: false,
      warnings,
      detail: `v2 present${hasCritic ? ' + critic' : ' (critic MISSING)'}.`,
    };
  }
...
function selftest(): number {
  ...
  const mk = (files: string[]): string => {
    ...
    for (const f of files)
      fs.writeFileSync(path.join(r, 'daily-briefs', f), 'x\n');
    return r;
  };
  const D = '2026-07-27';
  const roots: string[] = [];
  try {
    ...
    let r = mk([
      `${D}-cc-predraft.md`,
      `${D}-take-draft.md`,
      `${D}-predraft-manifest.md`,
      `${D}-v1-pre-quality-gate.md`,
      `${D}-v1.5.md`,
      `${D}-v2.md`,
      `${D}-critic.md`,
    ]);
    roots.push(r);
    results.push([
      'HEALTHY when v2 + critic + manifest present (silent, exit 0)',
      (() => {
        const d = diagnose(r, D);
        return d.state === 'HEALTHY' && !d.fail && d.warnings.length === 0;
      })(),
    ]);

    // HEALTHY but CRITIC-ABSENT warn: v2 present, critic missing (the 07-27 cascade tail).
    r = mk([`${D}-cc-predraft.md`, `${D}-take-draft.md`, `${D}-v2.md`]);
    roots.push(r);
    results.push([
      'HEALTHY + CRITIC-ABSENT warn when v2 but no critic (exit 0, warned)',
      (() => {
        const d = diagnose(r, D);
        return (
          d.state === 'HEALTHY' &&
          !d.fail &&
          d.warnings.some(w => w.startsWith('CRITIC-ABSENT'))
        );
      })(),
    ]);
    ...
    roots.push(r);
    results.push([
      'FIRES SILENT_V1_FAILURE on the 07-27 signature (inputs ready, no brief, no status) — exit 1',
      (() => {
        const d = diagnose(r, D);
        return (
          d.state === 'SILENT_V1_FAILURE' &&
          d.fail &&
          /SILENT failure/.test(d.detail)
        );
      })(),
    ]);
    ...
    roots.push(r);
    fs.writeFileSync(
      path.join(r, 'daily-briefs', `${D}-pipeline-status.md`),
      `2026-07-27T00:53Z | brief-draft | FAIL | writer produced no v1\n`
    );
    results.push([
      'SILENT_V1_FAILURE still FAILs but reports LOUD when a brief-draft status line exists',
      (() => {
        const d = diagnose(r, D);
        return (
          d.state === 'SILENT_V1_FAILURE' &&
          d.fail &&
          /LOUD failure/.test(d.detail)
        );
      })(),
    ]);
    ...
    roots.push(r);
    results.push([
      'FIRES PARTIAL when v1/v1.5 present but no v2 (editor HALT) — exit 1',
      (() => {
        const d = diagnose(r, D);
        return d.state === 'PARTIAL' && d.fail;
      })(),
    ]);
    ...
    roots.push(r);
    results.push([
      'SILENT on NOT_STARTED (no components, no brief) — exit 0, no false positive',
      (() => {
        const d = diagnose(r, D);
        return d.state === 'NOT_STARTED' && !d.fail;
      })(),
    ]);

    // Fewer than 2 components (upstream not ready) is NOT a Writer failure — NOT_STARTED.
    r = mk([`${D}-signal-draft.md`]);
    roots.push(r);
    results.push([
      '1 component only => NOT_STARTED, not SILENT (inputs not yet ready)',
      (() => {
        const d = diagnose(r, D);
        return d.state === 'NOT_STARTED' && !d.fail;
      })(),
    ...
    roots.push(r);
    results.push([
      'HEALTHY brief with full artifacts emits ZERO warnings (no wolf-crying)',
      (() => {
        const d = diagnose(r, D);
        return d.state === 'HEALTHY' && d.warnings.length === 0;
      })(),
    ]);
    ...
    roots.push(r);
    results.push([
      'IMP-106: FIRES PREDRAFT-STAMP-ABSENT when a produced brief has no manifest (warn, exit 0)',
      (() => {
        const d = diagnose(r, D);
        return (
          d.state === 'HEALTHY' &&
          !d.fail &&
          d.warnings.some(w => w.startsWith('PREDRAFT-STAMP-ABSENT'))
        );
      })(),
    ]);
    ...
    roots.push(r);
    results.push([
      'IMP-106: PREDRAFT-STAMP-ABSENT is SILENT once the manifest exists (self-clears)',
      (() => {
        const d = diagnose(r, D);
        return !d.warnings.some(w => w.startsWith('PREDRAFT-STAMP-ABSENT'));
      })(),
    ]);
    ...
    roots.push(r);
    fs.writeFileSync(
      path.join(r, 'daily-briefs', `${D}-v1-pre-quality-gate.md`),
      `| take-draft | ABSENT  --  confirmed via bash ls ${D}; generated inline |\n`
    );
    results.push([
      'IMP-106: FIRES PREDRAFT-FABRICATION when v1 says take-draft ABSENT but the file exists',
      (() => {
        const d = diagnose(r, D);
        return d.warnings.some(
          w => w.startsWith('PREDRAFT-FABRICATION') && /take-draft/.test(w)
        );
      })(),
    ]);
    ...
    fs.writeFileSync(
      path.join(r, 'daily-briefs', `${D}-v1-pre-quality-gate.md`),
      `| discovery-draft | ABSENT  --  no discovery-draft on disk, honest |\n`
    );
    results.push([
      'IMP-106: SILENT on an HONEST absent (v1 says absent AND the pre-draft truly is absent)',
      (() => {
        const d = diagnose(r, D);
        return !d.warnings.some(w => w.startsWith('PREDRAFT-FABRICATION'));
      })(),
    ]);
    ...
    results.push([
      'IMP-119: FIRES when the editor line claims "fact-gate PASS" and the re-run exits 1 (the real 08-02 line)',
      (() => {
        const w = selfReportContradiction(REAL_LINE, 1);
        return (
          !!w &&
          w.startsWith('GATE-SELFREPORT-CONTRADICTED') &&
          /fact-gate PASS/.test(w)
        );
      })(),
    ]);
    results.push([
      'IMP-119: SILENT when the claim is true (line says PASS, re-run exits 0) — self-clears',
      selfReportContradiction(REAL_LINE, 0) === null,
    ]);
    results.push([
      'IMP-119: SILENT when the editor line makes no fact-gate claim (nothing to contradict)',
      selfReportContradiction(
        '2026-08-01T23:27:45Z | brief-editor | v2 | SUCCESS | promoted.',
        1
      ) === null,
    ]);
    results.push([
      'IMP-119: SILENT on a non-editor line that mentions fact-gate PASS (scoping)',
      selfReportContradiction(
        '2026-08-01T10:00Z | brief-morning | fact-gate PASS after truth resolve',
        1
      ) === null,
    ]);
    // Disk wrapper is inert when the artifacts are absent — no crash, no false warn.
    r = mk([`${D}-cc-predraft.md`]);
    roots.push(r);
    results.push([
      'IMP-119: disk wrapper is INERT when v2/status are absent (no crash, no warn)',
      gateSelfReportWarns(r, D).length === 0,
    ]);
  } finally {
  ...
  if (ok) {
    console.log(
      '\n✅ SELFTEST PASS — a silent Writer failure now has a deterministic, actionable signature.'
    );
    return 0;
  }
  console.error('\n❌ SELFTEST FAIL');
...
function main(): number {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    console.error(`FAIL: bad --date "${date}"`);
    return 2;
  }
