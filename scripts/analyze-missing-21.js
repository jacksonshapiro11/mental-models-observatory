...
async function analyzeWhy21NotWorking() {
  ...
  for (const modelSlug of userListedModels) {
    ...
    console.log(
      `${isParsed ? '✅' : '❌'} ${modelSlug}: ${isParsed ? 'Parsed' : 'Not parsed'}`
    );
  }
  ...
  console.log('\n🔍 ANALYSIS:');
  console.log(
    `Static framework models (user listed): ${userListedModels.length}`
  );
  console.log(`Readwise curated models: ${allReadwiseModels.length}`);

  // Check overlap
  const overlap = userListedModels.filter(model =>
    allReadwiseModels.includes(model)
  );
  ...
  if (overlap.length === 0) {
    console.log('\n❌ PROBLEM IDENTIFIED:');
    console.log(
      'The 21 models from the static framework have NO Readwise highlights'
    );
    console.log(
      'because the Readwise files contain completely different model IDs.'
    );
    ...
    console.log('Either:');
    console.log(
      '1. Create Readwise highlights for the static framework model IDs'
    );
    console.log(
      '2. Map the static framework IDs to existing Readwise model IDs'
    );
