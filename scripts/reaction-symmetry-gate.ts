...
const NOT_A_COMPANY = new Set([
  'The', 'This', 'That', 'These', 'Those', 'Two', 'Three', 'Four', 'One', 'Both', 'It', 'Its',
  'But', 'And', 'A', 'An', 'In', 'On', 'At', 'For', 'From', 'With', 'By', 'As', 'If', 'When',
  'What', 'Which', 'Who', 'Whether', 'Every', 'Each', 'Their', 'They', 'He', 'She', 'We', 'You',
  'US', 'U.S.', 'American', 'America', 'Wall', 'Street', 'Q1', 'Q2', 'Q3', 'Q4', 'Adjusted',
  'Revenue', 'Earnings', 'Cloud', 'Data', 'Center', 'Free', 'Cash', 'Flow', 'Watch', 'Nothing',
  'Supply', 'Whether', 'Same', 'Meanwhile', 'Analysts', 'Consensus', 'Guidance', 'Management',
]);
 ...
 */
const ABSTRACT_NOUN_SUFFIX = /(?:ation|isation|ization|ment|ness|ity|ism|ology|ance|ence|ship|hood|dom|sion)$/i;
...
const METRIC_NOUN = /\b(?:comparable sales|comps?|same-store|revenue|sales|margin|earnings|EPS|growth|traffic|orders?|volume|bookings|billings|ARR|backlog|output|production|yield|share of|deposits|assets)\b/i;
const SHARE_WORD = /\b(?:shares?|stock|the tape|after hours|after-hours|in after-hours trading)\b/i;

interface Move { index: number; isReaction: boolean; }

/** Every percentage move in the bullet, classified METRIC vs PRICE REACTION. */
export function extractMoves(bullet: string): Move[] {
  const out: Move[] = [];
  const re = new RegExp(String.raw`\b(?:${MOVE_VERB})\s+(?:about\s+|roughly\s+|more than\s+|nearly\s+)?\d+(?:\.\d+)?\s*(?:percent|%)`, 'gi');
  let m: RegExpExecArray | null;
  while ((m = re.exec(bullet)) !== null) {
    ...
    const boundary = Math.max(
      ...
      before.lastIndexOf(' and '), before.lastIndexOf(' then '), before.lastIndexOf(' while '), -1,
    );
...
// ---------------------------------------------------------------------------
function sectionBullets(body: string, heading: RegExp, pick: (b: string) => boolean): string | null {
...
function selftest(): number {
  ...
  for (const p of [v15Path, v2Path]) {
    if (!fs.existsSync(p)) { console.error(`SELFTEST FAIL — missing fixture: ${p}`); return 1; }
  }
  ...
  const ait2Bad = sectionBullets(v15, /AI\s*&\s*Tech/i, (b) => /Cloudflare/.test(b) && /Atlassian/.test(b));
  t(!!ait2Bad, '[fixture] the pre-repair AI&T-2 Cloudflare/Atlassian bullet was located in 2026-08-07-v1.5.md');
  const ait2Findings = ait2Bad ? reactionSymmetry(ait2Bad) : [];
  t(ait2Findings.length === 1 && ait2Findings[0]!.withoutReaction.some((n) => /Atlassian/.test(n)),
  ...
  const ait2Fixed = sectionBullets(v2, /AI\s*&\s*Tech/i, (b) => /Cloudflare/.test(b) && /Atlassian/.test(b));
  t(ait2Fixed ? reactionSymmetry(ait2Fixed).length === 0 : false,
  ...
  const cc2 = sectionBullets(v2, /Companies\s*&\s*Crypto/i, (b) => /Burger King/.test(b) && /Popeyes/.test(b));
  t(!!cc2, '[fixture] the real C&C-2 Burger King/Popeyes/Papa John\'s bullet was located');
  t(cc2 ? reactionSymmetry(cc2).length === 0 : false,
    'SILENT on the real C&C-2 — three reporters, zero PRICE reactions (the percentages are comps)');
  t(cc2 ? extractReporters(cc2).length >= 2 : false,
  ...
  const cc1 = sectionBullets(v2, /Companies\s*&\s*Crypto/i, (b) => /SpaceX/.test(b) && /911/.test(b));
  t(cc1 ? reactionSymmetry(cc1).length === 0 : true,
  ...
  t(reactionSymmetry(bothPrinted).length === 0, 'SILENT when BOTH reporters get a reaction (symmetry restored = the fix)');
  ...
  t(reactionSymmetry(onePrinted).length === 1, 'FIRES on the minimal pair that differs ONLY by the withheld reaction');

  const total = 9;
  console.log(`\nreaction-symmetry-gate selftest — ${total - fails}/${total} assertions passed`);
  if (fails) { console.error('✗ SELFTEST FAILED'); return 1; }
  console.log('✓ reaction-symmetry-gate verified in BOTH directions on the real 2026-08-07 v2.');
  return 0;
}
...
if (/reaction-symmetry-gate\.ts$/.test(process.argv[1] ?? '')) process.exit(main());
