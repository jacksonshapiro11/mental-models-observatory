...
export const HANDLE =
  /(?:^|[\s(,])@[A-Za-z0-9_]{2,}\b|\b(?:surfaced by|relayed by|relaying|carried by|via|per)\s+@[A-Za-z0-9_]{2,}/i;
...
export function findRelayOnly(md: string): RelayHit[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const clause =
      (l.split(/\bSOURCE:/i)[1] ?? '').split(/\|\s*EVIDENCE:/i)[0] ?? '';
...
function resolveBrief(date: string, explicit?: string): string | null {
  ...
  for (const p of [
    ...
    `daily-briefs/${date}-v1-pre-quality-gate.md`,
...
function runOne(date: string, explicit?: string, quiet = false): number {
  ...
  if (!bp) {
    if (!quiet)
      console.error(`relay-provenance-gate: no brief found for ${date}`);
    return 2;
  }
  ...
  if (!quiet) {
    ...
    if (hits.length === 0)
      console.log('  ✓ no agency figure rests on a social relay alone.');
    for (const h of hits) {
      console.log(
        `  ✗ L${h.line}: ${h.agency} release credited to ${h.handle} with NO agency URL — this is a LEAD, not a capture.`
      );
      console.log(`      ${h.text}`);
      console.log(
        `      FIX: fetch the agency release itself and put its URL in the SOURCE clause, or label every figure from it RELAY-UNCONFIRMED and keep it out of a top slot.`
      );
    }
  }
...
function selftest(): number {
  ...
  const censusRelay =
    '- One Census release, three construction series | CLASSIFICATION: NEW | SOURCE: US Census construction spending release, Aug 4, surfaced by @JosephPolitano | EVIDENCE: release published Aug 4.';
  const jolts =
    '- JOLTS miss | SOURCE: BLS JOLTS release 10:00 AM ET, archived at bls.gov/news.release/archives/jolts_08042026.htm | EVIDENCE: BLS release Aug 4.';
  const amd =
    '- AMD cash conversion | SOURCE: ir.amd.com Q2 2026 release, 4:15 PM ET Aug 4 | EVIDENCE: primary release read in full.';
  const crypto =
    '- Hyperliquid gained share | SOURCE: @WuBlockchain relaying July DEX perpetuals volume, Aug 4 | EVIDENCE: SINGLE-SOURCE RELAY, flagged.';

  t(
    findRelayOnly(censusRelay).length === 1,
    'FIRES on the real 08-05 Census line (agency + handle, no agency URL)'
  );
  t(
    findRelayOnly(jolts).length === 0,
    "SILENT on the same brief's JOLTS line (carries bls.gov)"
  );
  t(
    findRelayOnly(amd).length === 0,
    'SILENT on AMD (ir.amd.com — a company, not an agency)'
  );
  t(
    findRelayOnly(crypto).length === 0,
    'SILENT on a crypto-data relay (no agency named)'
  );
  t(
    findRelayOnly(censusRelay + '\n' + jolts + '\n' + amd + '\n' + crypto)
      .length === 1,
    'exactly one hit across all four real lines together'
  );
  t(
    findRelayOnly(
      '- x | SOURCE: US Census release via @someone, see census.gov/construction/c30/current/index.html | EVIDENCE: y'
    ).length === 0,
    'an agency URL in the SOURCE clause clears the relay'
  );
  t(
    findRelayOnly(
      '- x | SOURCE: Census construction spending release, Aug 4 | EVIDENCE: relayed to us by @JosephPolitano'
    ).length === 0,
    'a handle in EVIDENCE prose does not condemn an agency-sourced capture'
  );
  t(
    findRelayOnly(
      'Some body prose mentioning @JosephPolitano and the Census release.'
    ).length === 0,
    'body prose without a SOURCE: clause is not provenance'
  );
  t(
    findRelayOnly(
      '- x | SOURCE: BEA personal income release, flagged by @econ_guy | EVIDENCE: z'
    ).length === 1,
  ...
  console.log(
    "✅ SELFTEST PASS — a relayed agency release is a LEAD; only the agency's own address makes it a capture."
  );
  return 0;
}
...
if (argv.includes('--selftest')) process.exit(selftest());
  ...
  if (!date) {
    console.error(
      'usage: relay-provenance-gate.ts YYYY-MM-DD [--brief <path>] | --selftest | --sweep [N]'
    );
    process.exit(2);
  }
