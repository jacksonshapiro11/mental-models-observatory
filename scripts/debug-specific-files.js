    ...
    if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
      ...
      for (let i = 0; i < Math.min(50, lines.length); i++) {
        ...
        if (line.includes('//') || line.includes('...')) {
          console.log(
            `   → Line ${i + 1}: "${line.trim()}" (contains comments/ellipsis)`
          );
          break;
        }
      }
...
function analyzeFile(fileName) {
  ...
  const expectedModels =
    (modelIdMatches?.length || 0) + (sectionMatches?.length || 0);
...
console.log('\n=== SUMMARY ===');
console.log(
  'Files analyzed for specific parsing issues that prevent model extraction.'
);
