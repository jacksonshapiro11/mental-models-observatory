...
try {
  ...
  getModelHighlightsFromAllDomains =
    parseDomains.getModelHighlightsFromAllDomains;
} catch (error) {
  ...
  const readwiseDataPath = path.join(
    __dirname,
    '..',
    'lib',
    'readwise-data.ts'
  ...
  const domainsMatch = content.match(
    /export const READWISE_DOMAINS: Domain\[\] = \[([\s\S]*?)\n\];/
  );
  if (domainsMatch) {
  ...
  try {
    ...
    console.error(
      '❌ Could not import parse-all-domains. Make sure to run: npm install ts-node --save-dev'
    );
    process.exit(1);
  }
}
...
const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL || 'https://www.cosmictrex.xyz';
...
function selectBestHighlight(highlights) {
  if (
    ...
    highlights.curatedHighlights.length === 0
  ) {
    return null;
  }
...
function generateTwitterThread(model, highlights) {
  ...
  if (
    !highlights?.curatedHighlights ||
    highlights.curatedHighlights.length < 3
  ) {
    console.warn(
      `⚠️  Need at least 3 highlights for ${model.name}, found ${highlights?.curatedHighlights?.length || 0}`
    );
    return null;
  }
  ...
  try {
    ...
    if (fs.existsSync(scenariosPath)) {
      ...
      if (
        modelScenarios &&
        modelScenarios.scenarios &&
        modelScenarios.scenarios.length >= 3
      ) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const scenario =
      scenarios[i] || model.description.substring(0, 120) + '...';
    const shortenedQuote = shortenHighlight(
      highlight.text || highlight.curatorReason || '',
      200
    );
...
function generateVisualCard(model, highlights) {
  ...
  return {
    ...
    design: {
      ...
      bullets:
        model.principles?.slice(0, 3).map(p => p.substring(0, 100)) || [],
      quote: topHighlight
        ? {
...
function generateNewsletterContent(model, highlights, weekNumber) {
  ...
  return {
    week: weekNumber,
    date: new Date(Date.now() + weekNumber * 7 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split('T')[0],
  ...
  return {
    ...
    outline: {
      ...
      insights: {
        title: 'Insights from Books',
        highlights:
          highlights?.curatedHighlights?.slice(0, 5).map(h => ({
            quote: h.text || '',
            book: h.book.title,
            author: h.book.author,
    ...
    tags: [
      ...
      domain?.slug || model.domainSlug,
    ],
    estimatedReadTime:
      Math.ceil(
        (model.description.length +
          (model.principles?.join('').length || 0) +
          (model.applications?.join('').length || 0)) /
          200
      ) + ' min',
  };
}
...
function generateAllContent(days = 30, format = 'all') {
  ...
  for (let day = 0; day < days; day++) {
    ...
    for (const model of dayModels) {
      ...
      if (format === 'all' || format === 'newsletter') {
        modelContent.newsletter = generateNewsletterContent(
          model,
          highlights,
          Math.floor(day / 7) + 1
        );
    ...
    if (format === 'all' || format === 'twitter') {
      ...
      const twitterText = dayContent.models
        .map(m => {
          if (!m.twitter) return '';
  ...
  const calendarRows = [
    'Date,Model,Slug,Twitter,Quotes,Visual,Newsletter,Blog,Video,URL',
  ];
