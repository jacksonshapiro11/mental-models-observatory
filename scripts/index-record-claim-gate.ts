...
export const INDEXES: IndexDef[] = [
  ...
  { key: 'dow', label: 'Dow', re: /\bDow(?:\s+Jones)?\b/gi },
  {
    key: 'nasdaq',
    label: 'Nasdaq',
    re: /\bNasdaq(?:\s*(?:100|Composite))?\b/gi,
  },
...
export function claimConfirmsRecord(claim: unknown): boolean {
  ...
  if (
    /\bnot\s+a\s+record\b|\bno\s+record\b|\bbelow\s+its\b|\bshy\s+of\b|\bnot\s+at\s+a\s+record\b/i.test(
      note
    )
  )
    return false;
...
export function mentionsIn(sentence: string): Mention[] {
  ...
  for (const idx of INDEXES) {
    ...
    while ((m = re.exec(sentence)) !== null)
      out.push({ key: idx.key, label: idx.label, pos: m.index });
  }
...
export function assertedIndexes(sentence: string): Mention[] {
  ...
  for (let i = ms.length - 2; i >= 0; i--) {
    if (frame[i]) continue;
    const tail = windows[i]!.replace(
      new RegExp(`^\\s*(?:${INDEXES.map(x => x.re.source).join('|')})`, 'i'),
      ''
    );
    if (CONNECTIVE_ONLY.test(tail)) {
      frame[i] = frame[i + 1]!;
      neg[i] = neg[i + 1]!;
    }
  }
...
export function checkBrief(
  md: string,
  truth: { claims?: Record<string, unknown> }
): Violation[] {
  const v: Violation[] = [];
  ...
  for (const s of sentences) {
    ...
    for (const m of assertedIndexes(s)) {
      ...
      v.push({
        kind: 'per-index',
        msg: `${m.label}: record asserted, ${why} (claim key "${m.key}") — "${s.trim().slice(0, 150)}"`,
      });
    }
  }
  ...
  const tm = title.match(
    /\b(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+records?\b/i
  );
  if (tm) {
    const claimed = NUMWORD[tm[1]!.toLowerCase()] ?? Number(tm[1]);
    const confirmed = INDEXES.filter(i =>
      claimConfirmsRecord(claims[i.key])
    ).length;
    if (claimed !== confirmed) {
      v.push({
        kind: 'title',
        msg: `title numeral "${tm[1]}" vs ${confirmed} confirmed record${confirmed === 1 ? '' : 's'} in truth.json — Daily Title: "${title}"`,
      });
    }
  }
  return v;
...
function resolveBrief(date: string, explicit?: string): string | null {
  ...
  for (const p of [
    ...
    `daily-briefs/${date}-v2.md`,
    `daily-briefs/${date}-v1.5.md`,
  ]) {
...
function runOne(date: string, explicit?: string, quiet = false): number {
  ...
  if (!bp) {
    if (!quiet)
      console.error(`index-record-claim-gate: no brief found for ${date}`);
    return 2;
  }
  if (!fs.existsSync(tp)) {
    if (!quiet)
      console.error(
        `index-record-claim-gate: no truth file ${tp} — cannot evaluate; a missing truth file is NOT a pass`
      );
    return 2;
  }
  ...
  if (!quiet) {
    ...
    if (v.length === 0)
      console.log(
        '  ✓ every index-record assertion carries its own resolved confirmation, and the title numeral matches.'
      );
...
function selftest(): number {
  ...
  const TRUTH = {
    claims: {
      ...
      dow: {
        resolved: true,
        note: 'RECORD CLOSE, first above 54,000 — confirmed',
      },
      nasdaq: {
        resolved: true,
        note: 'NOT A RECORD. Closed ~2% BELOW its early-June high.',
      },
    },
  };
  const fire =
    '### Three Records and One Leg\n\n*The S&P, the Dow and the Nasdaq all closed at records, with the Nasdaq leading.*';
  const clean =
  ...
  t(
    vf.some(x => x.kind === 'per-index' && /Nasdaq/.test(x.msg)),
    'FIRES on the real 08-05 group claim (Nasdaq inside "closed at records")'
  );
  t(
    vf.some(
      x =>
        x.kind === 'title' && /"Three"/.test(x.msg) && /2 confirmed/.test(x.msg)
    ),
    'FIRES on the real Daily Title numeral (Three vs 2 confirmed)'
  );
  ...
  const flaggedKeys = vf
    .filter(x => x.kind === 'per-index')
    .map(x => (x.msg.match(/claim key "(\w+)"/) ?? [])[1]);
  t(
    flaggedKeys.length === 1 && flaggedKeys[0] === 'nasdaq',
    'flags ONLY the unconfirmed index — the two real records are not touched'
  );

  const vc = checkBrief(clean, TRUTH);
  t(
    vc.length === 0,
    'SILENT on the corrected 08-05 sentence (explicit negation is not an assertion)'
  );

  t(
    assertedIndexes('The S&P, the Dow and the Nasdaq all closed at records')
      .length === 3,
    'a GROUP claim expands to N separate claims, never one'
  );
  t(
    assertedIndexes(
      "the Nasdaq's 2.59 percent led the session without setting a record of its own"
    ).length === 0,
    'an explicit negation asserts nothing'
  );
  t(
    checkBrief(
      'AMD posted a record $11,536 million of revenue and data-center construction hit a record annual pace.',
      TRUTH
    ).length === 0,
    'non-index records (revenue, construction pace) are out of scope'
  );
  t(
    checkBrief('The S&P rose 0.4 percent and the Dow fell 0.1 percent.', TRUTH)
      .length === 0,
    'an ordinary move sentence is not a record claim'
  );
  t(
    checkBrief('### Two Records and One Leg', {
      claims: {
        sp500: { resolved: true, note: 'record' },
        dow: { resolved: false, note: 'record' },
      },
    }).some(x => x.kind === 'title'),
    'an UNRESOLVED claim does not count toward the confirmed total'
  );
  t(
    checkBrief('The Nasdaq closed at a record.', {
      claims: { nasdaq: { resolved: true, note: 'confirmed up 2.6%' } },
    }).some(x => /note does not affirm a record/.test(x.msg)),
    'a resolved claim whose note never says "record" is not a confirmation'
  );
  t(
    claimConfirmsRecord({ resolved: true, note: 'RECORD CLOSE' }) &&
      !claimConfirmsRecord({ resolved: true, note: 'NOT A RECORD' }),
  ...
  console.log(
    '✅ SELFTEST PASS — one index, one confirmation; a counted title is checked against the count.'
  );
  return 0;
}
...
if (argv.includes('--selftest')) process.exit(selftest());
  ...
  if (!date) {
    console.error(
      'usage: index-record-claim-gate.ts YYYY-MM-DD [--brief <path>] | --selftest | --sweep [N]'
    );
    process.exit(2);
  }
