...
export function findMismatches(text: string, defaultYear: number): Mismatch[] {
  ...
  const push = (
    ...
    if (cal !== stated) {
      out.push({
        stated:
          WEEKDAYS[stated]![0]!.toUpperCase() + WEEKDAYS[stated]!.slice(1),
  ...
  const reWordFwd = new RegExp(
    `\\b${WD}\\b,?\\s+${MON}\\s+(\\d{1,2})(?:,?\\s+(\\d{4}))?`,
    'gi'
  );
  // (2) word-date THEN weekday (dash / paren / comma / "a"): "July 24, 2026 — Thursday", "July 24 (Thursday)"
  const reWordBack = new RegExp(
    `${MON}\\s+(\\d{1,2})(?:,?\\s+(\\d{4}))?\\s*[—\\-–(,]\\s*(?:a\\s+)?\\b${WD}\\b`,
    'gi'
  );
  // (3) weekday THEN ISO: "Friday 2026-07-24"
  const reIsoFwd = new RegExp(
    `\\b${WD}\\b,?\\s+(\\d{4})-(\\d{2})-(\\d{2})`,
    'gi'
  );
  // (4) ISO THEN weekday: "2026-07-24 (Friday)"
  const reIsoBack = new RegExp(
    `(\\d{4})-(\\d{2})-(\\d{2})\\s*[—\\-–(,]\\s*(?:a\\s+)?\\b${WD}\\b`,
    'gi'
  );
  ...
  while ((m = reWordFwd.exec(text)) !== null) {
    ...
    push(
      m[1]!,
      m[4] ? parseInt(m[4]!, 10) : defaultYear,
      mon,
      parseInt(m[3]!, 10),
      m[0]!
    );
  ...
  while ((m = reWordBack.exec(text)) !== null) {
    ...
    push(
      m[4]!,
      m[3] ? parseInt(m[3]!, 10) : defaultYear,
      mon,
      parseInt(m[2]!, 10),
      m[0]!
    );
...
function selftest(): number {
  ...
  const critic1 = findMismatches(
    'confirm the effective date as 12:01 AM EDT on July 24, 2026 — Thursday, not Friday.',
    2026
  );
  check(
    'FIRES on the real Critic "July 24, 2026 — Thursday" (July 24 = Friday)',
    critic1.some(
      x =>
        x.stated === 'Thursday' &&
        /Jul 24, 2026/.test(x.date) &&
        x.calendar === 'Friday'
    )
  );

  const critic2 = findMismatches(
    'tariffs took effect July 24 (Thursday) per USTR, not Friday.',
    2026
  );
  check(
    'FIRES on "July 24 (Thursday)" with the year inferred',
    critic2.length > 0
  );

  const fwd = findMismatches(
    'the deadline is Thursday, July 24 this year',
    2026
  );
  check('FIRES on weekday-first "Thursday, July 24"', fwd.length > 0);

  const iso = findMismatches(
    'closes frozen on Thursday 2026-07-24 for the tape',
    2026
  );
  check(
    'FIRES on ISO "Thursday 2026-07-24" (that date is a Friday)',
    iso.length > 0
  );

  // SILENT — the healthy cases, so the gate never "corrects" a correct brief.
  check(
    'SILENT on the correct brief "At midnight Friday, tariffs took effect" (bare weekday, no date)',
    findMismatches(
      'At midnight Friday, tariffs of 10-12.5% took effect on imports from 60 economies.',
      2026
    ).length === 0
  );
  check(
    'SILENT on the correct date-line "Saturday, July 25, 2026"',
    findMismatches('# Saturday, July 25, 2026', 2026).length === 0
  );
  check(
    'SILENT on the correct pairing "Friday, July 24, 2026"',
    findMismatches('markets closed Friday, July 24, 2026 for the weekend', 2026)
      .length === 0
  );
  check(
    'SILENT on the correct ISO pairing "Friday 2026-07-24"',
    findMismatches('the Friday 2026-07-24 close is frozen', 2026).length === 0
  );
  check(
    'SILENT on a bare future date with no weekday "FOMC July 28-29"',
    findMismatches('Watch FOMC July 28-29, where the committee meets.', 2026)
      .length === 0
  );
  ...
  if (fails) {
    console.error(
      '✗ SELFTEST FAILED — the calendar check no longer bites both directions.'
    );
    return 1;
  }
...
function main(): number {
  ...
  if (!file) {
    console.error(
      'Usage: weekday-date-consistency.ts <file.md> [--year YYYY] | --selftest'
    );
    return 2;
  }
  ...
  if (bad.length === 0) {
    console.log(
      `weekday-date-consistency — ${path.basename(fp)} — 0 mismatches. ✓`
    );
    return 0;
  }
  console.error(
    `weekday-date-consistency — ${path.basename(fp)} — ${bad.length} MISMATCH${bad.length === 1 ? '' : 'ES'}:`
  );
  for (const b of bad) {
    console.error(
      `  ✗ text says "${b.stated}" but ${b.date} is a ${b.calendar} — "${b.context}"`
    );
  }
  console.error(
    '  Correct the weekday to the calendar day, or the date to match the weekday. A weekday pinned to a date is arithmetic, not opinion.'
  );
  return 1;
}
process.exit(main());
