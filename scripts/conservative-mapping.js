...
async function conservativeMapping() {
    ...
    if (candidates.length > 0) {
      ...
      for (const candidate of candidates.slice(0, 2)) {
        ...
        try {
          ...
          const data = JSON.parse(response);
          const highlightCount = data.curatedHighlights
            ? data.curatedHighlights.length
            : 0;

          if (highlightCount > 0) {
            console.log(
              `   ✅ HIGH QUALITY MATCH: ${candidate.id} (${highlightCount} highlights, score: ${candidate.score})`
            );
      ...
      if (!foundMatch) {
        ...
        noCloseMatchFound.push({
          ...model,
          reason: 'candidates_no_highlights',
        });
      }
    } else {
  ...
  console.log(
    `✅ High quality mappings: ${highQualityMappings.length}/${brokenModels.length}`
  );
  console.log(
    `❌ No close match found: ${noCloseMatchFound.length}/${brokenModels.length}`
  );

  // Show only high-confidence mappings
  const highConfidenceMappings = highQualityMappings.filter(
    m => m.confidence === 'high'
  );
  const mediumConfidenceMappings = highQualityMappings.filter(
    m => m.confidence === 'medium'
  );
  const lowConfidenceMappings = highQualityMappings.filter(
    m => m.confidence === 'low'
  );
  ...
  if (highConfidenceMappings.length > 0) {
    ...
    for (const mapping of highConfidenceMappings) {
      console.log(
        `  '${mapping.brokenSlug}': '${mapping.workingId}', // ${mapping.brokenName} (${mapping.highlightCount} highlights)`
      );
    }
  }
  ...
  fs.writeFileSync(
    'conservative-mapping-results.json',
    JSON.stringify(conservativeResults, null, 2)
  );
  console.log(
