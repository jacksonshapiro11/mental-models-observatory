...
export const DATA_JSON_RE = /(factcheck|ledger|-log|manifest|calibration)\.json$/i;
...
export function git(cwd: string, args: string[]): { code: number; out: string; err: string } {
    ...
    if (fs.existsSync(gitDir))
      for (const f of fs.readdirSync(gitDir))
        if (f.endsWith('.lock')) {
          ...
          try { if (fs.statSync(p).size === 0) fs.renameSync(p, path.join(cwd, '_to_delete', `${f}.${Date.now()}`)); } catch { /* best effort */ }
        }
  } catch { /* best effort */ }
...
export function dirtyCode(cwd: string): string[] {
  ...
  return r.out.split('\n').filter(l => l.trim() && isTrackedCodeDirt(l)).map(l => l.slice(3).trim());
}

export interface ShuntResult { branch: string | null; files: string[]; alarm: string | null; note: string }

export function shunt(cwd: string, date: string, apply: boolean): ShuntResult {
  const files = dirtyCode(cwd);
  if (!files.length)
    return { branch: null, files: [], alarm: null, note: 'no tracked-code dirt — nothing to shunt, and the rebase is safe to run' };
  ...
  if (!apply)
    return { branch, files, alarm, note: 'DRY RUN — pass --shunt to create the branch and commit' };
  const head = git(cwd, ['rev-parse', '--abbrev-ref', 'HEAD']).out.trim() || 'main';
  const mk = git(cwd, ['checkout', '-B', branch]);
  if (mk.code !== 0) return { branch, files, alarm, note: `FAILED to create ${branch}: ${mk.err.trim().slice(0, 160)}` };
  git(cwd, ['add', '--', ...files]);
  const c = git(cwd, ['commit', '-m', `wip: tracked-code dirt shunted ${date}\n\nUncommitted source in the path of an integration step. Committed here rather\nthan left for a rebase to discard (the IMP-102 / b3512c2 class).\n\n${files.join('\n')}`]);
...
function main(): void {
  ...
  if (argv.includes('--selftest')) {
    ...
    t('[classify] a modified tracked .ts IS tracked-code dirt', isTrackedCodeDirt(' M scripts/six-conversion-gate.ts'));
    t('[classify] an UNTRACKED file is not — it survives a rebase already', !isTrackedCodeDirt('?? scripts/brand-new.ts'));
    t('[classify] a factcheck json is DATA, regenerated nightly — shunting it would make a branch every night and train everyone to ignore them',
      !isTrackedCodeDirt(' M content/daily-updates/weekly/2026-W28-factcheck.json'));
    t('[classify] published content is not code', !isTrackedCodeDirt(' M content/daily-updates/2026-08-28.md'));
    t('[classify] a staged-and-modified .ts still counts (status "MM")', isTrackedCodeDirt('MM scripts/x.ts'));
    ...
    fs.writeFileSync(path.join(repo, 'scripts', 'gate.ts'), 'export const v = 1;\n');
    fs.writeFileSync(path.join(repo, 'data.json'), '{}\n');
    git(repo, ['add', '.']); git(repo, ['commit', '-qm', 'init']);
    const clean = shunt(repo, '2026-08-28', false);
    t('[e2e] a CLEAN tree shunts nothing and says the rebase is safe', clean.files.length === 0 && /safe to run/.test(clean.note));

    fs.writeFileSync(path.join(repo, 'scripts', 'gate.ts'), 'export const v = 2; // uncommitted work\n');
    fs.writeFileSync(path.join(repo, 'data.json'), '{"regenerated":true}\n');
    const dry = shunt(repo, '2026-08-28', false);
    t('[e2e] DRY RUN names exactly the code file, not the json beside it', dry.files.length === 1 && dry.files[0] === 'scripts/gate.ts');
    t('[e2e] and it produces the alarm text with the branch and the files in it',
      !!dry.alarm && dry.alarm.includes('wip/dirt-2026-08-28') && dry.alarm.includes('scripts/gate.ts') && dry.alarm.includes('cosmictrex11@gmail.com'));

    const applied = shunt(repo, '2026-08-28', true);
    const onBranch = git(repo, ['show', '--stat', '--oneline', 'wip/dirt-2026-08-28']).out;
    t('[e2e] APPLY creates wip/dirt-<date> and the work is a COMMIT on it, not a diff on the floor',
      /shunted to wip\/dirt-2026-08-28/.test(applied.note) && /scripts\/gate\.ts/.test(onBranch));
    t('[e2e] the commit body names the class it prevents, so the branch explains itself later',
      /IMP-102 \/ b3512c2 class/.test(git(repo, ['log', '-1', '--format=%B', 'wip/dirt-2026-08-28']).out));
    t('[e2e] HEAD is returned to the original branch — the shunt must not leave the repo somewhere else',
      /^(main|master)$/.test(git(repo, ['rev-parse', '--abbrev-ref', 'HEAD']).out.trim()));
    // 🔴 THE PROPERTY THAT MATTERS: the work must now SURVIVE the thing that used to eat it.
    git(repo, ['checkout', '--', 'scripts/gate.ts']);
    t('[e2e] and after a hard checkout — the operation that used to discard it — the work is still recoverable from the branch',
      /uncommitted work/.test(git(repo, ['show', 'wip/dirt-2026-08-28:scripts/gate.ts']).out) &&
        !/uncommitted work/.test(fs.readFileSync(path.join(repo, 'scripts', 'gate.ts'), 'utf-8')));
  ...
  console.log(`wip-shunt ${date} — ${r.files.length} tracked-code file(s) dirty`);
  if (!r.files.length) { console.log(`✅ ${r.note}`); process.exit(0); }
