...
console.log('── 1. Healthy series: correct 1Y, silent tripwires ──');
{
  ...
  const prices = dates.map(
    (_, i) => 100 * (1 + (0.2 * i) / (dates.length - 1))
  ); // smooth +20%
  const r = calculateChangesChecked(dates, prices);
  check(
    '1Y ≈ +20 on a smooth +20% year',
    Math.abs((r.changes['1Y'] ?? 0) - 20) < 1.0,
    `got ${r.changes['1Y']}`
  );
  check(
    'no stale baselines on gap-free data',
    r.staleBaselines.length === 0,
    r.staleBaselines.join(',')
  );
  check(
    'no scale breaks on smooth data',
    detectScaleBreaks({ dates, prices }).length === 0
  );
}
...
console.log('── 2. The IWF class: 4:1 split in a raw-close series ──');
{
  ...
  const prices = dates.map((_, i) =>
    i < splitIdx
      ? 434
      : 108.5 + (8 * (i - splitIdx)) / (dates.length - splitIdx)
  );
  const series: PriceSeries = { dates, prices };
  const raw = calculateChangesChecked(dates, prices);
  check(
    'raw split-broken series reproduces the shipped −73% class',
    (raw.changes['1Y'] ?? 0) < -70,
    `got ${raw.changes['1Y']}`
  );
  const breaks = detectScaleBreaks(series);
  check(
    'tripwire DETECTS the 4:1 cliff (so the route refuses to publish)',
    breaks.length === 1 && breaks[0]!.ratio > 3.5,
    JSON.stringify(breaks)
  );
  check(
    'MA window over the cliff is also detected (blended-scale MAs never ship)',
    detectScaleBreaks({ dates: dates.slice(-200), prices: prices.slice(-200) })
      .length === 1
  );
  // The fix: an ADJUSTED series is continuous — no break, sane 1Y.
  const adjPrices = dates.map(
    (_, i) => (434 / 4) * (1 + (0.065 * i) / (dates.length - 1))
  );
  const adj = calculateChangesChecked(dates, adjPrices);
  check(
    'adjusted series: no breaks, 1Y ≈ +6.5',
    detectScaleBreaks({ dates, prices: adjPrices }).length === 0 &&
      Math.abs((adj.changes['1Y'] ?? 0) - 6.5) < 1.0,
    `got ${adj.changes['1Y']}`
  );
}
...
console.log('── 3. The SMH class: gap at the year-ago boundary ──');
{
  ...
  const r = calculateChangesChecked(dates, prices);
  check(
    '1Y baseline inside a data hole is flagged STALE, not published',
    r.staleBaselines.includes('1Y'),
    `stale=${r.staleBaselines.join(',')} changes=${JSON.stringify(r.changes)}`
  );
  check(
    '1M/1D unaffected by the old gap',
    r.changes['1M'] != null && r.changes['1D'] != null
  );
}
...
console.log('── 4. Yahoo chart parsing prefers ADJUSTED close ──');
{
  ...
  check(
    'adjclose preferred over raw close',
    s.prices.every(p => p < 200),
...
console.log('── 5. Latest-quote merge ──');
{
  ...
  check(
    'same-day quote replaces the last bar',
    replaced.prices[1] === 102 && replaced.prices.length === 2
  );
  const appended = mergeLatestIntoSeries(base, 103, '2026-07-27');
  check(
    'new-day quote appends',
    appended.prices.length === 3 && appended.dates[2] === '2026-07-27'
  );
  const stale = mergeLatestIntoSeries(base, 99, '2026-07-22');
  check(
    'older quote never rewrites history',
    stale.prices.length === 2 && stale.prices[1] === 101
  );
}

console.log(
  '── 7. The 2026-07-28 window-edge class: first bar lands AFTER the 1Y target ──'
);
{
  ...
  const r = calculateChangesChecked(dates, prices);
  check(
    '1Y PRESENT via the earliest-bar rule (was: silently missing)',
    r.changes['1Y'] != null && Math.abs(r.changes['1Y']! - 20) < 1.0,
    `got ${r.changes['1Y']}`
  );
  check(
    'window-edge baseline is not flagged stale',
    !r.staleBaselines.includes('1Y'),
    r.staleBaselines.join(',')
  );
  ...
  check(
    'first bar 25 days after target → 1Y omitted, not fabricated',
    late.changes['1Y'] == null,
    `got ${late.changes['1Y']}`
  );
}
...
console.log('── 6. MAs unchanged on healthy data ──');
{
  ...
  check(
    '50D of a rising series sits below the latest',
    mas['50D']! < prices[prices.length - 1]!
  );
}

console.log(
  '── 8. The NATGAS class: futures roll cliff — continuous proxy publishes horizons ──'
);
{
  ...
  const ngPrices = dates.map((_, i) =>
    i < cliffIdx
      ? 4.8
      : 2.5 + (0.2 * (i - cliffIdx)) / Math.max(1, dates.length - cliffIdx)
  );
  const ngBreaks = detectScaleBreaks({ dates, prices: ngPrices });
  check(
    'NG=F-like roll cliff is DETECTED (threshold stays 1.5)',
    ngBreaks.length === 1 && ngBreaks[0]!.ratio > 1.5,
    JSON.stringify(ngBreaks)
  );
  // Continuous proxy (UNG-class): no cliff → 5D/1M/1Y all present.
  const ungPrices = dates.map(
    (_, i) => 14 * (1 + (0.1 * i) / (dates.length - 1))
  );
  ...
  check(
    'UNG-like continuous series: 5D/1M/1Y all publish',
    ung.changes['5D'] != null &&
      ung.changes['1M'] != null &&
      ung.changes['1Y'] != null,
    JSON.stringify(ung.changes)
  );
  // Both directions: never merge futures dollars into the ETF series (instant invented cliff).
  const mixed = mergeLatestIntoSeries(
    { dates: dates.slice(0, -1), prices: ungPrices.slice(0, -1) },
    ngPrices[ngPrices.length - 1]!,
    dates[dates.length - 1]!
  );
  check(
    'mixing NG=F spot into UNG series creates a DETECTABLE break (caller must merge proxy quote)',
    detectScaleBreaks(mixed).length >= 1
  );
  ...
  check(
    'raw NG=F 200D MA window still trips',
    detectScaleBreaks(raw200).length === 1
  );
  const adj = backAdjustScaleBreaks(ngSeries);
  check(
    'back-adjust clears NG=F roll cliffs',
    detectScaleBreaks(adj).length === 0
  );
  const adjMas = calculateMAs(adj.prices);
  check(
    'back-adjusted NG=F publishes 50D and 200D MAs',
    adjMas['50D'] != null && adjMas['200D'] != null,
    JSON.stringify(adjMas)
  );
  check(
    'back-adjust preserves latest (current contract) price',
    adj.prices[adj.prices.length - 1] === ngPrices[ngPrices.length - 1]
  );
}

console.log(
  '── 9. Live merge: multi-day % must move with live price (2026-07-29) ──'
);
{
  ...
  const live = 504.22;
  const recomputed = recomputeChangesFromLive(live, baselines, {
    prevClose: 500,
  });
  check(
    '1Y recomputed from live + baseline ≈ +73 (not frozen +82)',
    Math.abs((recomputed['1Y'] ?? 0) - 72.99) < 0.5,
    `got ${recomputed['1Y']}`
  );
  check(
    'frozen snapshot 1Y next to moved live price is the FAIL class',
    Math.abs(frozen1Y - (recomputed['1Y'] ?? 0)) > 5
  );
  check(
    '5D and 1M also move with live price',
    recomputed['5D'] != null && recomputed['1M'] != null
  );
  check(
    '1D prefers live prevClose over stored 1D baseline',
    recomputed['1D'] === roundPct(live, 500),
    `got ${recomputed['1D']}`
  );
  // Both directions: missing baseline → omit, never invent.
  const partial = recomputeChangesFromLive(live, { '1D': 500, '1Y': 291.4 });
  check(
    'missing 5D/1M baselines are omitted, not fabricated',
    partial['5D'] == null && partial['1M'] == null && partial['1Y'] != null
  );
  ...
  const r = calculateChangesChecked(dates, prices);
  check(
    'calculateChangesChecked returns baselines for every published horizon',
    Object.keys(r.changes).every(
      k => r.baselines[k] != null && r.baselines[k]! > 0
    )
  );
  const fromBaselines = recomputeChangesFromLive(
    prices[prices.length - 1]!,
    r.baselines
  );
  check(
    'recompute from stored baselines matches snapshot changes',
    Object.keys(r.changes).every(
      k => Math.abs((fromBaselines[k] ?? 0) - r.changes[k]!) < 0.02
    ),
    JSON.stringify({ snap: r.changes, live: fromBaselines })
  );
}
...
console.log('── 10. 5D = 5 trading sessions (Yahoo), not 7 calendar days ──');
{
  ...
  const dates = weekdays('2026-01-02', '2026-01-23').filter(
    d => d !== '2026-01-19'
  );
  ...
  check(
    '5D baseline is exactly 5 sessions back (not calendar-7)',
    r.baselines['5D'] === prices[td5Idx],
    `base=${r.baselines['5D']} expected=${prices[td5Idx]} @ ${dates[td5Idx]}`
  );
  check(
    'calendar-7 would have picked a DIFFERENT bar this holiday week',
    dates[td5Idx] !== '2026-01-16',
    `td5date=${dates[td5Idx]}`
  );
  ...
  check(
    'crypto 5D = 5 daily bars back (24/7 series)',
    cr.baselines['5D'] === cryptoPrices[cLatest - 5],
    `base=${cr.baselines['5D']} @ expected idx ${cLatest - 5}`
  );
}
