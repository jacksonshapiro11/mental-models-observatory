...
async function manualOAuth2Flow() {
  ...
  if (!clientId || !clientSecret) {
    console.error(
      '❌ TWITTER_CLIENT_ID or TWITTER_CLIENT_SECRET not found in .env.local'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      "   (You'll be redirected to a page that might not load - that's OK!)"
    );
    console.log('\n📋 Step 3: Copy the FULL URL from your browser address bar');
    console.log(
      '   It will look like: http://localhost:3000/callback?state=...&code=...\n'
    );
    ...
    const returnedState = decodeURIComponent(
      urlParams.searchParams.get('state')
    );
