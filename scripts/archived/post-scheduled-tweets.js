...
async function postThread(threadData, dryRun = false) {
  ...
  for (let idx = 0; idx < threadData.thread.length; idx++) {
    ...
    if (dryRun) {
      console.log(
        `  [DRY RUN] Tweet ${tweetNum}/${threadData.thread.length}: ${tweetText.substring(0, 50)}...`
      );
      ...
      try {
        ...
        if (result.success) {
          console.log(
            `  ✅ Posted tweet ${tweetNum}/${threadData.thread.length}`
          );
...
async function postScheduledTweets(force = false, checkAll = false) {
  ...
  for (const item of toPost) {
    const threadPath = path.join(
      scheduledDir,
      item.date,
      `${item.slug}-thread-${item.thread}.json`
    );

    if (!fs.existsSync(threadPath)) {
      console.error(`❌ Thread file not found: ${threadPath}`);
      results.errors.push(
        `${item.model} - Thread ${item.thread}: File not found`
      );
      continue;
    }
    ...
    console.log(
      `🧵 ${item.model} - Thread ${item.thread}/${item.totalThreads}`
    );
    ...
    try {
      ...
      results.errors.push(
        `${item.model} - Thread ${item.thread}: ${error.message}`
      );
    }
  }
