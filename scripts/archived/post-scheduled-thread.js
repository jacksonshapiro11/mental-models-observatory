...
async function postScheduledThread() {
  ...
  if (!fs.existsSync(queueFile)) {
    ...
    console.log(
      '💡 Add tweets with: node scripts/parse-weekly-markdown.js <markdown-file>'
    );
    return;
  }
  ...
  if (toPost.length === 0) {
    ...
    if (nextScheduled) {
      ...
      console.log(
        `   Scheduled for: ${nextTime.toLocaleString('en-US', { timeZone: 'America/New_York' })} EST\n`
      );
    }
    return;
  }
  ...
  console.log(
    `   Scheduled: ${threadToPost.scheduledDate} ${threadToPost.scheduledTime} EST`
  );
  if (toPost.length > 1) {
    console.log(
      `   ⚠️  ${toPost.length - 1} other tweet(s) also need posting\n`
    );
  } else {
    console.log('');
  }
  ...
  const updatedQueue = queue.map(item =>
    item.threadIndex === threadToPost.threadIndex &&
    item.scheduledDateTime === threadToPost.scheduledDateTime
      ? threadToPost
      : item
  );
