...
function updateSecret(secretName, secretValue) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const token =
      process.env.PAT || process.env.GITHUB_PAT || process.env.GITHUB_TOKEN;
    ...
    gh.on('close', code => {
      if (code === 0) {
        ...
        console.error(
          '💡 Note: GITHUB_TOKEN may not have permission to update secrets.'
        );
        console.error(
          '   Consider using a PAT stored as GITHUB_PAT secret with repo scope.'
        );
        reject(error);
      }
    });
...
async function main() {
  ...
  if (updated) {
    ...
    console.error(
      '   Or set up a GITHUB_PAT secret with repo scope for automatic updates.'
    );
  }
}
