...
function loadAllModels() {
  ...
  try {
    ...
    console.log(
      `Parsed ${uniqueModels.length} unique models from ${files.length} Readwise files`
    );
...
function generateMarkdown(models) {
  ...
  const sortedModels = models.sort((a, b) =>
    a.modelTitle.localeCompare(b.modelTitle)
  );
...
function main() {
  try {
    ...
    if (models.length === 0) {
      console.log(
        '❌ No models found. Check the Readwise website notes directory.'
      );
      return;
    }
