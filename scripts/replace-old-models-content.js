...
const codeMapping = {
  '13B': {
    oldName: 'Social Identity & Group Belonging',
    newName: 'Social Reality Construction & Collective Belief',
  },
  '14B': {
    oldName: 'Communication Bandwidth & Medium Constraints',
    newName: 'Semantics vs. Meaning & Deep Understanding',
  },
  '15C': {
    oldName: 'Media as Reality Constructor',
    newName: 'Predictive Processing & Constructed Experience',
  },
  '18A': {
    oldName: 'Quality vs. Quantity in Relationships',
    newName: 'Love as Nuclear Fuel & Life Foundation',
  },
  '18D': {
    oldName: 'Attachment & Interdependence',
    newName: 'Compounding Human Connections',
  },
  '19C': {
    oldName: 'Sleep & Recovery as Performance Foundation',
    newName: 'Physiological States & Decision Quality',
  },
  '20B': {
    oldName: 'Flow States & Peak Performance',
    newName: 'Mind-Body Integration & Embodied Presence',
  },
  '20C': {
    oldName: 'Habit Formation & Behavior Change',
    newName: 'Deconstructing Mental Patterns & Pain Body',
  },
  '20D': {
    oldName: 'Meditation & Contemplative Practice',
    newName: 'Transcending Ego & Two-Selves Integration',
  },
};
...
for (let i = 1; i < modelSections.length; i++) {
  ...
  const lastCode = codeMatch
    ? codeMatch[codeMatch.length - 1].match(/(\d+[A-Z])/)[1]
    : null;
...
for (const model of models) {
  console.log(
    `📌 Updating ${model.code}: ${codeMapping[model.code]?.oldName} → ${model.name}`
  );
    ...
    if (!inString) {
      ...
      if (braceCount === 0) {
        ...
        if (!idMatch || !slugMatch || !domainMatch || !domainSlugMatch) {
          console.log(
            `   ❌ Could not extract required fields from model ${model.code}`
          );
          break;
        }
...
${escapedApplications}
    ...
    "updatedAt": "${new Date().toISOString()}"
        ...
        dataContent =
          dataContent.substring(0, modelStart) +
          newModel +
          dataContent.substring(modelEnd);
        updatedCount++;
