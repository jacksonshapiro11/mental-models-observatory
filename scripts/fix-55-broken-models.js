...
async function fix55BrokenModels() {
  ...
  for (let i = 0; i < brokenModels.length; i++) {
      ...
      for (const keyword of allKeywords) {
        ...
        for (const readwiseWord of readwiseWords) {
          if (
            readwiseWord &&
            keyword &&
            (readwiseWord.includes(keyword) || keyword.includes(readwiseWord))
          ) {
            score += 1;
          }
        }
    ...
    console.log(
      `   Top candidates: ${topCandidates.map(c => `${c.id}(${c.score})`).join(', ')}`
    );
    ...
    for (const candidate of topCandidates) {
      try {
        ...
        if (highlightCount > 0) {
          console.log(
            `   ✅ FOUND MATCH: ${candidate.id} (${highlightCount} highlights)`
          );
          foundMappings.push({
  ...
  console.log('\n=== RESULTS ===');
  console.log(
    `✅ Found mappings: ${foundMappings.length}/${brokenModels.length}`
  );
  ...
  const fixResults = {
    ...
    summary: {
      ...
      successRate: Math.round(
        (foundMappings.length / brokenModels.length) * 100
      ),
    },
  };
