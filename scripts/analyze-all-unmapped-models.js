...
async function analyzeAllUnmappedModels() {
  ...
  const staticFrameworkModels = modelMatches
  ...
  for (let domain = 1; domain <= 40; domain++) {
    if (modelsByDomain[domain]) {
      ...
      const mappedCount = domainModels.filter(m =>
        currentParsedModels.includes(m)
      ).length;
      const unmappedCount = domainModels.length - mappedCount;

      console.log(
        `Domain ${domain.toString().padStart(2, '0')}: ${domainModels.length} models | ${mappedCount} mapped ✅ | ${unmappedCount} unmapped ❌`
      );

      if (unmappedCount > 0) {
        const unmappedInDomain = domainModels.filter(
          m => !currentParsedModels.includes(m)
        );
        console.log(`    Unmapped: ${unmappedInDomain.join(', ')}`);
      }
    } else {
      console.log(
        `Domain ${domain.toString().padStart(2, '0')}: No models found`
      );
    }
  }
  ...
  console.log(
    `Mapped to Readwise: ${mappedModels.length} (${Math.round((mappedModels.length / staticFrameworkModels.length) * 100)}%)`
  );
  console.log(
    `Unmapped (no Readwise): ${unmappedModels.length} (${Math.round((unmappedModels.length / staticFrameworkModels.length) * 100)}%)`
  );
  ...
  if (unmappedModels.length > 0) {
    console.log(
      `1. Create slug mappings for ${unmappedModels.length} unmapped models`
    );
    console.log(`2. Or create Readwise curation files for these models`);
    console.log(
      `3. Or hide these models from the website until they have highlights`
    );
  } else {
