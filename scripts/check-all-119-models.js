...
async function checkAll119Models() {
  ...
  console.log('=== SUMMARY ===');
  console.log(
    `✅ Working models: ${workingModels.length}/${modelObjects.length} (${Math.round((workingModels.length / modelObjects.length) * 100)}%)`
  );
  console.log(
    `❌ Broken models: ${brokenModels.length}/${modelObjects.length} (${Math.round((brokenModels.length / modelObjects.length) * 100)}%)`
  );
