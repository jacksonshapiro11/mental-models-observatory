...
allDomains.forEach(domain => {
  ...
  if (domainModels.length >= 3) {
    ...
    curatedPaths.push({
      ...
      modelSlugs: domainModels
        .slice(0, Math.min(7, domainModels.length))
        .map(m => m.slug),
    });
    ...
    if (domainModels.length >= 6) {
      curatedPaths.push({
        ...
        modelSlugs: domainModels
          .slice(0, Math.min(10, domainModels.length))
          .map(m => m.slug),
      });
    }
  }
...
const quickWinTopics = [
  {
    keywords: ['decision', 'choice'],
    title: 'Quick Decision Tools',
    icon: '⚡',
  },
  {
    keywords: ['bias', 'cognitive'],
    title: 'Bias Awareness Quick Guide',
    icon: '🧩',
  },
  {
    keywords: ['system', 'feedback'],
...
const validPaths = curatedPaths.filter(
  p => p.modelSlugs && p.modelSlugs.length >= 3
);
...
categories.forEach(cat => {
  console.log(
    `   - ${cat}: ${validPaths.filter(p => p.category === cat).length}`
  );
});
