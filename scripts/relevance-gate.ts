...
function main() {
  ...
  const intelArg = args.includes('--intel')
    ? args[args.indexOf('--intel') + 1]
    : null;
  const strict = args.includes('--strict');
  if (!briefArg) {
    console.error(
      'Usage: relevance-gate.ts <brief.md> [--intel <file>] [--strict]'
    );
    process.exit(2);
  }
  ...
  if (!intelPath) {
    console.log(
      `relevance-gate — no intelligence file found for ${date}. Cannot verify relevance.`
    );
  ...
  for (const line of six.split('\n')) {
    ...
    const b = line.match(/^- \*\*(.+?)\*\*/);
    if (b && cur) targets.push({ section: cur, lead: b[1] });
  }
  const takeTitle = (sliceBetween(body, '# ▸ THE TAKE', '# ▸ INNER GAME').match(
    /^\s*##\s+(.+)$/m
  ) ?? [])[1];
  ...
  for (const t of targets) {
    ...
    if (hits.length < 2 || ratio < 0.25) {
      flags.push(
        `${t.section}: lead "${t.lead.slice(0, 70)}…" has weak support in today's intel (${hits.length}/${toks.length} key terms found). Verify it is sourced from today, not the worldview corpus.`
      );
    }
  }
  ...
  if (flags.length === 0) {
    console.log(
      `\n✅ RELEVANCE-GATE PASS — every section traces to today's intel.`
    );
    process.exit(0);
  }
  console.log(
