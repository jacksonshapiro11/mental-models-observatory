...
function selftest(): number {
  ...
  t(
    'FIRES on an uncommitted tracked script (the IMP-102 class)',
    analyse(' M scripts/ceiling-lint.ts', '').some(
      f => f.kind === 'UNCOMMITTED-TRACKED-CODE'
    )
  );
  t(
    'SILENT on an untracked-path edit (system/ is gitignored by policy)',
    analyse(' M system/Brief_Writer.md', '').length === 0
  );
  t(
    'FIRES on unpushed commits (the 4-day red)',
    analyse('', 'abc1234 gates: something').some(
      f => f.kind === 'UNPUSHED-COMMITS'
    )
  );
  t(
    'counts multiple unpushed commits',
    analyse('', 'a1 x\nb2 y\nc3 z')[0]!.message.includes('3 commit(s)')
  );
  t(
    'SILENT when everything is committed and pushed',
    analyse('', '').length === 0
  );
  t(
    'handles staged-and-modified (MM) markers',
    analyse('MM scripts/fact-gate.ts', '').some(f =>
      f.message.includes('scripts/fact-gate.ts')
    )
  );
...
function main(): number {
  ...
  for (const f of findings)
    console.error(`  ${strict ? '🔴' : '🟡'} ${f.kind}: ${f.message}`);
  if (!findings.length)
    console.log('  ✓ all tracked code committed and pushed.');
