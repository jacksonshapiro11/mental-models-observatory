...
function createPromptTemplate(model, highlights, frameworkRules) {
  ...
  const voiceSection =
    frameworkRules.match(/## Voice & Tone([\s\S]*?)---/)?.[1] || '';
  const scenarioSection =
    frameworkRules.match(/## Tweet 1: The Scenario([\s\S]*?)---/)?.[1] || '';
  const examplesSection =
    frameworkRules.match(/## Complete Thread Examples([\s\S]*?)---/)?.[1] || '';
...
function generateWeeklyPrompts(modelsPerWeek = 7) {
  ...
  console.log(
    `📝 Generating prompts for ${modelsPerWeek} models (${modelsPerWeek === 7 ? '1 per day' : '2 per day'})...\n`
  );
    ...
    JSON.stringify(
      {
        ...
        postingFrequency:
          modelsPerWeek === 7 ? 'once per day' : 'twice per day',
...
Process all prompts in the batch file. Be consistent, concrete, and relatable.`;
  ...
  console.log(
    `📅 Posting: ${modelsPerWeek === 7 ? 'Once per day' : 'Twice per day'} for ${Math.ceil(modelsPerWeek / (modelsPerWeek === 7 ? 1 : 2))} days`
  );
  console.log(`\n📋 Next steps:`);
...
if (modelArg) {
  ...
  if (promptData) {
    ...
    // Save to file
    const outputDir =
      outputArg || path.join(process.cwd(), 'monthly-prompts', 'single');
