...
function generateMarkdown(models) {
  ...
  const totalHighlights = sortedModels.reduce(
    (sum, model) => sum + model.curatedHighlights.length,
    0
  );
...
`;
  ...
  sortedModels.forEach((model, modelIndex) => {
    ...
    const sortedHighlights = [...model.curatedHighlights].sort(
...
function main() {
  ...
  const totalHighlights = models.reduce(
    (sum, m) => sum + m.curatedHighlights.length,
    0
  );
