...
async function analyzeSlugMismatches() {
  ...
  const modelsSection = content.match(
    /export const READWISE_MODELS[\s\S]*?^\];/m
  );
  if (!modelsSection) {
  ...
  // Find exact matches
  const exactMatches = staticSlugs.filter(slug =>
    readwiseModelIds.includes(slug)
  );
  console.log(`✅ Exact matches: ${exactMatches.length}`);

  // Find mismatches that need mapping
  const unmatchedStatic = staticSlugs.filter(
    slug => !readwiseModelIds.includes(slug)
  );
  const unmatchedReadwise = readwiseModelIds.filter(
    id => !staticSlugs.includes(id)
  );
  ...
  for (const staticSlug of unmatchedStatic.slice(0, 20)) {
    ...
    for (const readwiseId of unmatchedReadwise) {
      ...
      if (similarity > 0.7) {
        ...
        console.log(
          `  ${staticSlug} → ${readwiseId} (${Math.round(similarity * 100)}% similar)`
        );
  ...
  for (const testSlug of testCases) {
    ...
    if (!isInStatic && !isInReadwise) {
      ...
      const similarStatic = staticSlugs.find(
        s => calculateSimilarity(s, testSlug) > 0.8
      );
      const similarReadwise = readwiseModelIds.find(
        r => calculateSimilarity(r, testSlug) > 0.8
      );
      if (similarStatic) console.log(`    Similar static: ${similarStatic}`);
      if (similarReadwise)
        console.log(`    Similar Readwise: ${similarReadwise}`);
    }
  }
