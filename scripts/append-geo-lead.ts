...
export const THEATERS: { name: string; kws: RegExp }[] = [
  {
    ...
    kws: /\b(iran|iranian|tehran|irgc|hormuz|persian gulf|gulf state|gulf states|kuwait|bahrain|qatar|khamenei|revolutionary guard|chabahar|houthi|red sea)\b/gi,
  },
  {
    name: 'Russia/Ukraine',
    kws: /\b(russia|russian|ukraine|ukrainian|kyiv|kiev|moscow|putin|kremlin|iskander|zircon|donbas|kharkiv|zelensky)\b/gi,
  },
  {
    name: 'Korea',
    kws: /\b(north korea|pyongyang|kim jong|dprk|south korea|seoul|choe son hui)\b/gi,
  },
  {
    name: 'Taiwan/China',
    kws: /\b(taiwan|taipei|taiwan strait|cross-strait)\b/gi,
  },
  {
    name: 'China',
    kws: /\b(china|chinese|beijing|xi jinping|\bpla\b|south china sea)\b/gi,
  },
  {
    name: 'Europe/NATO',
    kws: /\b(nato|brussels|european union|germany|france|britain|british|poland|baltic)\b/gi,
  },
  { name: 'Japan/Asia', kws: /\b(japan|japanese|tokyo)\b/gi },
  {
    name: 'AI/Technology Policy',
    kws: /\b(executive order|white house|ai policy|chip export|semiconductor export|tech policy)\b/gi,
  },
];
...
export function parseLogRows(trackerMd: string): LogRow[] {
  ...
  for (const line of lines) {
    ...
    const m = inSection
      ? line.match(
          /^\|\s*(\d{4}-\d{2}-\d{2})\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|/
        )
      : null;
    if (m)
      rows.push({ date: m[1]!, theater: m[2]!.trim(), count: m[3]!.trim() });
  }
  return rows;
}
...
export function insertRow(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inSection && /^###\s/.test(lines[i]!) && !lines[i]!.startsWith(SECTION))
      break;
    if (inSection && /^\|\s*\d{4}-\d{2}-\d{2}\s*\|/.test(lines[i]!))
      lastRowIdx = i;
  }
...
function run(date: string): number {
  ...
  if (!fs.existsSync(briefPath)) {
    console.error(`No published brief for ${date} at ${briefPath}`);
    return 2;
  }
  const trackerPath = path.join(root, 'system', 'Thesis_Tracker.md');
  if (!fs.existsSync(trackerPath)) {
    console.error('Thesis_Tracker.md not found');
    return 2;
  }
  ...
  if (updated === null) {
    console.log(
      `append-geo-lead — ${date} already recorded (or no log table); no-op.`
    );
    return 0;
  }
  fs.writeFileSync(trackerPath, updated);
  console.log(
    `append-geo-lead — appended ${date} | ${theater} | ${theater === 'NEEDS-REVIEW' ? '—' : count} (auto).`
  );
  return 0;
}
...
function selftest(): number {
  ...
  if (fs.existsSync(jul21)) {
    ...
    assert(
      'extractGeo1 pulls the real 07-21 Geo-1 (mentions Iran)',
      /iran/i.test(g),
      g.slice(0, 60)
    );
    assert(
      'classifyTheater(real 07-21 Geo-1) = Iran/Middle East',
      classifyTheater(g) === 'Iran/Middle East',
      classifyTheater(g)
    );
  }
  // classifyTheater — synthetic both directions.
  assert(
    'classify a Kyiv barrage → Russia/Ukraine',
    classifyTheater(
      '- **Russia launched ~40 missiles at Kyiv overnight, incl. Iskander and Zircon.**'
    ) === 'Russia/Ukraine'
  );
  assert(
    'classify a NK foreign-minister trip → Korea',
    classifyTheater(
      "- **North Korea's Choe Son Hui arrived in Pyongyang for talks.**"
    ) === 'Korea'
  );
  assert(
    'classify an unmatched bullet → NEEDS-REVIEW',
    classifyTheater(
      '- **A domestic infrastructure bill advanced through committee this week.**'
    ) === 'NEEDS-REVIEW'
  );
  assert(
    'classify an empty Geo-1 → NEEDS-REVIEW',
    classifyTheater('') === 'NEEDS-REVIEW'
  );

  // computeConsecutive — increment on same theater, reset on switch.
  const priorIran4: LogRow[] = [
    { date: '2026-07-18', theater: 'Iran/Middle East', count: '4' },
  ];
  assert(
    'computeConsecutive: prior Iran day 4 + Iran → 5',
    computeConsecutive(priorIran4, 'Iran/Middle East') === 5
  );
  const priorRussia: LogRow[] = [
    { date: '2026-07-20', theater: 'Russia/Ukraine', count: '1' },
  ];
  assert(
    'computeConsecutive: prior Russia + Iran → 1 (streak reset)',
    computeConsecutive(priorRussia, 'Iran/Middle East') === 1
  );
  ...
  if (fs.existsSync(trackerPath)) {
    ...
    const rows = parseLogRows(tracker);
    assert(
      'parseLogRows reads the real log (≥ 20 rows)',
      rows.length >= 20,
      `${rows.length} rows`
    );
    assert(
      'idempotent: inserting a date already present is a no-op',
      insertRow(
        tracker,
    ...
    assert(
      'insertRow adds one dated row for a new date',
      !!updated &&
        parseLogRows(updated!).some(
          r => r.date === '2099-01-01' && r.theater === 'Iran/Middle East'
        )
    );
    ...
    assert(
      'NEEDS-REVIEW row uses a "—" count (no fabricated number)',
      !!nr && /\| 2099-01-02 \| NEEDS-REVIEW \| — \|/.test(nr!)
    );
  }
  ...
  for (const r of results) {
    console.log(
      `  ${r.pass ? '✓' : '✗'} ${r.name}${!r.pass && r.detail ? ` — ${r.detail}` : ''}`
    );
    if (!r.pass) failed++;
  }
  console.log(
  ...
  console.log(
    '✓ derives the theater from the real Geo-1, computes consecutive days, is idempotent, and flags the unclassifiable LOUD.'
  );
  return 0;
}
