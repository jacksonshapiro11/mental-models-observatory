...
console.log(
  '📝 Updating remaining models with case-insensitive code matching...\n'
);
...
console.log(
  `✅ Parsed ${Object.keys(allNewContent).length} models from description files\n`
);
  ...
  while (i < dataContent.length) {
    ...
    if (!inString) {
      ...
      if (braceCount === 0) {
        ...
        console.log(
          `📌 Updating ${code}: ${oldNameMatch[1]} → ${newContent.name}`
        );

        // Create the new model
        const escapedDescription = newContent.description
          .replace(/"/g, '\\"')
          .replace(/\n/g, ' ');
        const escapedName = newContent.name.replace(/"/g, '\\"');
        const escapedPrinciples = newContent.principles
          .map(p => `      "${p.replace(/"/g, '\\"').replace(/\n/g, ' ')}"`)
          .join(',\n');
        const escapedApplications = newContent.applications
