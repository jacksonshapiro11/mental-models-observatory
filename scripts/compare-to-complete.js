...
console.log(
  '🔍 Comparing lib/readwise-data.ts vs MENTAL_MODELS_COMPLETE.md...\n'
);
...
const modelsByCode = {};
const modelMatches = dataContent.matchAll(
  /"code":\s*"([^"]+)",[\s\S]*?"name":\s*"([^"]+)",[\s\S]*?"description":\s*"([^"]+)"/g
);
...
for (const match of matches.slice(0, 10)) {
  ...
  if (match.reason === 'Name updated (same substance)') {
    console.log(
      `CODE ${match.code}: "${match.old}" → "${match.current}" (${match.descSimilarity}% similarity)`
    );
  }
}
