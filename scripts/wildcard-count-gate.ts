...
interface Result {
  ...
  floor: number;
  target: number;
  status: 'PASS' | 'ADVISORY' | 'FAIL' | 'NO-SECTION';
  message: string;
}
...
export function evaluate(
  ...
  if (count === null) {
    return {
      ...
      status: 'NO-SECTION',
      message:
        'No "## The Wild Card" section found — cannot count. (If the brief has no Wild Card by design, this gate does not apply.)',
    };
  }
  if (count < floor) {
    return {
      ...
      target,
      status: 'FAIL',
      message: `WILD CARD UNDERCOUNT — ${count} item${count === 1 ? '' : 's'} present, floor is ${floor} (target ${target}). A half-empty Wild Card is a structural shortfall (07-20: 2 of 4). Backfill from system/Wild_Card_Generator.md (four distinct domains, Four-Test Gate) or the QG must restore the missing items before publish.`,
    };
  }
  if (count < target) {
    return {
      ...
      target,
      status: 'ADVISORY',
      message: `WILD CARD AT FLOOR — ${count} items (target ${target}). Acceptable minimum, but below spec: log why the 4th item was cut.`,
    };
  }
  return {
    count,
    ...
    target,
    status: 'PASS',
    message: `Wild Card carries ${count} items (target ${target}).`,
  };
}
...
function selftest(): number {
  ...
  const okFourPass =
    evaluate(four).status === 'PASS' && countWildCardItems(four) === 4;
  const okThreeAdvisory =
    evaluate(three).status === 'ADVISORY' && countWildCardItems(three) === 3;
  const okTwoFail =
    evaluate(two).status === 'FAIL' && countWildCardItems(two) === 2;
  ...
  if (fs.existsSync(realPub))
    okRealPubParses =
      countWildCardItems(fs.readFileSync(realPub, 'utf8')) !== null;

  console.log('wildcard-count-gate --selftest');
  console.log(`  4 items -> PASS: ${okFourPass ? '✓' : '✗'}`);
  console.log(
    `  3 items -> ADVISORY (floor, below target): ${okThreeAdvisory ? '✓' : '✗'}`
  );
  console.log(
    `  2 items -> FAIL (structural shortfall): ${okTwoFail ? '✓' : '✗'}`
  );
  console.log(
    `  bold-led items are counted (not just plain paragraphs): ${okBoldedFour ? '✓' : '✗'}`
  );
  console.log(`  HTML comments are not items: ${okCommentIgnored ? '✓' : '✗'}`);
  console.log(
    `  absent section -> NO-SECTION (never a silent pass): ${okNoSection ? '✓' : '✗'}`
  );
  console.log(
    `  FIRES on the real 07-20 v2 (2 of 4): ${okReal2026 ? '✓' : '✗'}`
  );
  console.log(
    `  the real published 07-20 parses to a count: ${okRealPubParses ? '✓' : '✗'}`
  );
  ...
  if (ok) {
    console.log(
      '\n✅ SELFTEST PASS — a half-empty Wild Card now FAILs instead of shipping 50% short.'
    );
    return 0;
  }
  console.error('\n❌ SELFTEST FAIL');
...
function main(): number {
  ...
  if (!file) {
    console.error(
      'usage: wildcard-count-gate.ts <brief.md> [--min N] [--target N] | --selftest'
    );
    return 2;
  }
  ...
  const floor =
    floorIdx > -1 ? parseInt(args[floorIdx + 1]!, 10) : DEFAULT_FLOOR;
  const target =
    targetIdx > -1 ? parseInt(args[targetIdx + 1]!, 10) : DEFAULT_TARGET;
  ...
  console.log(
    `wildcard-count-gate — ${path.basename(file)}: ${r.count ?? 'no section'} item(s) [${r.status}]`
  );
