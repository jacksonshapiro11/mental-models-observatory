...
function parseDescriptionFiles() {
  ...
  for (const file of files) {
    ...
    for (const section of modelSections) {
      ...
      const principlesMatch = section.match(
        /\*\*Core Principles:\*\*\s*([\s\S]*?)\n\*\*Key Applications:\*\*/
      );
      const applicationsMatch = section.match(
...
function migrateModels() {
  ...
  content = content.replace(/"code":\s*"([^"]+)"/g, (match, code) => {
  ...
  console.log(`\n🔄 Next steps:`);
  console.log(
    `   1. Regenerate markdown docs: node scripts/export-all-models-simple.js`
  );
