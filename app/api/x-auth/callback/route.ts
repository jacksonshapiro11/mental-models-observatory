...
export async function GET(req: NextRequest) {
  ...
  const pending = parseRedisJson<{ codeVerifier: string; state: string }>(
    pendingRaw
  );
  ...
  if (!pending) {
    return NextResponse.json(
      {
        error:
          'No pending auth found. Start over at /api/x-auth?secret=YOUR_SNAPSHOT_SECRET',
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (!stored) {
      return NextResponse.json(
        {
          error:
            'Token exchange succeeded but Redis write failed. Check UPSTASH env vars.',
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      ...
      nextStep:
        'Optional: update TWITTER_OAUTH2_ACCESS_TOKEN and TWITTER_OAUTH2_REFRESH_TOKEN in Vercel as backup seeds.',
    });
  } catch (err) {
