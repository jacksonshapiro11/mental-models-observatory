...
export function Breadcrumbs({
  ...
  if (showHome && (items.length === 0 || items[0]?.href !== '/')) {
    breadcrumbItems = [{ label: 'Home', href: '/', current: false }, ...items];
  }
              ...
              {item.tier && showTierColors && (
                <Badge
                  ...
                  className={`text-xs flex-shrink-0 ${
                    item.tier === 1
                      ? 'bg-foundational-50 text-foundational-700 border-foundational-200'
                      : item.tier === 2
                        ? 'bg-practical-50 text-practical-700 border-practical-200'
