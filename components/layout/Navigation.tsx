...
export function Navigation({
  ...
  const textClass = variant === 'light' ? 'text-[#555555]' : 'text-[#888888]';
    ...
    <nav className={`h-[44px] ${bgClass} ${className}`}>
      <div className='h-full px-4 sm:px-6 lg:px-8 flex items-center justify-between max-w-screen-2xl mx-auto'>
        ...
        >
          ...
          <span
            className={`font-mono text-xs font-bold hidden sm:block ${variant === 'light' ? 'text-ct-dark' : 'text-white'}`}
          >
            cosmic_trex
          </span>
        </Link>
        ...
        >
          {mobileMenuOpen ? (
            <X
              size={20}
              className={variant === 'light' ? 'text-ct-dark' : 'text-white'}
            />
          ) : (
            <Menu
              size={20}
      ...
      {mobileMenuOpen && (
        <div
          className={`md:hidden border-t ${variant === 'light' ? 'border-gray-200 bg-white' : 'border-gray-800 bg-black/50'}`}
        >
