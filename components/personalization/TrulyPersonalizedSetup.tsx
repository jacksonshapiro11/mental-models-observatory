...
export default function TrulyPersonalizedSetup({
  onComplete,
  onSkip,
}: TrulyPersonalizedSetupProps) {
  ...
  const getNextQuestion = () => {
    switch (step) {
      case 1:
        return {
          question:
            "What's a specific challenge you're facing right now that better thinking could help with?",
          placeholder:
            "e.g., 'I'm starting a business but struggle with making decisions under uncertainty' or 'I want to understand why people behave irrationally in my team'",
          followUp:
            'Be as specific as possible - this helps us find the most relevant mental models for your situation.',
        };
      ...
      case 2:
        return {
          ...
          placeholder:
            "e.g., 'I overthink everything' or 'I don't know how to weigh different factors' or 'I get overwhelmed by too many options'",
          followUp:
            'Understanding your thinking patterns helps us identify which frameworks will be most useful.',
        };

      case 3:
        return {
          question:
            "What's your learning style? How do you best absorb new concepts?",
          options: [
            {
              id: 'story-driven',
      ...
      case 5:
        return {
          ...
          followUp:
            'This helps us prioritize which mental models will have the biggest impact for you.',
        };
  ...
  const handleNext = () => {
    if (step < 5) {
      ...
      const personalizedProfile: UserProfile & { personalContext: any } = {
        ...
        experience: responses.thinkingPattern?.includes('overthink')
          ? 'some'
          : responses.thinkingPattern?.includes('overwhelmed')
            ? 'new'
            : 'experienced',
        interests: extractInterests(responses.challenge),
        goals:
          responses.challenge?.includes('business') ||
          responses.challenge?.includes('work')
            ? 'work'
            : responses.challenge?.includes('team') ||
                responses.challenge?.includes('people')
              ? 'teaching'
              : 'learning',
  ...
  const extractInterests = (challenge: string) => {
    const interests = [];
    if (
      challenge?.toLowerCase().includes('business') ||
      challenge?.toLowerCase().includes('startup')
    )
      interests.push('business');
    if (
      challenge?.toLowerCase().includes('decision') ||
      challenge?.toLowerCase().includes('choose')
    )
      interests.push('decisions');
    if (
      challenge?.toLowerCase().includes('team') ||
      challenge?.toLowerCase().includes('people')
    )
      interests.push('psychology');
    if (
      challenge?.toLowerCase().includes('creative') ||
      challenge?.toLowerCase().includes('innovation')
    )
      interests.push('creativity');
    if (
      challenge?.toLowerCase().includes('system') ||
      challenge?.toLowerCase().includes('complex')
    )
      interests.push('systems');
  ...
  const derivePrimaryFocus = (responses: any) => {
    ...
    if (
      responses.challenge?.includes('business') ||
      responses.challenge?.includes('startup')
    )
    ...
    if (
      responses.challenge?.includes('creative') ||
      responses.challenge?.includes('innovation')
    )
  ...
  const deriveRecommendedApproach = (responses: any) => {
    const approach = {
      contentStyle:
        responses.learningStyle === 'story-driven'
          ? 'example-heavy'
          : responses.learningStyle === 'principle-first'
            ? 'concept-focused'
            : responses.learningStyle === 'hands-on'
              ? 'application-oriented'
              : 'connection-based',

      pacing:
        responses.timeCommitment === 'micro-learning'
          ? 'bite-sized'
          : responses.timeCommitment === 'intensive-learning'
            ? 'comprehensive'
            : 'balanced',

      difficulty: responses.thinkingPattern?.includes('overthink')
        ? 'structured'
        : responses.thinkingPattern?.includes('overwhelmed')
          ? 'simplified'
          : 'challenging',
            ...
            {!currentQuestion.options && (
              <div className='space-y-4'>
                ...
                <p className='text-xs text-neutral-500'>
                  The more specific you are, the better I can personalize your
                  experience
                </p>
              </div>
            )}
            ...
            {currentQuestion.options && (
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                {currentQuestion.options.map((option: any) => {
                  ...
                  const isSelected =
                    responses[
                      step === 3 ? 'learningStyle' : 'timeCommitment'
                    ] === option.id;
