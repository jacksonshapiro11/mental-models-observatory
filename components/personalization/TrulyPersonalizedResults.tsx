...
} from '@/lib/dynamic-path-generator';
...
export default function TrulyPersonalizedResults({
  profile,
}: TrulyPersonalizedResultsProps) {
  ...
  const generateTrulyPersonalizedPaths = () => {
    try {
      ...
      const completed = progress.pathsStarted
      ...
      console.log(
        `Progress: ${viewed.length} viewed models, ${completed.length} completed paths`
      );

      // 1. Get 2-3 curated paths (always shown)
      const bestCuratedPaths = PathMatcher.getBestMatches(
        profile,
        viewed,
        completed
      );
      ...
      console.log(
        'Curated paths:',
        bestCuratedPaths.map(p => p.title)
      );
      ...
      if (hasCompletedEnough) {
        ...
        const curatedModelSets = bestCuratedPaths.map(
          p => new Set(p.modelSlugs)
        );
        const uniqueDynamic = dynamicPathsGenerated
          .filter(dynPath => {
            ...
            const hasOverlap = curatedModelSets.some(curatedSet => {
              const intersection = [...dynModels].filter(slug =>
                curatedSet.has(slug)
              ).length;
    ...
    const scoredModels = allModels.map(model => {
      ...
      if (context.specificChallenge) {
        ...
        if (challenge.includes('decision') || challenge.includes('choose')) {
          if (
            model.slug.includes('decision') ||
            model.slug.includes('probabilistic') ||
            model.slug.includes('bias')
          )
            score += 10;
        }
        ...
        if (challenge.includes('business') || challenge.includes('startup')) {
          if (
            model.slug.includes('competitive') ||
            model.slug.includes('strategy') ||
            model.slug.includes('market')
          )
            score += 10;
        }
        ...
        if (
          ...
          if (
            model.slug.includes('psychology') ||
            model.slug.includes('behavior') ||
            model.slug.includes('cooperation')
          )
            score += 10;
        }
        ...
        if (
          ...
          if (
            model.slug.includes('creative') ||
            model.slug.includes('innovation') ||
            model.slug.includes('design')
          )
            score += 10;
        }
      }
      ...
      if (context.thinkingPattern) {
        ...
        if (
          pattern.includes('overthink') &&
          model.slug.includes('decision-quality')
        )
          score += 8;
        if (pattern.includes('overwhelmed') && model.slug.includes('systems'))
          score += 8;
        if (pattern.includes('options') && model.slug.includes('probabilistic'))
          score += 8;
      }

      // Learning style bonus
      if (
        context.learningStyle === 'story-driven' &&
        model.examples &&
        model.examples.length > 0
      )
        score += 3;
      if (
        context.learningStyle === 'principle-first' &&
        model.principles &&
        model.principles.length > 2
      )
        score += 3;
  ...
  const findFoundationalModels = (allModels: any[], context: any) => {
    ...
    return allModels
      .filter(model => foundationalSlugs.includes(model.slug))
      .slice(0, 3);
  };
  ...
  const generatePersonalizedTitle = (context: any) => {
    if (context.specificChallenge) {
      ...
      if (challenge.includes('decision'))
        return 'Make Better Decisions in Your Situation';
      if (challenge.includes('business') || challenge.includes('startup'))
        return 'Navigate Your Business Challenge';
      if (challenge.includes('team') || challenge.includes('people'))
        return 'Understand & Influence People Better';
      if (challenge.includes('creative') || challenge.includes('innovation'))
        return 'Unlock Your Creative Problem-Solving';
    }
  ...
  const generateFoundationDescription = (context: any) => {
    const focus =
      context.primaryFocus === 'decision-making'
        ? 'decision-making'
        : context.primaryFocus === 'business-strategy'
          ? 'strategic thinking'
          : context.primaryFocus === 'human-psychology'
  ...
  const calculatePersonalizedTime = (timeData: any, multiplier: number = 1) => {
    ...
    const baseTime =
      timeData.time === '5min' ? 8 : timeData.time === '15min' ? 18 : 35;
      ...
      <div className='min-h-screen bg-gradient-to-br from-foundational-50 via-neutral-25 to-accent-50 dark:from-[var(--espresso-bg-dark)] dark:via-[var(--espresso-bg-medium)] dark:to-[var(--espresso-bg-light)] flex items-center justify-center'>
        <div className='text-center'>
          ...
          <div className='space-y-2'>
            <p className='text-lg font-medium text-neutral-700 dark:text-[var(--espresso-h1)]'>
              Analyzing your specific situation...
            </p>
            <p className='text-sm text-neutral-600 dark:text-[var(--espresso-body)]'>
              Finding the mental models that will help you most
            </p>
          </div>
        </div>
      </div>
      ...
      <div className='container mx-auto px-4 py-8'>
        ...
        <div className='text-center mb-12'>
          ...
          <h1 className='text-4xl md:text-5xl font-bold text-neutral-800 dark:text-[var(--espresso-h1)] mb-6'>
            ...
            <span className='gradient-text'>
              {profile.personalContext.specificChallenge
                ? 'Solve This'
                : 'Think Better'}
            </span>
          </h1>
        ...
        {viewedModelSlugs.length > 0 && (
          <div className='bg-gradient-to-r from-green-50 to-emerald-50 dark:!bg-transparent rounded-xl p-6 mb-8 border-2 border-green-200 dark:border-[var(--espresso-accent)]/30'>
            <div className='flex items-center justify-between'>
              <div>
                <h3 className='text-xl font-bold text-green-800 dark:text-[var(--espresso-accent)] mb-2'>
                  Your Learning Progress
                </h3>
                <p className='text-green-700 dark:text-[var(--espresso-body)]'>
                  You've explored{' '}
                  <span className='font-bold text-2xl dark:text-[var(--espresso-accent)]'>
                    {viewedModelSlugs.length}
                  </span>{' '}
                  mental models so far!
                </p>
              </div>
              <div className='text-right'>
                <div className='text-3xl font-bold text-green-600 dark:text-[var(--espresso-accent)]'>
                  {completedPathIds.length}
                </div>
                <div className='text-sm text-green-700 dark:text-[var(--espresso-body)]'>
                  Paths Completed
                </div>
              </div>
            </div>
        ...
        <div className='mb-12'>
          <div className='flex items-center justify-center gap-3 mb-4'>
            ...
            <h2 className='text-3xl font-bold text-neutral-800 dark:text-[var(--espresso-h1)]'>
              Recommended For You
            </h2>
            <Star className='h-8 w-8 text-yellow-500 dark:text-[var(--espresso-accent)]' />
          </div>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6'>
            {curatedPaths.map((path, index) => {
              const viewedCount = path.modelSlugs.filter(slug =>
                viewedModelSlugs.includes(slug)
              ).length;
              ...
              return (
                <div
                  key={path.id}
                  className={`bg-white dark:bg-transparent rounded-xl shadow-lg dark:shadow-none overflow-hidden hover:shadow-xl dark:hover:shadow-none transition-shadow border border-neutral-200 dark:border-[var(--espresso-accent)]/25 ${isCompleted ? 'ring-4 ring-green-400 dark:ring-[var(--espresso-accent)]' : ''}`}
                >
                  {/* Completed Badge */}
                  {isCompleted && (
                  ...
                  {/* Path Header */}
                  <div
                    className={`p-6 ${getPathColor(path.category)} text-white`}
                  >
                        ...
                        className={`text-xs font-medium px-2 py-1 rounded-full backdrop-blur-sm ${
                          path.level === 'beginner'
                            ...
                            : path.level === 'intermediate'
                              ? 'bg-[var(--espresso-accent)]/40 text-white'
                              : path.level === 'advanced'
                    ...
                    <div className='space-y-2 mb-6 max-h-64 overflow-y-auto'>
                      {path.modelSlugs.slice(0, 5).map((slug, modelIndex) => {
                        ...
                        return (
                          <div
                            key={modelIndex}
                            className={`flex items-center space-x-3 p-2 rounded-lg transition-all ${isViewed ? 'bg-green-50 dark:bg-transparent border border-green-200 dark:border-[var(--espresso-accent)]/40' : 'bg-neutral-50 dark:bg-transparent hover:bg-neutral-100 dark:hover:bg-[var(--espresso-accent)]/10 border border-transparent dark:border-[var(--espresso-accent)]/20'}`}
                          >
                            <div
                              className={`h-6 w-6 rounded-full flex items-center justify-center font-semibold text-xs flex-shrink-0 ${isViewed ? 'bg-green-500 dark:bg-[var(--espresso-accent)] text-white dark:text-[var(--espresso-cta-text)]' : 'bg-foundational-100 dark:bg-[var(--espresso-accent)]/20 text-foundational-600 dark:text-[var(--espresso-accent)]'}`}
                            >
                              {isViewed ? (
                            ...
                            <div className='flex-1 min-w-0'>
                              <p
                                className={`font-medium text-sm truncate ${isViewed ? 'text-green-800 dark:text-[var(--espresso-body)] line-through' : 'text-neutral-800 dark:text-[var(--espresso-h1)]'}`}
                              >
                                {model?.name || slug}
                                {isViewed && (
                                  <span className='ml-2 text-xs text-green-600 dark:text-[var(--espresso-accent)] font-bold no-underline'>
                                    ✓ DONE
                                  </span>
                                )}
                              </p>
                      ...
                      {path.modelSlugs.length > 5 && (
                        <p className='text-xs text-neutral-500 dark:text-[var(--espresso-body)]/70 text-center'>
                          +{path.modelSlugs.length - 5} more models
                        </p>
                      )}
                    </div>
                    ...
                    {viewedCount > 0 && (
                      <div className='mb-4'>
                        <div className='flex items-center justify-between text-sm mb-2'>
                          <span className='text-neutral-600 dark:text-[var(--espresso-body)]'>
                            Your Progress:
                          </span>
                          <span className='font-medium text-green-600 dark:text-[var(--espresso-accent)]'>
                            {viewedCount}/{totalCount} models
                          </span>
                        </div>
        ...
        {showDynamic && dynamicPaths.length > 0 && (
          <div className='mb-12'>
            ...
            <p className='text-center text-neutral-600 dark:text-[var(--espresso-body)] mb-2'>
              🎉 Unlocked! {dynamicPaths.length} dynamic paths generated based
              on your progress
            </p>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6'>
              {dynamicPaths.map((path, index) => {
                const viewedCount = path.models.filter(m =>
                  viewedModelSlugs.includes(m.model.slug)
                ).length;
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`p-6 ${getPathColor(path.pathType)} text-white`}
                    >
                      <div className='flex items-center justify-between mb-2'>
                        <h3 className='text-lg font-bold'>{path.title}</h3>
                        <span
                          className={`text-xs font-medium px-2 py-1 rounded-full backdrop-blur-sm ${path.difficulty === 'gentle' ? 'bg-green-500/30 text-white' : path.difficulty === 'moderate' ? 'bg-[var(--espresso-accent)]/40 text-white' : path.difficulty === 'challenging' ? 'bg-red-500/30 text-white' : 'bg-white/20 text-white'}`}
                        >
                          {path.difficulty}
                        </span>
                      </div>
                    ...
                    <div className='p-4 bg-foundational-50 dark:bg-transparent border-b dark:border-[var(--espresso-accent)]/25'>
                      <div className='flex items-start space-x-3'>
                        ...
                        <div>
                          <p className='text-sm font-medium text-foundational-800 dark:text-[var(--espresso-h1)] mb-1'>
                            Why this path?
                          </p>
                          <p className='text-sm text-foundational-700 dark:text-[var(--espresso-body)]'>
                    ...
                    <div className='p-6'>
                      ...
                      <div className='space-y-2 mb-6 max-h-64 overflow-y-auto'>
                        {path.models
                          .slice(0, 5)
                          .map((pathModel, modelIndex) => {
                            const isViewed = viewedModelSlugs.includes(
                              pathModel.model.slug
                            );
                            return (
                              <div
                                key={modelIndex}
                        ...
                        {path.models.length > 5 && (
                          <p className='text-xs text-neutral-500 dark:text-[var(--espresso-body)]/70 text-center'>
                            +{path.models.length - 5} more models
                          </p>
                        )}
                      </div>
        ...
        <div className='bg-white rounded-xl shadow-lg p-8 mb-8'>
          ...
          <div className='bg-green-50 rounded-lg p-4'>
            ...
            <p className='text-green-700'>
              {profile.personalContext.successDefinition ||
                'You can apply these mental models to solve similar challenges with confidence.'}
            </p>
          </div>
        </div>
