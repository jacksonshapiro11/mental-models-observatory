...
function TierBadge({ tier }: { tier: string }) {
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 rounded text-xs font-semibold ${colors[tier] || 'bg-gray-500 text-white'}`}
    >
      {tier}
    </span>
  );
    ...
    <div className='border border-neutral-200 dark:border-[var(--espresso-accent)]/20 rounded-lg p-5 hover:border-neutral-300 dark:hover:border-[var(--espresso-accent)]/40 transition-colors bg-white dark:bg-[var(--espresso-bg-medium)]'>
      <div className='flex items-start justify-between mb-3'>
        <div>
          ...
          <p className='text-sm text-neutral-500 dark:text-[var(--espresso-body)]/70'>
            {position.name}
          </p>
        </div>
      ...
      {position.mispricingMarket && (
        <div className='mb-3'>
          <p className='text-xs font-medium uppercase tracking-wide text-neutral-400 dark:text-[var(--espresso-body)]/50 mb-1'>
            Mispricing
          </p>
        ...
        <div className='mb-3'>
          ...
          <ul className='space-y-1'>
            {position.killCriteria.slice(0, 3).map((k: string, i: number) => (
              <li
                key={i}
                className='text-sm text-neutral-600 dark:text-[var(--espresso-body)]/70 flex items-start gap-1.5'
              >
            ...
            {position.keyAssumptions.map(
              (
                ...
                <div key={i} className='flex items-center gap-2 text-sm'>
                  ...
                  <span className='text-neutral-600 dark:text-[var(--espresso-body)]/70 truncate'>
                    {a.text}
                  </span>
                </div>
              )
  ...
  return (
    <section className='mb-10'>
      <div className='flex items-baseline justify-between mb-4'>
        <div>
          <h2 className='text-xl font-bold text-neutral-800 dark:text-[var(--espresso-h1)]'>
            {title}
          </h2>
          <p className='text-sm text-neutral-500 dark:text-[var(--espresso-body)]/60'>
            {description}
          </p>
        </div>
        <span className='text-sm font-medium text-neutral-400 dark:text-[var(--espresso-body)]/50'>
          {weight}
        </span>
      </div>
    ...
    <>
      <div className='flex items-center justify-between mb-8'>
        <div>
          <div className='flex items-center gap-3 mb-2'>
            <h1 className='text-2xl sm:text-3xl font-bold text-neutral-800 dark:text-[var(--espresso-h1)]'>
              Portfolio Tracker
            </h1>
          ...
          <p className='text-neutral-500 dark:text-[var(--espresso-body)]/70 text-sm max-w-2xl'>
            Structural thesis positions applying our frameworks to specific
            assets. Not investment advice — a testing ground for our mental
            models against real markets.
          </p>
          <div className='flex gap-6 mt-4 text-sm'>
            <div>
              <span className='text-neutral-400 dark:text-[var(--espresso-body)]/50'>
                Positions
              </span>
              <span className='ml-1.5 font-semibold text-neutral-800 dark:text-[var(--espresso-h1)]'>
                {positions.length}
              </span>
            </div>
            <div>
      ...
      <div className='mt-8 p-4 border border-neutral-200 dark:border-[var(--espresso-accent)]/15 rounded-lg bg-neutral-50 dark:bg-[var(--espresso-bg-light)]/30'>
        <p className='text-xs text-neutral-400 dark:text-[var(--espresso-body)]/50 italic'>
          This portfolio is purely illustrative — not investment advice. It
          applies our structural theses to specific assets to test frameworks
          against real markets. The tiered structure reflects conviction levels,
          not recommended allocations. Do not invest in anything because it
          appears here.
        </p>
      </div>
    </>
