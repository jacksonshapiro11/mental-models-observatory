...
export function BasketPriceBadge({
  prices,
}: {
  prices: Record<string, PriceData> | undefined;
}) {
  ...
  const entries = BASKET_TICKERS.map(t => ({
    ticker: t,
    data: prices[t],
  })).filter(e => e.data);
      ...
      {entries.map(({ ticker, data }) => {
        ...
        return (
          <div key={ticker} className='flex items-center gap-2 text-sm'>
            <span className='text-xs text-neutral-400 dark:text-neutral-500 w-8'>
              {ticker}
            </span>
            <span className='font-mono font-semibold text-neutral-800 dark:text-[var(--espresso-h1)]'>
              $
