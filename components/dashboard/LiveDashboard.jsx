...
function PriceRow({ asset, data, color, priceOpts = {} }) {
  ...
  return (
    <tr>
      ...
      <td style={styles.td}>{formatPrice(price, priceOpts)}</td>
...
export default function LiveDashboard({
  analysisTimestamp = /** @type {string|null} */ (null),
  contextNotes = /** @type {string|null} */ (null),
  equityNotes = /** @type {string|null} */ (null),
  cryptoNotes = /** @type {string|null} */ (null),
  commodityNotes = /** @type {string|null} */ (null),
}) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const nextMs =
          json.meta?.pollIntervalMs ??
          getDashboardPollIntervalMs(json.meta?.marketStatus);
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <div style={styles.tabBar}>
          <button
            style={styles.tab(activeTab === 'equities')}
            onClick={() => setActiveTab('equities')}
          >
            Equities
          </button>
          <button
            style={styles.tab(activeTab === 'crypto')}
            onClick={() => setActiveTab('crypto')}
          >
            Crypto
          </button>
          <button
            style={styles.tab(activeTab === 'commodities')}
        ...
        <div style={styles.freshness}>
          ...
          <span>
            {status === 'live' ? 'LIVE' : status === 'stale' ? 'STALE' : 'OFF'}
          </span>
      ...
      {error && (
        <div style={styles.errorBanner}>⚠ Data may be delayed: {error}</div>
      )}
      ...
      {activeTab === 'equities' && (
        <>
                  ...
                  <tbody>
                    {Object.entries(data.futures).map(([name, info]) => (
                      <tr key={name}>
                        ...
                        <td style={styles.td}>
                          {formatPrice(info.price, { prefix: '', decimals: 0 })}
                        </td>
                        <td
                          style={{
                            ...styles.td,
                            ...styles.change(info.change),
                          }}
                        >
          ...
          {equityNotes && <div style={styles.contextNote}>{equityNotes}</div>}
        </>
      )}
      ...
      {activeTab === 'crypto' && (
        <>
          ...
          {cryptoNotes && <div style={styles.contextNote}>{cryptoNotes}</div>}
            ...
            <div style={styles.metricsRow}>
              <div style={styles.metric}>
                ...
                <span style={styles.metricValue}>
                  {meta.cryptoMeta.btcDominance?.toFixed(1)}%
                </span>
              </div>
              <div style={styles.metric}>
                <span style={styles.metricLabel}>ETH Dom</span>
                <span style={styles.metricValue}>
                  {meta.cryptoMeta.ethDominance?.toFixed(1)}%
                </span>
              </div>
              <div style={styles.metric}>
                ...
                <span style={styles.metricValue}>
                  $
                  {meta.cryptoMeta.totalMarketCap
                    ? (meta.cryptoMeta.totalMarketCap / 1e12).toFixed(2) + 'T'
                    : '—'}
                </span>
              </div>
              <div style={styles.metric}>
                ...
                <span
                  style={{
                    ...styles.metricValue,
                    color:
                      (meta.cryptoMeta.fearGreed?.value || 50) < 25
                        ? CT.pink
                        : (meta.cryptoMeta.fearGreed?.value || 50) < 45
                          ? CT.yellow
                          : (meta.cryptoMeta.fearGreed?.value || 50) < 55
                            ? CT.textMuted
                            : (meta.cryptoMeta.fearGreed?.value || 50) < 75
                              ? CT.greenData
                              : CT.greenData,
                  }}
                >
                  {meta.cryptoMeta.fearGreed?.value ?? '—'} (
                  {meta.cryptoMeta.fearGreed?.label ?? '—'})
                </span>
              </div>
              ...
              <div style={styles.metricsRow}>
                ...
                {meta.cryptoMeta.defiDominance != null && (
                  <div style={styles.metric}>
                    ...
                    <span style={styles.metricValue}>
                      {meta.cryptoMeta.defiDominance.toFixed(1)}%
                    </span>
                  </div>
                )}
                {meta.cryptoMeta.defiToEthRatio != null && (
                  <div style={styles.metric}>
                    ...
                    <span style={styles.metricValue}>
                      {meta.cryptoMeta.defiToEthRatio.toFixed(2)}%
                    </span>
                  </div>
                )}
                {meta.cryptoMeta.btcFundingRate != null && (
                  <div style={styles.metric}>
                    ...
                    <span
                      style={{
                        ...styles.metricValue,
                        color:
                          meta.cryptoMeta.btcFundingRate > 0.01
                            ? CT.greenData
                            : meta.cryptoMeta.btcFundingRate < -0.01
                              ? CT.pink
                              : CT.textMuted,
                    ...
                    >
                      {meta.cryptoMeta.btcFundingRate > 0 ? '+' : ''}
                      {(meta.cryptoMeta.btcFundingRate * 100).toFixed(4)}%
                    </span>
                  </div>
                )}
                ...
                {meta.cryptoMeta.corporateBtcHoldings != null && (
                  <div style={styles.metric}>
                    ...
                    <span style={styles.metricValue}>
                      {(meta.cryptoMeta.corporateBtcHoldings / 1000).toFixed(0)}
                      K BTC
                    </span>
                  </div>
                )}
            ...
            >
              ...
              >
                {meta.cryptoMeta.trending.map((t, i) => (
                  <span key={i}>
                    <span style={{ color: CT.textPrimary, fontWeight: 500 }}>
                      {t.symbol}
                    </span>
      ...
      {activeTab === 'commodities' && (
        <>
          ...
          >
            Prices reflect front-month futures contracts and may diverge from
            spot prices.
          </div>
          <ScrollableTable>
            <table style={styles.table}>
              ...
              <tbody>
                ...
                <PriceRow
                  asset='Copper'
                  data={commodities.COPPER}
                  color={CT.yellow}
                />
                <PriceRow
                  asset='Nat Gas'
                  data={commodities.NATGAS}
                  color={CT.greenDisc}
                />
                <PriceRow
                  asset='10Y'
                ...
                <div style={styles.metric}>
                  ...
                  <span style={styles.metricValue}>
                    ...
                    {meta.dxy.yoyChange != null && (
                      <span
                        style={{
                          ...styles.change(meta.dxy.yoyChange),
                          fontSize: '11px',
                          marginLeft: '6px',
                        }}
                      >
