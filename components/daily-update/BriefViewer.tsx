...
> = {
  ...
  watching: {
    bg: 'bg-ct-dark',
    text: 'text-ct-green-data',
    border: 'border-ct-green-data',
  },
  building: {
    bg: 'bg-ct-dark',
    text: 'text-ct-green-data',
...
function inferStatus(title: string): string {
  ...
  if (
    t.includes('accelerating') ||
    t.includes('deepening') ||
    t.includes('surging')
  )
    return 'accelerating';
  if (
  ...
  if (
    t.includes('building') ||
    t.includes('industrialization') ||
    t.includes('renaissance')
  )
    return 'building';
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-block text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded border font-mono ${s.bg} ${s.text} ${s.border}`}
    >
      {status}
    </span>
  );
      ...
      if (linkHref.startsWith('#') && onAnchorClick) {
        ...
        parts.push(
          <a
            ...
            className='text-ct-pink hover:text-ct-yellow underline cursor-pointer'
          >
            {linkText}
          </a>
        );
      ...
      <table className='w-full text-sm border-collapse min-w-[600px] font-mono'>
        <thead>
          <tr className='bg-surface-warm border-b border-text-muted/20'>
            {headers.map((h, i) => (
              <th
                key={i}
                className='text-left py-3 px-3 text-xs font-semibold uppercase tracking-wider text-text-secondary'
              >
                {h}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row, ri) => (
            <tr
              key={ri}
              className='border-b border-text-muted/10 hover:bg-surface-warm/50 transition-colors'
            >
              {row.map((cell, ci) => {
...
function parseBlocks(content: string): { type: string; content: string }[] {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('- ')) {
      ...
      blocks.push({ type: 'list', content: listContent.trim() });
      continue;
    }
    ...
    if (/^\d+\.\s/.test(line)) {
      ...
      blocks.push({ type: 'numbered-list', content: listContent.trim() });
      continue;
    }
    ...
    if (line.startsWith('*') && line.endsWith('*') && !line.startsWith('**')) {
      blocks.push({ type: 'italic', content: line.slice(1, -1) });
      i++;
      continue;
    }
    ...
    while (
      i < lines.length &&
      ln(lines, i).trim() &&
      !ln(lines, i).trim().startsWith('|') &&
      !ln(lines, i).trim().startsWith('#') &&
      !ln(lines, i).trim().startsWith('- ') &&
      !/^\d+\.\s/.test(ln(lines, i).trim()) &&
      ln(lines, i).trim() !== '---' &&
      !ln(lines, i).trim().startsWith('<!--')
    ) {
      para += '\n' + ln(lines, i).trim();
      ...
      {blocks.map((block, i) => {
        if (block.type === 'h3')
          return (
            <h3
              key={i}
              className='text-[10px] font-bold text-ct-yellow uppercase tracking-[0.06em] mt-10 first:mt-0 flex items-center gap-2 font-mono'
            >
        ...
        if (block.type === 'italic')
          return (
            <p
              key={i}
              className='text-sm text-text-secondary leading-[1.6] italic pl-3 border-l-[3px] border-ct-yellow'
            >
        ...
        return (
          <p
            key={i}
            className='text-sm text-text-secondary font-mono leading-[1.6]'
          >
            <RichText text={block.content} />
          </p>
        );
...
const SIX_SUBSECTION_STYLES: Record<
  string,
  {
    bg: string;
    border: string;
...
> = {
  markets: {
    bg: 'bg-white',
    border: 'border-t-[3px] border-ct-dark',
    tag: 'text-ct-dark',
    hlColor: 'text-text-primary',
    bodyColor: 'text-text-secondary',
    divider: 'border-[#f0f0ec]',
    listBorder: 'border-text-muted/20',
  },
  crypto: {
    bg: 'bg-ct-dark',
    border: 'border-t-2 border-ct-yellow',
    tag: 'text-ct-yellow',
    hlColor: 'text-[#fff]',
    bodyColor: 'text-[#aaa]',
    divider: 'border-[#333]',
    listBorder: 'border-[#333]',
  },
  ai: {
    bg: 'bg-ct-yellow',
    border: '',
    borderStyle: {
      ...
      borderBottom: '3px solid #00885a',
    },
    tag: 'text-ct-dark',
    hlColor: 'text-ct-dark',
    bodyColor: 'text-[#444]',
    divider: 'border-black/10',
    listBorder: 'border-black/15',
  },
  geopolitics: {
    bg: 'bg-white',
    border: 'border-t border-[#e8e8e4]',
    tag: 'text-ct-dark',
    hlColor: 'text-text-primary',
    bodyColor: 'text-text-secondary',
    divider: 'border-[#f0f0ec]',
  ...
  wildcard: {
    bg: 'bg-[#F8F8F4]',
    border: 'border-t-2 border-ct-pink',
    tag: 'text-ct-pink',
    hlColor: 'text-text-primary',
    bodyColor: 'text-text-secondary',
    divider: 'border-[#e8e8e4]',
    listBorder: 'border-text-muted/20',
  },
  signal: {
    bg: 'bg-ct-dark',
    border: 'border-t-2 border-ct-green-data',
    tag: 'text-ct-green-data',
    hlColor: 'text-[#fff]',
    bodyColor: 'text-[#aaa]',
    divider: 'border-[#333]',
    listBorder: 'border-[#333]',
  ...
  return (
    <div className='relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] w-screen'>
      {subsections.map((sub, si) => {
        ...
        const s =
          SIX_SUBSECTION_STYLES[styleKey] || SIX_SUBSECTION_STYLES['markets']!;

        return (
          <div
            key={si}
            className={`${s.bg} ${s.border} px-4 py-4`}
            style={s.borderStyle}
          >
            <div className='max-w-5xl mx-auto'>
              {/* Subsection heading */}
              <div
                className={`text-sm sm:text-base tracking-[0.06em] uppercase font-bold font-mono mb-4 ${s.tag}`}
              >
                {sub.heading}
              </div>
                ...
                {sub.blocks.map((block, bi) => {
                    ...
                    return (
                      <div key={bi} className='space-y-0'>
                        {items.map((item, j) => {
                          ...
                          const hlMatch = stripped.match(
                            /^\*\*(.+?)\*\*\s*([\s\S]*)/
                          );
                          return (
                            <div
                              key={j}
                              className={`py-3 border-b ${s.divider} last:border-b-0`}
                            >
                              {hlMatch ? (
                                <p
                                  className={`text-[13px] leading-[1.65] ${s.bodyColor}`}
                                >
                                  <strong
                                    className={`font-semibold ${s.hlColor}`}
                                  >
                                    {hlMatch[1]}
                                  </strong>{' '}
                  ...
                  if (block.type === 'italic')
                    return (
                      <p
                        key={bi}
                        className={`text-sm leading-[1.6] italic ${styleKey === 'crypto' ? 'text-[#666]' : 'text-text-muted'}`}
                      >
                        <RichText text={block.content} />
                      </p>
                    );
                  if (block.type === 'paragraph')
                    return (
                      <p
                        key={bi}
                        className={`text-sm leading-[1.6] ${s.bodyColor}`}
                      >
                        <RichText text={block.content} />
                      </p>
                    );
      ...
      {blocks.map((block, i) => {
        if (block.type === 'h3')
          return (
            <h3
              key={i}
              className='text-xl font-serif font-bold text-text-primary mb-3 leading-tight'
            >
              {block.content}
            </h3>
          );
        if (block.type === 'italic')
          return (
            <div
              key={i}
              className='bg-[#F8F4E0] border-l-[3px] border-ct-yellow px-4 py-3 rounded-sm'
            >
...
function InnerGameSection({ content }: { content: string }) {
  ...
  const elements: {
    type: 'body' | 'quote' | 'attribution' | 'action';
    content: string;
  }[] = [];
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (
      ...
      if (
        next &&
        (next.type === 'paragraph' || next.type === 'italic') &&
    ...
    if (
      ...
      /^\*\*Today's (action|practice):/.test(text)
    ) {
      ...
      {elements.map((el, i) => {
        if (el.type === 'quote')
          return (
            <blockquote
              key={i}
              className='font-serif text-xl font-semibold italic leading-snug text-text-primary text-center px-4 py-4'
            >
              <RichText text={el.content} />
            </blockquote>
          );
        if (el.type === 'attribution')
          return (
            <p
              key={i}
              className='text-sm text-ct-pink text-center font-medium -mt-4'
            >
              <RichText text={el.content} />
            </p>
          );
        if (el.type === 'action')
          return (
            <div
              key={i}
              className='border border-ct-pink px-4 py-3 mt-4 text-left'
            >
        ...
        return (
          <p
            key={i}
            className='text-sm text-text-secondary leading-[1.7] text-center'
          >
            <RichText text={el.content} />
          </p>
        );
...
function DiscoverySectionRenderer({ content }: { content: string }) {
  ...
  const elements: {
    type: 'title' | 'body' | 'callout' | 'source';
    content: string;
  }[] = [];
      ...
      {elements.map((el, i) => {
        if (el.type === 'title')
          return (
            <h3
              key={i}
              className='text-xl font-serif font-bold text-text-primary leading-tight'
            >
              {el.content}
            </h3>
          );
        if (el.type === 'callout')
          return (
            <div
              key={i}
              className='bg-[#00885a]/10 border border-[#00885a]/20 px-4 py-3 rounded-sm'
            >
              <p className='text-sm leading-[1.65] text-text-secondary'>
                <strong className='text-[#00885a] font-semibold'>
                  Decision tool:
                </strong>{' '}
                <RichText
                  text={el.content.replace(/^Decision tool:\s*/i, '')}
                />
              </p>
            </div>
          );
...
function TheModelSection({
  ...
  const calloutBg = darkBg
    ? 'bg-ct-yellow/10 border-ct-yellow/20'
    : 'bg-ct-yellow/10 border-ct-yellow/20';
  ...
  const elements: {
    type: 'title' | 'body' | 'callout' | 'link' | 'italic';
    content: string;
  }[] = [];
      ...
      {elements.map((el, i) => {
        if (el.type === 'title')
          return (
            <h3
              key={i}
              className={`text-xl font-serif font-bold ${headColor} leading-tight`}
            >
              {el.content}
            </h3>
          );
        ...
        if (el.type === 'link')
          return (
            <p key={i}>
              <RichText
                text={el.content}
                className={`text-sm font-mono ${linkColor} underline`}
              />
            </p>
          );
        if (el.type === 'italic')
      ...
      {blocks.map((block, i) => {
        ...
        if (labelMatch) {
          return (
            <div
              key={i}
              className='py-2 border-b border-text-muted/10 last:border-0'
            >
              <p className='text-sm text-text-secondary leading-[1.65]'>
...
function BigStoriesSection({ content }: { content: string }) {
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'h3') {
      if (currentTitle)
        stories.push({ title: currentTitle, body: currentBody.trim() });
      currentTitle = block.content;
      currentBody = '';
    } else if (
      block.type === 'italic' &&
      block.content.startsWith('Remaining Big Stories')
    ) {
      if (currentTitle)
        stories.push({ title: currentTitle, body: currentBody.trim() });
      currentTitle = '';
  ...
  return (
    <div className='space-y-4'>
      {stories.map((story, i) => {
        if (story.title === '__remaining__')
          return (
            <p
              key={i}
              className='text-xs text-text-muted italic mt-6 pt-4 border-t-[0.5px] border-text-muted/20'
            >
        ...
        return (
          <div
            key={i}
            className='p-4 bg-white border-[0.5px] border-text-muted/20'
          >
            <div className='flex items-start justify-between gap-3 mb-3'>
              <h4 className='text-base font-bold text-text-primary leading-snug'>
                {num && (
                  <span className='text-ct-yellow font-mono mr-1.5'>
                    {num}.
                  </span>
                )}
                {cleanTitle}
            ...
            <div className='space-y-2'>
              {story.body
                ...
                .map((p, j) => (
                  <p
                    key={j}
                    className='text-sm text-text-secondary leading-[1.6]'
                  >
                    <RichText text={p} />
                  </p>
                ))}
...
function TomorrowsHeadlinesSection({ content }: { content: string }) {
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'h3') {
      ...
      block.type === 'italic' &&
      block.content.includes('Full reference list')
    ) {
      if (currentTitle)
        items.push({ title: currentTitle, body: currentBody.trim() });
      currentTitle = '';
...
function WatchlistSection({ content }: { content: string }) {
  ...
  const labelPattern =
    /^(?:\*{1,2})(Framework error|Data signal|Signal|Upside|Downside|Upside\/downside|Validates|Rejects)(?::?\*{1,2})/i;
  ...
  function splitLabels(line: string): string[] {
    ...
    const splitRegex =
      /\s+(?=\*{1,2}(?:Framework error|Data signal|Signal|Upside|Downside|Upside\/downside|Validates|Rejects):?\*{0,2})/i;
        ...
        <div className='space-y-3'>
          ...
          {updateItems.map((item, i) => {
            ...
            return (
              <div
                key={`update-${i}`}
                className='p-4 bg-surface-warm border-[0.5px] border-text-muted/20'
              >
                {lines.map((line, j) => (
                  <p
                    key={j}
          ...
          >
            {lines.map((line, j) => {
              ...
              const labelType = labelMatch
                ...
                : null;
              const isUpside =
                labelType === 'upside' || labelType === 'upside/downside';
              const isDownside = labelType === 'downside';
              const isValidates = labelType === 'validates';
              const isRejects = labelType === 'rejects';
              const isSignal =
                labelType === 'signal' || labelType === 'data signal';
              const isDateNote =
                /^\*(?:Feb|Mar|Jan|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s/.test(
                  line
                );
              const isTitle = j === 0;
              ...
              return (
                <p
                  ...
                  className={`leading-[1.6] mb-2 ${
                    isTitle
                      ? 'text-base font-bold text-text-primary mb-3'
                      : isUpside
                        ? 'text-sm text-ct-green-data'
                        : isDownside
                          ? 'text-sm text-ct-pink'
                          : isValidates
                            ...
                            : isRejects
                              ...
                              : isSignal
                                ? 'text-sm text-ct-pink'
                                : isDateNote
                                  ? 'text-xs text-ct-yellow italic mt-2 font-mono'
      ...
      if (titleMatch) {
        ...
        if (descMatch) {
          ...
          if (recommendations.length > 0) {
            recommendations[recommendations.length - 1]!.timeEstimate =
              timeEstimate;
          }
        } else {
          ...
          if (descBlock.type === 'paragraph') {
            ...
            if (i < blocks.length) {
              ...
              if (linkBlock.type === 'paragraph') {
                const linkMatch = linkBlock.content.match(
                  /\[→\s*(.+?)\]\((.+?)\)/
                );
                if (linkMatch) {
                  url = linkMatch[2] ?? '';
      ...
      {blocks.map((block, i) => {
        if (block.type === 'h2')
          return (
            <h3
              key={i}
              className={`text-base font-bold ${h2Color} mt-6 first:mt-0`}
            >
              {block.content}
            </h3>
          );
        if (block.type === 'h3')
          return (
            <h4 key={i} className={`text-sm font-semibold ${h3Color} mt-4`}>
              {block.content}
            </h4>
          );
        if (block.type === 'table')
        ...
        if (block.type === 'italic')
          return (
            <p
              key={i}
              className={`text-sm ${italicColor} italic leading-[1.6]`}
            >
              <RichText text={block.content} />
            </p>
          );
            ...
            <div key={i} className='space-y-0'>
              {items.map((item, j) => {
                ...
                return (
                  <div
                    key={j}
                    className={`py-3 border-b ${listBorder} last:border-b-0`}
                  >
                    {hlMatch ? (
                      <p className={`text-sm ${bodyColor} leading-[1.65]`}>
                        <strong className={`font-semibold ${h2Color}`}>
                          {hlMatch[1]}
                        </strong>{' '}
                        <RichText text={hlMatch[2] ?? ''} />
                      </p>
              ...
              {items.map((item, j) => {
                ...
                return (
                  <div key={j} className='flex gap-3 items-start'>
                    <span
                      className={`text-sm font-bold ${numColor} font-mono min-w-[24px]`}
                    >
                      {match[1] ?? ''}.
                    </span>
      ...
      {items.map((item, i) => (
        ...
        >
          <div className='flex items-baseline gap-2 mb-2'>
            <span className='text-sm font-bold text-ct-yellow font-mono'>
              {item.num}.
            </span>
          ...
          <p className='text-sm text-text-secondary leading-[1.6] mb-1'>
            {item.text}
          </p>
          {item.date && (
            <p className='text-xs text-text-muted italic'>{item.date}</p>
          )}
        </div>
      ))}
    </div>
...
function RefTomorrowsSection({ content }: { content: string }) {
  ...
  while ((match = itemRegex.exec(fullText)) !== null) {
    ...
    const evidence = evidenceMatch
      ? (evidenceMatch[0] ?? '').slice(1, -1)
      : undefined;
    const body =
      evidence && evidenceMatch
        ? text.slice(0, evidenceMatch.index).trim()
        : text;
    items.push({
      num: match[1] ?? '',
      title: match[2] ?? '',
    ...
    <div className='space-y-3'>
      {items.map((item, i) => (
        ...
        >
          ...
          {item.evidence && (
            <p className='text-xs text-ct-pink italic mt-1.5'>
              {item.evidence}
            </p>
          )}
        </div>
...
function SectionContent({
  ...
  switch (section.type) {
    case 'dashboard': {
      ...
      for (const block of blocks) {
        if (block.type === 'h3') {
          ...
          else if (h.includes('commodit') || h.includes('rate'))
            currentSection = 'commodity';
          else currentSection = '';
        } else if (
          (block.type === 'italic' || block.type === 'paragraph') &&
          currentSection
        ) {
      ...
      const equityNotes =
        (sectionNotes.equity ?? []).length > 0
          ? sectionNotes.equity!.join(' ')
          : undefined;
      const cryptoNotes =
        (sectionNotes.crypto ?? []).length > 0
          ? sectionNotes.crypto!.join(' ')
          : undefined;
      const commodityNotes =
        (sectionNotes.commodity ?? []).length > 0
          ? sectionNotes.commodity!.join(' ')
          : undefined;
    ...
    case 'overnight':
      return <GenericSection content={section.content} darkBg={darkBg} />;
    case 'the-six':
      return <TheSixSection content={section.content} />;
    // Weekly-only — bold-headline + prose paragraphs; rendered on a dark background.
    case 'the-signal':
      return <GenericSection content={section.content} darkBg={darkBg} />;
    case 'deep-read':
      return <DeepReadSection content={section.content} />;
    case 'the-take':
      return <TheTakeSection content={section.content} />;
    ...
    case 'inner-game':
      return <InnerGameSection content={section.content} />;
    case 'the-model':
      return <TheModelSection content={section.content} darkBg={darkBg} />;
    case 'discovery':
      return <DiscoverySectionRenderer content={section.content} />;
    case 'big-stories':
      return <BigStoriesSection content={section.content} />;
    case 'tomorrows-headlines':
      return <TomorrowsHeadlinesSection content={section.content} />;
    case 'watchlist':
      return <WatchlistSection content={section.content} />;
    case 'ref-big-stories':
      return <RefBigStoriesSection content={section.content} />;
    case 'ref-tomorrows':
      return <RefTomorrowsSection content={section.content} />;
    default:
      return <GenericSection content={section.content} darkBg={darkBg} />;
  }
}
...
export default function BriefViewer({ brief }: { brief: DailyBrief }) {
  ...
  const briefPath = isWeekly
    ? `/weekly/${brief.date}`
    : `/daily-update/${brief.date}`;
  ...
  const activeSectionIndex = brief.sections.findIndex(
    s => s.id === activeSection
  );

  return (
    <div ref={containerRef} className='min-h-screen bg-surface-reading'>
      ...
      <div>
          ...
          return (
            <>
              {brief.sections.map((section, idx) => {
                ...
                const borderClass =
                  SECTION_BORDER[section.id] || 'border-t border-[#e8e8e4]';
                const labelColor =
                  SECTION_LABEL_COLOR[section.id] || 'text-text-secondary';
                ...
                return (
                  <React.Fragment key={section.id}>
                    <div
                      className={`${bgClass} ${isTheSix ? 'px-0 py-0' : 'px-4 py-12'} ${borderClass}`}
                    >
                      <section
                        ref={el => {
                      ...
                      >
                        {/* Section label + vote */}
                        <div
                          className={`flex items-center justify-between mb-8 ${isTheSix ? 'max-w-5xl mx-auto px-4 pt-8' : ''} ${isCentered ? 'flex-col gap-4' : ''}`}
                        >
                          <div
                            className={`text-base sm:text-xl font-mono font-bold ${labelColor} uppercase tracking-[0.08em]`}
                          >
        ...
        <div className='bg-ct-dark px-4 py-8 border-t-[3px] border-ct-yellow text-center'>
          <p className='text-[12px] font-mono text-ct-green-data font-medium mb-1'>
            ✓ Fully caught up
          </p>
          <p className='text-[10px] font-mono text-text-on-dark-muted mb-4'>
            Edition {brief.date} ·{' '}
            <Link href='/archive' className='text-ct-pink hover:text-ct-yellow'>
              Archive
            </Link>
          </p>
          <div className='flex gap-4 justify-center text-[10px] font-medium font-mono'>
            {!isWeekly && (
              <Link className='text-ct-yellow' href='/archive'>
                Yesterday →
              </Link>
            )}
            <Link className='text-ct-pink' href='/archive'>
              Archive →
            </Link>
            <Link className='text-ct-green-data' href='/models'>
              Models →
            </Link>
          </div>
        </div>
