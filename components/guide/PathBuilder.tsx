...
export default function PathBuilder({
  ...
  const createPath = () => {
    ...
    const domains = [
      ...new Set(
        selectedModelsData.map(model => model?.domainSlug).filter(Boolean)
      ),
    ] as string[];
    const tags = [
      ...new Set(selectedModelsData.flatMap(model => model?.tags || [])),
    ];
    ...
    const newPath: LearningPath = {
      ...
      description:
        pathDescription ||
        `A custom learning path with ${selectedModels.length} mental models`,
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      case 'beginner':
        return 'bg-specialized-100 text-specialized-800 dark:bg-[#3d4a2a] dark:text-[#a8c97f]';
      case 'intermediate':
        return 'bg-accent-100 text-accent-800 dark:bg-[var(--espresso-accent)]/20 dark:text-[var(--espresso-accent)]';
      case 'advanced':
        return 'bg-practical-100 text-practical-800 dark:bg-[#5a2a2a] dark:text-[#d4a3a3]';
      default:
      ...
      <div className='bg-white rounded-xl shadow-xl w-full max-w-4xl h-[90vh] flex flex-col'>
          ...
          <div className='flex items-center space-x-3'>
            ...
            <div>
              ...
              <p className='text-sm text-neutral-600'>
                Create a custom learning journey
              </p>
            </div>
          </div>
          <button
        ...
        <div className='p-6 border-t border-neutral-200 flex items-center justify-between'>
          <div className='text-sm text-neutral-500'>
            {selectedModels.length} models selected • Estimated time:{' '}
            {selectedModels.length * 5}-{selectedModels.length * 5 + 5} minutes
          </div>
