...
export default function ConversationalInterface({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      type: 'assistant',
      content:
        'Hi! What mental models or learning challenges can I help you with?',
      timestamp: new Date(),
    },
  ]);
  ...
  const findRelevantContent = (query: string) => {
    ...
    const relevantModels = allModels.filter(model => {
      const modelText =
        `${model.name} ${model.description} ${model.tags.join(' ')} ${model.principles.join(' ')}`.toLowerCase();
      return (
        keywords.some(keyword => modelText.includes(keyword)) ||
    ...
    const relevantPaths = DEFAULT_LEARNING_PATHS.filter(path => {
      const pathText =
        `${path.title} ${path.description} ${path.tags.join(' ')}`.toLowerCase();
      return (
        keywords.some(keyword => pathText.includes(keyword)) ||
    ...
    const finalPaths =
      relevantPaths.length > 0
        ? relevantPaths
        : DEFAULT_LEARNING_PATHS.slice(0, 3);
    const finalModels =
      relevantModels.length > 0
        ? relevantModels
        : allModels.filter(m => m.difficulty === 'beginner').slice(0, 5);
  ...
  const generateResponse = (userQuery: string) => {
    ...
    if (
      queryLower.includes('10 best') ||
      queryLower.includes('top 10') ||
      (queryLower.includes('best') && queryLower.includes('critical'))
    ) {
      response =
        'Here are the 10 most critical mental models and learning paths to master for thinking excellence:';
    } else if (
      queryLower.includes('best') ||
      queryLower.includes('top') ||
      queryLower.includes('critical') ||
      queryLower.includes('essential')
    ) {
      response =
        'Here are the most critical mental models and learning paths to master:';
    } else if (
      queryLower.includes('learn') &&
      (queryLower.includes('ton') ||
        queryLower.includes('lot') ||
        queryLower.includes('everything'))
    ) {
      response =
        "Great! Here's a comprehensive selection of mental models and learning paths to build your thinking toolkit:";
    } else if (
      queryLower.includes('decision') ||
      queryLower.includes('choice')
    ) {
      response =
        'For better decision-making, these mental models will help you think more clearly:';
    } else if (
      queryLower.includes('business') ||
      queryLower.includes('strategy')
    ) {
      response =
        'For business and strategy, these frameworks will give you competitive advantages:';
    } else if (
      queryLower.includes('system') ||
      queryLower.includes('complex')
    ) {
      response =
        'For systems thinking and complexity, these models will help you see the bigger picture:';
    } else if (queryLower.includes('lead') || queryLower.includes('team')) {
      response =
        'For leadership and influence, these mental models will help you work with people effectively:';
    } else if (queryLower.includes('learn') || queryLower.includes('skill')) {
      response =
        'For learning and skill development, these models will accelerate your growth:';
    } else {
      response =
        'Based on your question, here are some relevant mental models and learning paths:';
    }
      ...
      setTimeout(() => {
        try {
          ...
          const errorMessage: Message = {
            ...
            type: 'assistant',
            content:
              "I'm here to help! Here are some mental models and learning paths that might be useful:",
            timestamp: new Date(),
            recommendations: {
          ...
          <div className='flex items-center space-x-3'>
            ...
            <div>
              ...
              <p className='text-sm text-neutral-600'>
                Get personalized mental model recommendations
              </p>
            </div>
          </div>
          <button
