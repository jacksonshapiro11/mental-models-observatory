...
export default function ContinuationWidget({
  className = '',
}: ContinuationWidgetProps) {
  ...
  const getNextStepComponent = () => {
    ...
    if (!nextStep) {
      return (
        ...
        >
          <div>
            <p className='font-medium text-foundational-800'>
              Explore More Models
            </p>
            <p className='text-sm text-foundational-600'>
              Discover new mental frameworks
            </p>
          </div>
    ...
    const getStepLink = () => {
      switch (nextStep.type) {
        ...
        case 'path':
          return nextStep.slug === 'personalized-onboarding'
            ? '/?personalize=true'
            : `/guide/path/${nextStep.slug}`;
        default:
          return '/models';
      }
    };
  ...
  const getRecentAchievement = () => {
    ...
    const latest = progress.achievements.sort(
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-lg border border-neutral-200 ${className}`}
    >
      {/* Header */}
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-sm text-neutral-600'>
              {insights.totalModelsViewed} models explored •{' '}
              {Math.round(insights.completionRate * 100)}% completion rate
            </p>
          </div>
          <div className='text-right'>
            <div className='text-2xl font-bold text-foundational-600'>
...
export function ContinuationBanner({
  ...
  useEffect(() => {
    ...
    const hasActivity =
      userProgress.modelsViewed.length > 0 || userProgress.quickStartCompleted;
    const isRecentlyActive =
      Date.now() - userProgress.lastUpdated < 24 * 60 * 60 * 1000; // 24 hours
  ...
  return (
    <div
      className={`bg-gradient-to-r from-foundational-500 to-foundational-600 text-white rounded-lg p-4 ${className}`}
    >
      <div className='flex items-center justify-between'>
        <div>
