...
export class DynamicPathGenerator {
  ...
  static generateDynamicPaths(
    ...
    const unseenModels = allModels.filter(
      m => !viewedModelSlugs.includes(m.slug)
    );
    const modelsToUse = unseenModels.length >= 10 ? unseenModels : allModels; // Fallback to all if too few unseen

    console.log(
      `Generating paths: ${unseenModels.length} unseen models out of ${allModels.length} total`
    );
    ...
    const immediatePath = this.generateImmediateSolutionPath(
      modelsToUse,
      context
    );
    ...
    const crossDomainPaths = this.generateCrossDomainPaths(
      modelsToUse,
      context
    );
    const weaknessPaths = this.generateWeaknessTargetedPaths(
      modelsToUse,
      context
    );
    ...
    const explorationPaths = this.generateExplorationPaths(
      modelsToUse,
      context
    );
    const applicationPaths = this.generateApplicationPaths(
      modelsToUse,
      context
    );
    ...
    const uncompletedPaths = paths.filter(
      p => !completedPathIds.includes(p.id)
    );
    console.log(
      `Filtered paths: ${uncompletedPaths.length} uncompleted out of ${paths.length} total`
    );

    // Sort by continuation type priority, then by relevance
    return this.rankAndFilterPaths(
      uncompletedPaths,
      context,
      continuationType,
      viewedModelSlugs
   ...
   */
  private static generateAdvancedPaths(
    allModels: MentalModel[],
    context: any
  ): DynamicLearningPath[] {
    const paths: DynamicLearningPath[] = [];
    ...
    let advancedModels = allModels.filter(
      m =>
    ...
    if (advancedModels.length >= 3) {
      paths.push({
        ...
        description:
          'Dive deeper into sophisticated concepts and advanced applications',
        personalizedReason:
          'Build on your foundation with more complex mental models',
   ...
   */
  private static generateRelatedPaths(
    allModels: MentalModel[],
    context: any
  ): DynamicLearningPath[] {
    const paths: DynamicLearningPath[] = [];
    ...
    let relatedModels = allModels.filter(
      m =>
        m.tags &&
        m.tags.some(
          tag =>
            context.primaryFocus &&
            tag.includes(context.primaryFocus.split('-')[0])
        )
    );

    // Fallback: Use models from similar categories
    if (relatedModels.length < 3) {
      relatedModels = allModels
    ...
    if (relatedModels.length >= 3) {
      paths.push({
        ...
        description:
          "Explore connected ideas that complement what you've learned",
   ...
   */
  private static generateNewDomainPaths(
    allModels: MentalModel[],
    context: any
  ): DynamicLearningPath[] {
    const paths: DynamicLearningPath[] = [];
    ...
    const newDomainModels = allModels.filter(
      m =>
        !m.domainSlug ||
        !context.primaryFocus ||
   ...
   */
  private static generateExplorationPaths(
    allModels: MentalModel[],
    context: any
  ): DynamicLearningPath[] {
    const paths: DynamicLearningPath[] = [];
    ...
    const explorationModels = allModels.filter(
      m =>
        (m.difficulty === 'beginner' && m.slug.includes('foundation')) ||
   ...
   */
  private static generateApplicationPaths(
    allModels: MentalModel[],
    context: any
  ): DynamicLearningPath[] {
    const paths: DynamicLearningPath[] = [];
    ...
    const applicationModels = allModels.filter(
      m =>
   ...
   */
  private static generatePracticalPaths(
    allModels: MentalModel[],
    context: any
  ): DynamicLearningPath[] {
    const paths: DynamicLearningPath[] = [];
    ...
    const practicalModels = allModels.filter(
      m =>
  ...
  private static generateImmediateSolutionPath(
    ...
    const sequencedModels = this.sequenceModelsForChallenge(
      relevantModels,
      challenge
    );
    ...
    return {
      ...
      estimatedTotalTime: this.calculateTotalTime(
        sequencedModels.slice(0, 4),
        context
      ),
      successMetrics: this.generateSuccessMetrics(context),
      adaptiveFeatures: {
        branchingPoints: [
          `After model 2: If still struggling, add reinforcement models`,
        ],
  ...
  private static generateSkillBuildingPaths(
    ...
    const coreSkills = this.identifyCoreSkills(
      challenge,
      context.thinkingPattern
    );
    ...
    coreSkills.forEach(skill => {
      ...
      if (skillModels.length >= 2) {
        paths.push({
          ...
          difficulty: this.determineDifficulty(context),
          estimatedTotalTime: this.calculateTotalTime(
            skillModels.slice(0, 3),
            context
          ),
          successMetrics: [
            `You can apply ${skill} to similar challenges`,
            `You recognize when ${skill} is needed`,
          ],
          adaptiveFeatures: {
  ...
  private static generateDeepMasteryPath(
    ...
    return {
      ...
      models: masteryModels.slice(0, 8).map((model, index) => ({
        ...
        sequenceReason: this.getMasterySequenceReason(
          index,
  ...
  private static generateCrossDomainPaths(
    ...
    relatedDomains.forEach(domain => {
      const crossDomainModels = this.findCrossDomainModels(
        allModels,
        primaryDomain,
        domain
      );
      if (crossDomainModels.length >= 2) {
        paths.push({
          ...
          models: crossDomainModels.slice(0, 3).map((model, index) => ({
            ...
            sequenceReason: this.getCrossDomainSequenceReason(
              index,
              domain,
              primaryDomain
            ),
          ...
          difficulty: this.determineDifficulty(context),
          estimatedTotalTime: this.calculateTotalTime(
            crossDomainModels.slice(0, 3),
            context
          ),
          successMetrics: [
            `You can see your challenge from multiple angles`,
            `You find novel solutions by combining perspectives`,
          ],
          adaptiveFeatures: {
            branchingPoints: [
              `After each model: Connect back to original challenge`,
            ],
    ...
    weaknesses.forEach(weakness => {
      ...
      if (targetedModels.length >= 2) {
        paths.push({
          ...
          difficulty: 'moderate',
          estimatedTotalTime: this.calculateTotalTime(
            targetedModels.slice(0, 3),
            context
          ),
          successMetrics: [
            `You catch yourself before ${weakness.toLowerCase()}`,
            `You have alternative thinking strategies`,
          ],
          adaptiveFeatures: {
  ...
  private static findModelsForChallenge(
    models: MentalModel[],
    challenge: string
  ): MentalModel[] {
    return models.filter(model => {
      const modelText =
        `${model.name} ${model.description} ${model.tags.join(' ')}`.toLowerCase();

      // Decision-making challenges
      if (challenge.includes('decision') || challenge.includes('choose')) {
        return (
          modelText.includes('decision') ||
          modelText.includes('probabilistic') ||
          modelText.includes('bias') ||
          modelText.includes('choice')
        );
      }

      // Business challenges
      if (
        challenge.includes('business') ||
        challenge.includes('startup') ||
        challenge.includes('company')
      ) {
        return (
          modelText.includes('competitive') ||
          modelText.includes('strategy') ||
          modelText.includes('market') ||
          modelText.includes('business')
        );
      }
      ...
      if (
        ...
        challenge.includes('people') ||
        challenge.includes('manage')
      ) {
        return (
          modelText.includes('psychology') ||
          modelText.includes('behavior') ||
          modelText.includes('cooperation') ||
          modelText.includes('social')
        );
      }

      // Creative challenges
      if (
        challenge.includes('creative') ||
        challenge.includes('innovation') ||
        challenge.includes('idea')
      ) {
        return (
          modelText.includes('creative') ||
          modelText.includes('innovation') ||
          modelText.includes('design') ||
          modelText.includes('problem')
        );
      }

      // System challenges
      if (
        challenge.includes('complex') ||
        challenge.includes('system') ||
        challenge.includes('overwhelm')
      ) {
        return (
          modelText.includes('system') ||
          modelText.includes('complex') ||
          modelText.includes('emergence') ||
          modelText.includes('structure')
        );
      }
  ...
  private static identifyCoreSkills(
    challenge: string,
    thinkingPattern: string
  ): string[] {
    const skills = [];

    if (challenge.includes('decision'))
      skills.push('Decision Making', 'Risk Assessment');
    if (challenge.includes('business'))
      skills.push('Strategic Thinking', 'Competitive Analysis');
    if (challenge.includes('team') || challenge.includes('people'))
      skills.push('Human Psychology', 'Influence');
    if (challenge.includes('creative'))
      skills.push('Creative Problem Solving', 'Innovation');
    if (challenge.includes('complex') || challenge.includes('system'))
      skills.push('Systems Thinking', 'Pattern Recognition');
  ...
  private static findModelsForSkill(
    models: MentalModel[],
    skill: string
  ): MentalModel[] {
    const skillKeywords = {
    ...
    return models.filter(model => {
      const modelText =
        `${model.slug} ${model.name} ${model.description}`.toLowerCase();
  ...
  private static rankAndFilterPaths(
    paths: DynamicLearningPath[],
    context: any,
    ...
    for (const scoredPath of sortedPaths) {
      const pathModelSlugs = new Set(
        scoredPath.path.models.map(m => m.model.slug)
      );

      // Check if this path has >50% overlap with any existing path
      const hasTooMuchOverlap = usedModelSets.some(existingSet => {
        const intersection = [...pathModelSlugs].filter(slug =>
    ...
    console.log(
      `Deduplicated: ${uniquePaths.length} unique paths from ${sortedPaths.length} total`
    );
   ...
   */
  private static calculatePathFreshness(
    path: DynamicLearningPath,
    viewedModelSlugs: string[]
  ): number {
    if (path.models.length === 0) return 0;

    const unseenCount = path.models.filter(
      m => !viewedModelSlugs.includes(m.model.slug)
    ).length;
  ...
  private static calculatePathRelevance(
    ...
    if (continuationType === 'deeper') {
      ...
      if (path.title.includes('Advanced') || path.title.includes('Mastery'))
        score += 10;
    } else if (continuationType === 'adjacent') {
      if (path.pathType === 'cross-domain') score += 20;
      if (
        path.title.includes('Related') ||
        path.title.includes('Connected') ||
        path.title.includes('Expansion')
      )
        score += 15;
    } else if (continuationType === 'new') {
      if (
        path.title.includes('New') ||
        path.title.includes('Exploration') ||
        path.title.includes('Territory')
      )
        score += 20;
      ...
      if (
        path.title.includes('Practical') ||
        path.title.includes('Application') ||
        path.title.includes('Work')
      )
        score += 15;
    } else {
    ...
    if (context.timeCommitment === 'micro-learning' && path.models.length <= 3)
      score += 5;
    if (
      context.timeCommitment === 'intensive-learning' &&
    ...
    if (
      context.learningStyle === 'hands-on' &&
      path.pathType === 'skill-building'
    )
      score += 3;
    if (
      context.learningStyle === 'connection-based' &&
      path.pathType === 'cross-domain'
    )
      score += 3;
  ...
  private static sequenceModelsForChallenge(
    models: MentalModel[],
    challenge: string
  ): MentalModel[] {
  ...
  private static getModelReasoningForChallenge(
    model: MentalModel,
    challenge: string
  ): string {
  ...
  private static getSequenceReasoning(
    model: MentalModel,
    index: number,
    challenge: string
  ): string {
    ...
    if (index === 1)
      return 'Core application - apply to your specific situation';
    return 'Advanced integration - master the nuances';
  }

  private static calculateModelTime(
    model: MentalModel,
    context: any,
    multiplier: number = 1
  ): number {
    const baseTime =
      context.timeCommitmentData?.time === '5min'
        ? 8
        : context.timeCommitmentData?.time === '15min'
          ? 18
          : 25;
  ...
  private static calculateTotalTime(
    ...
    const totalMinutes =
      models.length *
      this.calculateModelTime(
        models[0] || ({} as MentalModel),
        context,
        multiplier
      );
  ...
  private static findMasteryModels(
    models: MentalModel[],
    challenge: string,
    context: any
  ): MentalModel[] {
  ...
  private static getMasteryDomain(challenge: string): string {
    return 'your domain';
  }
  private static getMasterySequenceReason(
    index: number,
    total: number
  ): string {
    return `Step ${index + 1} of ${total}`;
  }
  private static identifyPrimaryDomain(challenge: string): string {
    return 'decision-making';
  }
  private static findRelatedDomains(
    primaryDomain: string,
    context: any
  ): string[] {
    return ['psychology', 'systems-thinking'];
  }
  private static findCrossDomainModels(
    models: MentalModel[],
    primary: string,
    related: string
  ): MentalModel[] {
  ...
  private static getCrossDomainSequenceReason(
    index: number,
    domain: string,
    primary: string
  ): string {
    return `Bridge ${domain} to ${primary}`;
  }
  ...
  private static findModelsForWeakness(
    models: MentalModel[],
    weakness: string
  ): MentalModel[] {
    return models.slice(0, 3);
  }
  private static getWeaknessCorrection(
    model: MentalModel,
    weakness: string
  ): string {
    return 'providing alternative approaches';
  }
  private static getWeaknessSequenceReason(
    index: number,
    weakness: string
  ): string {
    return `Address ${weakness} step ${index + 1}`;
  }
  private static getSkillRelevance(skill: string, challenge: string): string {
    return 'it directly impacts your situation';
  }
  private static getSkillDevelopmentReason(
    model: MentalModel,
    skill: string
  ): string {
    return 'practical application';
  }
  private static getSkillSequenceReason(index: number, skill: string): string {
    return `Build ${skill} foundation`;
  }
}
