...
export class ProgressTracker {
  ...
  static trackDomainExploration(slug: string): void {
    ...
    const existingIndex = progress.domainsExplored.findIndex(
      d => d.slug === slug
    );
    if (existingIndex >= 0) {
  ...
  static trackPathProgress(
    ...
    const existingIndex = progress.pathsStarted.findIndex(
      p => p.pathId === pathId
    );
    if (existingIndex >= 0) {
  ...
  static endSession(sessionLength: number): void {
    ...
    progress.averageSessionLength =
      (progress.averageSessionLength * (progress.sessionCount - 1) +
        sessionLength) /
      progress.sessionCount;
  ...
  static getLearningInsights(): {
    ...
    const completedModels = progress.modelsViewed.filter(
      m => m.completed
    ).length;
    const completionRate =
      totalModelsViewed > 0 ? completedModels / totalModelsViewed : 0;

    // Calculate learning streak (days with activity)
    const daysSinceStart = Math.floor(
      (Date.now() - (progress.modelsViewed[0]?.viewedAt || Date.now())) /
        (1000 * 60 * 60 * 24)
    );
  ...
  private static checkAchievements(progress: UserProgress): void {
    const achievements = [
      ...
      },
      {
        ...
        condition: () =>
          progress.modelsViewed.filter(m => m.completed).length >= 10,
      },
      {
        id: 'path_walker',
        title: 'Path Walker',
  ...
  private static updateSuggestedNextSteps(progress: UserProgress): void {
    ...
    if (progress.domainsExplored.length > 0) {
      const strongestDomain = progress.domainsExplored.sort(
        (a, b) => b.modelsViewed - a.modelsViewed
      )[0];
    ...
    progress.suggestedNextSteps = suggestions.sort(
