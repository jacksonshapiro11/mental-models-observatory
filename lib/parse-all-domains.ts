...
export function parseAllDomainFiles(): ModelHighlights[] {
  ...
  try {
    ...
    console.log(
      `Parsed ${uniqueModels.length} unique models from ${files.length} files`
    );
    return uniqueModels;
  } catch (error) {
...
function parseMarkdownFile(filePath: string): ModelHighlights[] {
  ...
  if (jsonMatches && jsonMatches.length > 0) {
    console.log(
      `Processing ${jsonMatches.length} JSON blocks in ${path.basename(filePath)}`
    );
    ...
    for (const jsonMatch of jsonMatches) {
      try {
        ...
        console.error(
          `  ❌ Error parsing JSON in markdown file ${path.basename(filePath)}:`,
...
function parseJsonFile(filePath: string): ModelHighlights[] {
  ...
  try {
    ...
    console.error(
      `Error parsing JSON file ${path.basename(filePath)}:`,
      error instanceof Error ? error.message : String(error)
    );
    ...
    try {
      ...
      console.error(
        `Fallback parsing also failed for ${path.basename(filePath)}:`,
...
function extractPartialModels(
      ...
      try {
        ...
        if (modelData.modelId) {
          models.push({
            ...
            modelDescription:
              modelData.modelDescription || 'Model from corrupted file',
            curatedHighlights: modelData.curatedHighlights || [],
          });
        }
      } catch (e) {
  ...
  console.log(
    `Extracted ${models.length} partial models from corrupted file ${fileName}`
  );
  return models;
}
...
function parseTextFile(filePath: string, fileName: string): ModelHighlights[] {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        return data.map((item: any) => ({
          ...
          modelDescription:
            item.modelDescription || 'Philosophy-based mental model',
          curatedHighlights: item.curatedHighlights || [],
        }));
      }
    } catch (error) {
      console.error(
        `Error parsing text file ${fileName} as JSON:`,
        error instanceof Error ? error.message : String(error)
      );
    }
  }
  ...
  console.log(
    `Text file ${fileName} could not be parsed - may need custom parsing logic`
  );
  return [];
}
...
const MODEL_SLUG_MAPPINGS: { [key: string]: string } = {
  ...
  'models-as-mental-procedures-operating-systems':
    'models-mental-procedures-operating-systems',
  ...
  'language-as-mental-model-reverse-compression':
  ...
  'power-laws-concentration-vs-diversification':
    'power-laws-concentration-diversification',
  ...
  'data-interpretation-aesthetic-vs-functional-truth':
    'data-interpretation-aesthetic-functional-truth',

  // Neuroscience & Consciousness - FIXED (#43)
  'the-hard-problem-consciousness-integration':
  ...
  'story-as-sense-making-mental-model-construction':
    'story-sense-making-mental-models',
  'identity-affirmation-conscious-self-creation':
    'identity-affirmation-conscious-creation',
  ...
  'universality-infinite-reach-from-finite-means':
    'universality-infinite-reach',

  // ALL MODELS NOW WORKING! 🎉
};

// Get highlights for a specific model slug
export function getModelHighlightsFromAllDomains(
  modelSlug: string
): ModelHighlights | null {
  ...
  if (!result && modelSlug === 'energy-as-core-resource-ultimate-constraint') {
    result = allModels.find(
      model => model.modelId === 'energy-core-resource-ultimate-constraint'
    );
  }
