...
export class IntelligentPersonalizationEngine {
  ...
  static analyzeUserLearningProfile(behavior: DetailedUserBehavior): {
    ...
    const avgVelocity =
      domainVelocities.reduce((sum, [, vel]) => sum + vel, 0) /
      domainVelocities.length;

    // Determine learning style from engagement patterns
    const totalHighlightTime = Object.values(
      behavior.highlightEngagement
    ).reduce((sum, eng) => sum + eng.viewTime, 0);
    const avgTimePerHighlight =
      totalHighlightTime / Object.keys(behavior.highlightEngagement).length;

    let learningStyle: 'visual' | 'analytical' | 'practical' | 'exploratory' =
      'analytical';
    if (avgTimePerHighlight > 120)
      learningStyle = 'analytical'; // Deep reading
    else if (
      behavior.applicationAttempts &&
      Object.keys(behavior.applicationAttempts).length > 5
    )
      learningStyle = 'practical';
    else if (
      behavior.visitedDomains.length >
      behavior.visitedModels.length * 0.3
    )
    ...
    const growthAreas = Object.entries(behavior.conceptMastery)
      .filter(
        ([, mastery]) => mastery.masteryScore < 0.4 && mastery.totalTime > 300
      )
      .map(([concept]) => {
        const model = allModels.find(m => m.slug === concept);
        return model?.domain || 'unknown';
    ...
    return {
      ...
      growthAreas,
      interestEvolution:
        behavior.interestDrift.length > 2 ? 'shifting' : 'stable',
    };
  }
  ...
  static scoreContentForUser(
    ...
    const knownRelatedConcepts =
      model.relatedModels?.filter((slug: string) =>
  ...
  static generateAdaptivePaths(
    ...
    if (learningProfile.strongDomains.length > 0) {
      const strengthModels = scoredModels
        .filter(({ model }) =>
          learningProfile.strongDomains.includes(model.domain)
        )
        .slice(0, 5);
    ...
    const crossDomainModels = this.findCrossDomainConnections(
      scoredModels,
      profile
    );
    if (crossDomainModels.length > 0) {
      paths.push({
        ...
        personalizedReason:
          'Based on your interests, these models will reveal surprising connections.',
   ...
   */
  private static findConnectedModels(
    model: MentalModel,
    ...
    allModels
      .filter(
        (m: MentalModel) => m.domain === model.domain && m.slug !== model.slug
      )
      .slice(0, 2)
  ...
  private static estimateTimeForUser(
    model: MentalModel,
    learningProfile: ReturnType<
      typeof IntelligentPersonalizationEngine.analyzeUserLearningProfile
    >
  ): number {
